...
import type { LiveTextData, NodeMap, TextOperation } from "@liveblocks/core";
...
describe("Storage LiveText", () => {
  ...
  test.each<{
    ...
    normalizedOperation: TextOperation;
    expectedData: LiveTextData;
  }>([
    {
      name: "insert",
      operation: { type: "insert", index: 2, text: "X" },
      normalizedOperation: { type: "insert", index: 1, text: "X" },
      expectedData: [["aX😀b"]],
    },
    {
      name: "delete",
      operation: { type: "delete", index: 2, length: 1 },
      normalizedOperation: { type: "delete", index: 1, length: 2 },
      expectedData: [["ab"]],
    },
    {
      name: "format",
      ...
      expectedData: [["a"], ["😀", { bold: true }], ["b"]],
    },
  ])(
    "normalizes a surrogate-splitting $name before accepting it",
    (testCase) => {
      const driver = new InMemoryDriver();
      initInMemory(
        driver,
      ...
      const result = storage.applyOps([
        updateTextOp("0:1", 0, [testCase.operation]),
      ]);

      expect(result[0]).toMatchObject({
        action: "accepted",
        op: { ops: [testCase.normalizedOperation] },
      });
      expect(storage.driver.get_node("0:1")).toMatchObject({
        data: testCase.expectedData,
        version: 1,
      });
      expect(
        storage.driver.get_live_text_history_since("0:1", 0)[0]?.ops
      ).toEqual([testCase.normalizedOperation]);
    }
  );

  test("normalizes surrogate boundaries after rebasing stale operations", () => {
    const driver = new InMemoryDriver();
    initInMemory(
      driver,
      new Map([
    ...
    expect(
      storage.applyOps([
        updateTextOp(
          "0:1",
          0,
          [{ type: "insert", index: 0, text: "Z" }],
          "first"
        ),
      ])[0]?.action
    ).toBe("accepted");

    const staleResult = storage.applyOps([
      updateTextOp(
        "0:1",
        0,
    ...
    expect(staleResult[0]).toMatchObject({
      action: "accepted",
      op: {
        baseVersion: 1,
        version: 2,
        ops: [{ type: "insert", index: 2, text: "X" }],
      },
    });
    expect(storage.driver.get_node("0:1")).toMatchObject({
      data: [["ZaX😀b"]],
  ...
  test("replay acknowledges missed remote edits before a newly accepted operation", () => {
    const driver = new InMemoryDriver();
    initInMemory(
      driver,
      new Map([
    ...
    expect(driver.get_node("0:1")).toMatchObject({
      data: [["Allo"]],
      version: 2,
    });
  });

  test("empty replay recovers missed edits and advances the version only once", () => {
    const driver = new InMemoryDriver();
    initInMemory(
      driver,
      new Map([
    ...
    const storage = new Storage(driver);
    const remoteOps = [{ type: "insert" as const, index: 0, text: "A" }];
    storage.applyOps([updateTextOp("0:1", 0, remoteOps, "remote")]);

    const replay = updateTextOp("0:1", 0, [], "replay");
    expect(storage.applyOps([replay], { includeTextHistory: true })).toEqual([
      {
        action: "accepted",
        op: {
          ...replay,
          baseVersion: 1,
          version: 2,
          history: [{ version: 1, ops: remoteOps }],
        },
      },
    ]);
    expect(driver.get_node("0:1")).toMatchObject({
    ...
    expect(driver.get_live_text_history_since("0:1", 0)).toMatchObject([
      { version: 1, opId: "remote", ops: remoteOps },
      { version: 2, opId: "replay", ops: [] },
    ]);
  ...
  test("replay of a lost ack includes edits before and after the original without reapplying it", () => {
    const driver = new InMemoryDriver();
    initInMemory(
      driver,
      new Map([
      ...
      expect(
      ...
      ).toEqual([
        {
          ...
          ackOp: {
            ...
            history: [
              { version: 1, ops: [{ type: "insert", index: 0, text: "A" }] },
              { version: 2, ops: [{ type: "delete", index: 1, length: 2 }] },
              { version: 3, ops: [{ type: "insert", index: 4, text: "!" }] },
            ],
          },
        },
      ]);
  ...
  test.each([false, true])(
    "rejects incomplete replay history even if the original is retained (duplicate: %s)",
    (duplicate) => {
      const driver = new InMemoryDriver();
      initInMemory(
        driver,
      ...
      expect(result).toMatchObject([
        {
          action: "rejected",
          opIds: [duplicate ? original.opId : "new"],
        },
      ]);
      expect(driver.get_node("0:1")).toMatchObject({
        data: [["Allo"]],
