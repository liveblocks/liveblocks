...
export function putRandom(stub: Stub, key: string) {
  stub.set(key, Math.floor(Math.random() * 1_000_000));
}

export function putAndFail(stub: Stub, key: string, value: Json) {
  put(stub, key, value);
  throw new Error("b0rked");
}

export function dupe(stub: Stub, src: string, target: string) {
...
export function inc(stub: Stub, key: string) {
  const count = stub.getNumber(key) ?? 0;
  stub.set(key, count + 1);
}

export function dec(stub: Stub, key: string) {
  const count = stub.getNumber(key) ?? 0;
  if (count <= 0) {
