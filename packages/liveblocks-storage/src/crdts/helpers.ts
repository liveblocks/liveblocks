...
export function liveObjectFromNodeStream(
  ...
  return LiveObject._fromItems(nodes, doc);
}
...
export function deserialize(
  ...
  if (isObjectStorageNode(node)) {
    return LiveObject._deserialize(node, parentToChildren, doc);
  } else if (isListStorageNode(node)) {
    return LiveList._deserialize(node, parentToChildren, doc);
  } else if (isMapStorageNode(node)) {
    return LiveMap._deserialize(node, parentToChildren, doc);
  } else if (isRegisterStorageNode(node)) {
    return LiveRegister._deserialize(node, parentToChildren, doc);
  } else {
 ...
 */
export function dumpDoc(doc: StorageDoc): string {
  const rows = Array.from(doc.nodes.values(), (node) => {
...
export { mergeStorageUpdates } from "./mergeStorageUpdates";
