...
<PropertiesList title="Parameters">
  ...
  <PropertiesListItem name="query" type="str | Unset">
    Query to filter rooms. You can filter by `roomId` and `metadata`, for
    example, `metadata["roomType"]:"whiteboard" AND
    roomId^"liveblocks:engineering"`. Learn more about [filtering rooms with
    query
