...
describe("useMarkThreadAsUnresolved", () => {
  ...
  test.fails(
    ...
    async () => {
      ...
      let resolveMarkAsUnresolved: (() => void) | undefined;
      const markAsUnresolved$ = new Promise<void>((resolve) => {
        resolveMarkAsUnresolved = resolve;
      });
      ...
      server.use(
        ...
        mockMarkThreadAsUnresolved({ threadId: initialThread.id }, async () => {
          await markAsUnresolved$;
          return HttpResponse.json(null, { status: 200 });
        })
      );
