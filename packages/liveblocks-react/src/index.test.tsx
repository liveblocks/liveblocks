...
describe("Storage (with Suspense)", () => {
  test("useObject initialization", async () => {
    ...
    expect(screen.getByTestId(testId("liveObject")).textContent).toEqual(
      "Suspended..."
    );
    ...
    await waitFor(() =>
      expect(screen.getByTestId(testId("liveObject")).textContent).toBe(
        JSON.stringify({ a: 0 })
      )
    );
  });
  ...
  test("unmounting useObject while storage is loading should not cause a memory leak", async () => {
    ...
    act(() => {
      fireEvent.click(screen.getByTestId(testId("unmount")));
    });
