 ...
 *
 * They come in two independent flavours. Don't conflate them:
 *
 *  - LOST UPDATE (test 1): undoing a `set` destroys a *different*, concurrently
 *    inserted item because the undo identifies its victim by position *key*
 *    rather than by *id*, and two independently created items can share a key.
 *    The clients still agree afterwards, they just silently agree on the wrong
 *    thing, so a pure convergence check does NOT catch it.
 *
 *  - NON-CONVERGENCE (tests 2 and 3): a `set` and a `delete` race on the same
 *    item and both clients then undo. The set's value survives as a phantom on
 *    one client only. This needs no key collision at all and is what the
 *    `list-property` convergence assertion trips over.
 */

/**
 * TEST 1 - lost update (clients converge, but on the wrong state).
 *
...
/**
 * TEST 2 - non-convergence: phantom survivor of an undone `set`.
 *
...
/**
 * TEST 3 - same race as test 2, but with one extra A->server sync before the
 * undos. The phantom now surfaces as a DUPLICATE instead of a divergence-by-one:
 *
