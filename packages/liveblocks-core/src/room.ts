...
>(
  ...
  function onDispatch(
    ...
    // unless the ops were generated by WASM itself (skipWasmSync).
    if (context.wasmShadow && ops.length > 0 && !options?.skipWasmSync) {
      try {
  ...
  function createOrUpdateRootFromMessage(nodes: NodeMap) {
    ...
    if (context.root !== undefined) {
      ...
      // that pool.generateId() delegates to WASM from the very start.
      // This makes WASM the sole IdGenerator — no JS counter handoff needed.
