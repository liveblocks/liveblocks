 ...
 *
 * Verifies that JSHistoryEngine (and by extension WasmHistoryEngine, which
 * mirrors the same logic in Rust) produces identical outcomes to the original
 * inline JS code in room.ts for:
...
import { JSHistoryEngine } from "../room-engine";
...
describe("JSHistoryEngine", () => {
  let engine: HistoryEngine<P>;

  beforeEach(() => {
    engine = new JSHistoryEngine<P>();
  });
