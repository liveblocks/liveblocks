...
describe("transformCommentBody", () => {
  ...
  describe("transforms in an CommentBodyJson", () => {
    ...
    test.each<[string, CommentBody, CommentBodyJson]>([
      [
    ...
    ])("transforms %s", async (_, body, expected) => {
      await expect(
        transformCommentBody(body, { format: "json", resolveUsers })
