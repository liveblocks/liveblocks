...
const myMapping = {
  bears: "sync-with-storage",
  maxBears: "sync-with-storage",
  cursor: "sync-with-presence",
} as const;
...
> = {
  [K in keyof TMapping]: TMapping[K] extends "sync-with-presence" ? K : never;
}[keyof TMapping] &
  keyof TState &
  string;

type _Test1 = PresenceKeys<
  MyState,
  { bears: "sync-with-storage"; cursor: "sync-with-presence" }
>;
...
type StorageKeys<
  ...
  TMapping extends MappingConfig<TState>
> = {
  [K in keyof TMapping]: TMapping[K] extends "sync-with-storage" ? K : never;
}[keyof TMapping] &
  keyof TState &
  string;
...
type PresenceFromLiveblocksState<T> = T extends LiveblocksState<
  any,
  infer TPresence,
  any,
  any,
...
type StorageFromLiveblocksState<T> = T extends LiveblocksState<
  any,
  any,
  infer TStorage,
  any,
...
type UserMetaFromLiveblocksState<T> = T extends LiveblocksState<
  any,
  any,
  any,
  infer TUserMeta,
...
type RoomEventFromLiveblocksState<T> = T extends LiveblocksState<
  any,
  any,
  any,
  any,
...
type MappingConfig<TState extends Pojo> = {
  [K in JsonKeys<TState> & keyof TState]?:
    | "sync-with-presence"
    | "sync-with-storage";
};
...
> {
  return (options) => {
    ...
    return (_set, _get, api) => {
      // NOTE: In Zustand v4, `get` and `set` are potentially undefined. We'll
