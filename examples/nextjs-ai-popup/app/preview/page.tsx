...
import { ClientSideSuspense, useSendAiMessage, useAiChats, useDeleteAiChat } from "@liveblocks/react";
      ...
      <div className="absolute inset-0">
        {config.url && config.urlType === "iframe" ? (
          ...
          <div className="w-full h-full bg-neutral-100 dark:bg-neutral-900 flex items-center justify-center">
            <p className="text-neutral-500 dark:text-neutral-400">No content URL provided</p>
          </div>
        )}
      </div>
...
function Chat({ chatId, config }: { chatId: string; config: any }) {
  const aiChatProps: any = {
    ...
    chatId,
    ...(config.copilotId ? { copilotId: config.copilotId } : {})
  };
  ...
  return (
    <div className="absolute inset-0 flex flex-col">
      <AiChat
        {...aiChatProps}
        style={config.accentColor ? { "--lb-accent": config.accentColor } as React.CSSProperties : undefined}
        components={{
          Empty: ({ chatId }) => {
            ...
            return (
              <div className="p-[var(--spacing)] h-full flex flex-col gap-5 justify-end">
                <div>
                  <h3 className="text-neutral-900 dark:text-neutral-100">{config.title}</h3>
                  {config.description && (
                    <p className="text-sm text-neutral-600 dark:text-neutral-400 mt-1">{config.description}</p>
                  )}
                </div>
...
function ChatListing({
  ...
  if (error) {
    return <div className="text-neutral-900 dark:text-neutral-100">error: {error.message}</div>;
  }
