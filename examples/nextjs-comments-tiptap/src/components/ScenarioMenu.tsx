...
export function ScenarioMenu() {
  ...
  return (
    <div className="fixed left-4 top-1/2 -translate-y-1/2 z-50">
      <div className="bg-white border border-border rounded-sm shadow-xl p-3">
        ...
        <Select.Root value={scenario} onValueChange={updateScenario}>
          ...
          <Select.Portal>
            <Select.Content className="z-50 bg-surface-elevated border border-border rounded-sm shadow-xl overflow-hidden">
              <Select.Viewport>
                {scenarios.map((item) => (
                  <Select.Item
                    key={item.value}
