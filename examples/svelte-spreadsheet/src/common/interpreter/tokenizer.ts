...
export type Token = NumberToken | CellToken | RefToken | SimpleCharToken | EOFToken | FunctionToken;
...
const syntaxKindToChar = new Map(
	Array.from(simpleCharToSyntaxKindMap.entries()).map((entry) => [entry[1], entry[0]])
);
