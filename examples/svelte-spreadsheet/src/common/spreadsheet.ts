...
import { LiveObject, type Room, type BaseUserMeta, type User } from '@liveblocks/client';
import type { Presence, Storage, Column, Row } from '../types';
...
export type LiveSpreadsheet = {
	...
	onOthersChange(callback: (others: User<Presence, BaseUserMeta>[]) => void): () => void;
...
export async function createSpreadsheet(
	...
	function insertColumn(index: number, width: number) {
		liveSpreadsheet.get('columns').insert(new LiveObject({ id: nanoid(), width }), index);
	}

	function insertRow(index: number, height: number) {
		liveSpreadsheet.get('rows').insert(new LiveObject({ id: nanoid(), height }), index);
	}
	...
	function deleteColumn(index: number) {
		...
		for (const row of liveSpreadsheet.get('rows')) {
	...
	function deleteRow(index: number) {
		...
		for (const column of liveSpreadsheet.get('columns')) {
			liveSpreadsheet.get('cells').delete(column.get('id') + row!.get('id'));
		}
	}
	...
	function refToCell(token: RefToken): CellToken {
		...
		const rowIndex = liveSpreadsheet.get('rows').findIndex((row) => row.get('id') === rowId);
	...
	function updateCellValue(columnId: string, rowId: string, value: string) {
		...
		const tokensWithRefs = tokens.map((token) =>
			token.kind === SyntaxKind.CellToken ? cellToRef(token as CellToken) : token
		);
	...
	function selectedCell(coordinates: { columnId: string; rowId: string } | null) {
		room.updatePresence({
			selectedCell: coordinates ? coordinates.columnId + coordinates.rowId : null
		});
	}

	function evaluateCellRef(ref: string): number {
		const result = evaluateCell(ref.substring(0, ref.length / 2), ref.substring(ref.length / 2));
		if (result.type !== 'number') {
			throw new Error(
				`expected an expression result of type number but got ${JSON.stringify(result)}`
			);
		}
		return result.value;
	}
	...
	const othersCallbacks: Array<(others: User<Presence, BaseUserMeta>[]) => void> = [];
	function onOthersChange(callback: (others: User<Presence, BaseUserMeta>[]) => void) {
	...
	room.subscribe(
		...
		() => {
			const columns = liveSpreadsheet.get('columns').map((col) => col.toObject());
	...
	function onCellsChange(callback: (cells: Record<string, string>) => void) {
		...
		const cells = Object.fromEntries(
			Array.from(liveSpreadsheet.get('cells').entries()).map(([key, cell]) => [
				key,
				getCellDisplay(key.substring(0, key.length / 2), key.substring(key.length / 2))
			])
		);
		callback(cells);
