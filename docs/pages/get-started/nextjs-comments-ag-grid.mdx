...
<Steps>
  ...
  <Step>
    ...
    <StepContent>
      ...
      Now that we’re connected to a room, we can [set up AG grid](https://www.ag-grid.com/react-data-grid/getting-started/)
      andstart using the Liveblocks hooks and components.
      ...
      type RowData = { name: string; price: number };
      ...
      function CellCommentButton({
        ...
        rowId: string;
        columnId: string;
        openCell: { rowId: string; columnId: string } | null;
        setOpenCell: (openCell: { rowId: string; columnId: string } | null) => void;
      }) {
