...
collaborative experiences. Follow this guide to notify users when an AI agent
has completed its work, using a custom in-app notification in your Next.js
`/app` directory application, with the hooks from
[`@liveblocks/react`](/docs/api-reference/liveblocks-react), the components
from [`@liveblocks/react-ui`](/docs/api-reference/liveblocks-react-ui), and
