...
definitely be used without TypeScript. We believe typings is helpful to make
...
```tsx file="src/liveblocks.config.ts" highlight="2,8"
import { createClient } from "@liveblocks/client";
...
To start using presence, let’s define a type `Presence` in
`liveblocks.config.ts` and use it as a generic argument of `createRoomContext`.
All of our _presence_ hooks returned `createRoomContext` will be according to
the newly defined `Presence` type.
...
To use storage, first define a type `Storage` in `liveblocks.config.ts`, like we
did for `Presence`. In this example we’ll define a [`LiveObject`][] called
`scientist`, containing first and last name properties.

```tsx file="liveblocks.config.ts" highlight="4-6,11"
...
loaded from the server. This quickly become cumbersome to handle `null` whenever
we use `useStorage`, we have some good new for you; `@liveblocks/react` contains
a [`Suspense`](https://beta.reactjs.org/reference/react/Suspense) version of all
of our hooks.

### Using Suspense

If you can use `Suspense` in your application, re-export our hooks from
`liveblocks.config.ts` like this.
...
If you’re using a framework that support Server Side Rendering like
...
using `useStorage` will never be able to render. To still benefit from
