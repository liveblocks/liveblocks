...
All of our _presence_ hooks returned by `createRoomContext` will be typed correspondingly to
...
To use storage, first define a type named `Storage` in `liveblocks.config.ts`, like we
...
loaded from the server. It can quickly become cumbersome to handle `null` whenever
we use `useStorage`, but we have some good new for you; `@liveblocks/react` contains
a [`Suspense`](https://beta.reactjs.org/reference/react/Suspense) version of all
of our hooks.

### Using Suspense

If you’d like to use `Suspense` in your application, make sure to re-export our hooks from
