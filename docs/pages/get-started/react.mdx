...
To use Liveblocks, we’ll need to sign up and get an API key.
...
```ts file="src/liveblocks.config.ts" highlight="2,8-10"
import { createClient } from "@liveblocks/client";
...
```tsx file="src/index.tsx" highlight="7,11"
import App from "./App";
...
```ts file="liveblocks.config.ts" highlight="11"
import { createClient } from "@liveblocks/client";
import { createRoomContext } from "@liveblocks/react";
...
const cursor:Cursor= { x: 256, y: 367 }
```

To start using presence, we must define an `initialPresence` value on our
`RoomProvider`. We’ll set the initial cursor to `null` to represent a user whose
cursor is currently off-screen.

```tsx file="src/index.tsx" highlight="7"
import App from "./App";
import { ClientSideSuspense } from "@liveblocks/react";
import { RoomProvider } from "./liveblocks.config.js";
...
```ts file="liveblocks.config.js" highlight="7"
// ...
...
```tsx file="src/index.tsx" highlight="11-16"
import App from "./App";
import { ClientSideSuspense } from "@liveblocks/react";
...
```ts
// Define mutation in typescript
const updateName = useMutation(
  ({ storage }, nameType: "firstName" | "lastName", newName: string) => {
...
use it with presence, for example, there is no reason to undo the position of a
...
const updateSelected = useMutation(({ setMyPresence }, rectangleId: string) => {
