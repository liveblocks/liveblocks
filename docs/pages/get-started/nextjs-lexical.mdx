...
<Steps>
  ...
  <Step>
    ...
    <StepContent>
      ...
      To create threads with the [`FloatingComposer`](/docs/api-reference/liveblocks-react-lexical#FloatingComposer),
      we need to create a button that opens the composer. Create a toolbar in `Toolbar.tsx`
      with a button that opens the composer using [`OPEN_FLOATING_COMPOSER_COMMAND`](/docs/api-reference/liveblocks-react-lexical#Opening-the-composer).
