...
You can also
[follow our step-by-step tutorial](/docs/tutorials/multiplayer-to-do-list/react-redux)
...
Liveblocks and react/ redux dependencies:
...
export type LiveBlocksState = WithLiveblocks<
...
function YourComponent() {
  ...
  return (
    <div
      ...
      onPointerMove={(e:PointerEvent) => dispatch(setCursor({ x: e.clientX, y: e.clientY }))}
    />
  );
}
```
...
const slice = createSlice({
  ...
  reducers: {
    ...
    addDiscovery: (state, action: PayloadAction<string>) => {
