...
Use our `liveblocksEnhancer` in your store setup and pass the client created
previously as the configuration. This will add a new state called `liveblocks`
to your store, enabling you to interact with our Presence and Storage APIs.
...
going to add a new `presenceMapping` option to our `liveblocksEnhancer` to
specify which part of the state maps to the current user’s `presence`.
