...
<Step>
  ...
  <StepContent>
    ...
    [`useLiveblocksExtension`](/docs/api-reference/liveblocks-react-blocknote#useLiveblocksExtension)
...
<Step>
  ...
  <StepContent>
    ...
    {/* TODO check if step needed after package release */}

    BlockNote text editor is unstyled by default, so we can create some custom styles for it in a `globals.css` file. Import `globals.css`, alongside the default Liveblocks styles. You can import them into the
