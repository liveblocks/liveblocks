...
with the default
[`Thread`](/docs/api-reference/liveblocks-react-comments#Thread) component.
...
The only information Comments stores about users is their user ID, which is set
when [authenticating with Liveblocks](/docs/rooms/authentication). With the
...
[`resolveUsers`](/docs/api-reference/liveblocks-react#resolveUsers) function in
