...
The `<Composer />` component is used to create threads and comments, and by
default it create a new thread when its form is submitted.
...
When using it to create a thread, you can pass custom metadata, which will be
attached to the thread. You can also import your `ThreadMetadata` type from your
config file to help you prevent mistakes.
