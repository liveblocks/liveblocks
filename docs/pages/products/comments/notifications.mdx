...
When using the access token authentication method, Liveblocks will return all
notifications created for the user, without taking into consideration the
permissions contained in the access token. This means that if a user is
mentioned in a comment in a room to which they don't have access, they will
still see the notification.

If you would like to have more control over the notifications permissions, we
recommend using [IDs tokens](/docs/authentication/id-token).

### ID token

When using the ID token authentication method, Liveblocks will use the rooms
accesses configured to return notifications that the user has access to.
