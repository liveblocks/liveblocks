...
When employing the access token authentication method, Liveblocks disregards the
permissions specified in the access token and returns all notifications created
for the user. Consequently, a user will receive notifications for mentions in
comments within rooms they do not have access to.
...
With the ID token authentication method, Liveblocks respects the configured room
access permissions to deliver notifications that the user is authorized to view.
