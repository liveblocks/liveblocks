...
_TODO: Insert image showing an InboxNotification component_

Inbox notifications are what a user usually sees when clicking on 🔔 in an app.

A single inbox notification regroup all the activities that happened on a single
thread.

Inbox notifications belong to a single user. When an inbox notification is
marked as read, the associated thread is also marked as read. The opposite is
also true; when a thread is marked as read, the associated inbox notification is
marked as read.
...
- They have been mentioned in a thread
- Someone posted a comment on a thread they have been participating in

_TODO: Insert image showing room notification settings like Notion or Figma_

This default behavior can be **overridden** by changing the notification
settings at the room level

For example, if someone created a document, they will likely want to receive a
notification for every new thread associated with it. Room notification settings
are defined per user.
...
_TODO: Insert InboxNotification email default template_

We're not going to send emails for you for the following reasons:

- You might not want to share your users emails with us for compliance reasons
  (GDPR, SOC 2)
- You most likely already have your own infrastructure for transactional emails
  (branded templates and sender reputation)

Instead, we do as much as possible to help you handle email notifications
without actually sending the emails. We're using webhooks to notify you when to
send an email to your users and we provide utilities to help you craft the email
template.
