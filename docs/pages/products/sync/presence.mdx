...
[TODO: Video of avatars, cursors, agent selection, typing indicator]

<Banner title="Learn with our interactive tutorial">
  Our [interactive tutorial](/docs/tutorial/react/getting-started) walks you
  through the basics of presence, including setting up live cursors.
</Banner>
...
Next, when [authenticating](/docs/authentication) Liveblocks, pass corresponding
user info from your auth system to `userInfo` inside your authentication
endpoint. Any string, number, or boolean property can be passed in.
...
const { output: content } = await generateText({
  ...
  prompt: "Write a paragraph about realtime collaboration",
});

await liveblocks.mutateStorage(roomId, ({ root }) => {
  const document = root.get("document");
  document.set("content", record.content);
});
