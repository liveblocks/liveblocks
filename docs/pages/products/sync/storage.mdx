...
JavaScript object that is synchronized on all clients. Use this for storing
...
JavaScript array that is synchronized across clients. Use this for storing
ordered collection of items, for example a list of layers on a canvas.
...
JavaScript map that is synchronized across clients. Use this for storing
key-value pairs, for example a list of users and their upvotes.
...
collaborative rich text data. It’s generally most useful when used internally by
a [text editing](/docs/products/sync/text-editing) integration, but it can be
