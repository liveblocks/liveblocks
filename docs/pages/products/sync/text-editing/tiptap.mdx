...
meta:
  ...
  description: "Learn about using Liveblocks with Tiptap"
---
...
features. Each [collaborative room](/docs/concepts#Rooms) in your application
can store one document each, and these documents are persisted on the cloud,
visible on [your dashboard](/dashboard), and are integrated into other
Liveblocks products such as [Comments](/docs/products/comments) and
...
<Banner title="LiveText vs Yjs">
  ...
  collaboration](/docs/guides/livetext-vs-yjs).
</Banner>
...
Skip the information and get started now.
...
<ListGrid columns={2} defaultVisibleItems={2}>
  ...
  <DocsCard
    type="technology"
    title="Get started with Tiptap and LiveText"
    href="/docs/get-started/nextjs-tiptap-storage"
    description="LiveText-backed text editor with Next.js"
    visual={<DocsNextjsIcon />}
  />
  <DocsCard
    type="technology"
  ...
  <DocsCard
    ...
    description="LiveText-backed text editor with React"
    visual={<DocsReactIcon />}
  />
</ListGrid>
...
When [authenticating your users](/docs/api-reference/authentication) with
...
[get started guides](/docs/get-started/nextjs-notifications-in-app) to learn how
to set up
...
[`TextMention`](/docs/api-reference/webhook-events#TextMention-notification)
notification webhook events. Learn more about
...
Tiptap uses Yjs-backed collaboration by default. You can opt into
Liveblocks-backed Storage instead by passing `collaborationMode: "liveblocks"`.
...
In this mode, documents are stored in `root._tiptap_docs`, keyed by `field`.
This makes it easy to render editors dynamically from Storage.

```tsx
function Editors() {
  const fields = useStorage((root) => Object.keys(root._tiptap_docs ?? {}));

  return fields.map((field) => <Editor key={field} field={field} />);
}
```

`offlineSupport_experimental` and `ai` stay Yjs-only, as do server-side editing
with
...
Follow the [React](/docs/get-started/react-tiptap-storage) or
...
Choose a starting point for your Tiptap editor.
