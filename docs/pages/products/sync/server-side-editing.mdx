...
Sync data can be modified from your back end, using the same
[conflict resolution](/docs/products/sync/conflict-resolution) that enables
multiplayer collaboration on the front end. This allows you to create initial
room data, schedule jobs, run migrations,
...
[`LiveList`](/docs/products/sync/data-types#livelist) of
[`LiveObject`](/docs/products/sync/data-types#liveobject) shapes
...
[conflict-free data types](/docs/products/sync/data-types) used by the front
end. For example, to add a new shape from the server, get the `shapes` list with
...
You can edit ProseMirror, Tiptap, and BlockNote text documents from the server
using
...
You can edit Lexical text documents from the server using
