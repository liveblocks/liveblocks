...
alongside `LiveObject`, `LiveList`, and `LiveMap`. Because text is is part of
the same document as the rest of your collaborative data, you read and write to
it using the same APIs, plus edits share one undo history, and restoring a
version restores everything together.

We recommend `LiveText` as the default choice for new applications. It currently
supports Tiptap, BlockNote, ProseMirror, and CodeMirror—Tiptap is
...
We recommend Yjs when your editor only has a Yjs binding, or when you need
something only Yjs provides.
