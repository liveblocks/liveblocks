...
Here’s an example of an inbox notification object for a custom notification.
...
[project-based](/docs/concepts#Projects), which means that
a user can receive notifications that were sent from other
[rooms](/docs/concepts#Rooms). This differs to other
...
[Comments](/docs/products/comments) can send `thread` notifications for you
automatically, notifying you of events such as a mention, or a new reply in a
thread you’re participating in. Additionally,
[text editors](/docs/use-cases/text-editor) can send `textMention` notifications
when users are mentioned inside the editor.
...
[email notifications](/docs/products/notifications/email-notifications) page.
...
[ID tokens](/docs/api-reference/authentication#id-token).
