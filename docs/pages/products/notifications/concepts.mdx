...
[project-level](/docs/concepts/how-liveblocks-works#Project), which means that a
user can receive notifications that were sent from other
[rooms](/docs/concepts/how-liveblocks-works#Rooms). This is different to other
Liveblocks products which are room-based.

## Notification kinds

Each inbox notification has a `kind`, and this represents the type of
notification it is. A `kind` can be a custom string (e.g.
`$myCustomNotification`) when sending a custom notification, or it can be a
value set by Liveblocks (e.g. `thread`) when it’s sent by a Liveblocks product.

### Custom

When sending custom notifications
