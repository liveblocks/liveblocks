...
meta:
  title:
    "Creating a collaborative online whiteboard with React, Redux, and
    Liveblocks"
...
React, Redux and Liveblocks. As users add and move rectangles in a canvas,
...
```bash
npx create-react-app redux-whiteboard --template typescript
```
...
export type State = {
  /* Your state type will be defined here */
};
...
export type LiveblocksState = WithLiveblocks<State>;
```

And edit `src/index.js` to add the react-redux provider to your app:

```ts highlight="6,7,11,13" file="src/index.tsx"
...
```ts file="src/store.ts" highlight="10,27"
import { createClient } from "@liveblocks/client";
...
Afterward, we draw the shapes inside our canvas. To keep it simple for the
tutorial, we are going to only support rectangles.

```tsx file="src/App.tsx" highlight="3,9-10,25-35,38-48"
import { useEffect } from "react";
import "./App.css";
...
```ts file="src/store.ts" highlight="9-17,27-35,39"
import { createClient } from "@liveblocks/client";
import { liveblocksEnhancer, withLiveBlocks } from "@liveblocks/redux";
import { configureStore, createSlice } from "@reduxjs/toolkit";
...
export default store;
export type LiveblocksState = WithLiveblocks<State, JsonObject, BaseUserMeta>;
```

Then add a button to dispatch this action from the board.

```tsx file="src/App.tsx" highlight="6,18,24-27"
import { useEffect } from "react";
import "./App.css";
import { useDispatch, useSelector } from "react-redux";
import { actions } from "@liveblocks/redux";

import { insertRectangle, LiveblocksState } from "./store";

const roomId: string = "redux-whiteboard";

export default function App() {
//shapes using Liveblocks state
  const shapes = useSelector<LiveblocksState, LiveblocksState["shapes"]>(
    (state) => state.shapes );
  // const shapes = useSelector((state) => state.shapes);
  const isLoading = useSelector<LiveblocksState, boolean>(
...
```ts file="src/store.ts" highlight="5,15-17,21,30"
/* ... */
...
```tsx file="src/App.tsx" highlight="6,13-14,23-31,36,38,50-51,59,61-64"
import { useEffect } from "react";
import "./App.css";
import { useDispatch, useSelector } from "react-redux";
import { actions } from "@liveblocks/redux";

import { insertRectangle, onShapePointerDown, LiveblocksState } from "./store";

const roomId = "redux-whiteboard";

export default function App() {
  const shapes = useSelector<LiveblocksState, LiveblocksState["shapes"]>((state) => state.shapes);
  const isLoading = useSelector<LiveblocksState, boolean>((state) => state.liveblocks.isStorageLoading);
  const others = useSelector<LiveblocksState, LiveblocksState["liveblocks"]["others"]>((state) => state.liveblocks.others);
  const selectedShape = useSelector<LiveblocksState, LiveblocksState["selectedShape"]>((state) => state.selectedShape);
  const dispatch = useDispatch();
...
```ts file="src/store.ts" highlight="20,25-30,34"
/* ... */
...
```tsx file="src/App.tsx" highlight="6,25-30"
import { useEffect } from "react";
import "./App.css";
import { useDispatch, useSelector } from "react-redux";
import { actions } from "@liveblocks/redux";

import { insertRectangle, onShapePointerDown, deleteShape, LiveblocksState } from "./store";

const roomId = "redux-whiteboard";

export default function App() {
  const shapes = useSelector<LiveblocksState, LiveblocksState["shapes"]>((state) => state.liveblocks.shapes);
  const isLoading = useSelector<LiveblocksState,boolean>((state) => state.liveblocks.isStorageLoading);
  const selectedShape = useSelector<LiveblocksState, LiveblocksState["selectedShape"]>((state) => state.liveblocks.selectedShape);
  const others = useSelector<LiveblocksState, LiveblocksState["others"]>((state) => state.liveblocks.others);
...
```ts file="src/store.ts" highlight="6,18,23-31,39-40"
/* ... */
...
export default store;
export type LiveblocksState = withLiveblocksState<State>;
```

```tsx file="src/App.tsx" highlight="10-11,30-36"
import { useEffect } from "react";
import "./App.css";
import { useDispatch, useSelector } from "react-redux";
import { actions } from "@liveblocks/redux";
...
```tsx file="src/App.tsx" highlight="12,48-53"
import { useEffect } from "react";
import "./App.css";
import { useDispatch, useSelector } from "react-redux";
import { actions } from "@liveblocks/redux";
...
```ts file="src/App.ts" highlight="36,59"
import { useEffect } from "react";
import "./App.css";
import { useDispatch, useSelector } from "react-redux";
import { actions } from "@liveblocks/redux";
...
In this tutorial, we’ve learned about the concept of rooms, presence, and others.
We've also leared how to put all these into practice, and how to persist state
using storage too.
