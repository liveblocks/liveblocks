...
so `ClientSideSuspense` ensure nothing its children are not rendered on the
server. More information can be found
[here](/docs/api-reference/liveblocks-react#suspense-avoid-ssr).

```tsx file="pages/index.tsx" highlight="1,7-9,14-16"
