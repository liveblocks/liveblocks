---
meta:
  title: "Creating a multiplayer to-do list with JavaScript and Liveblocks"
  parentTitle: "Tutorials"
  description: "Build a multiplayer to-do list with JavaScript and Liveblocks"
---
...
- [React tutorial](/docs/tutorials/multiplayer-to-do-list/react)
...
```bash
npm install @liveblocks/client esbuild typescript --save-dev
```
If you do not already have a typescript configuration file, you can create one with `npx tsc --init`.
Or you can create the file with the following options:
```ts file=tsconfig.json
{
  "compilerOptions": {
    "target": "es2016", 
    "module": "commonjs",  
...
```json
"scripts": {
  "build": "esbuild app.ts --bundle --outfile=static/app.ts"
},
```
...
And replace `app.ts` content with the code below, build your app with
`npm run build` and open `static/index.html` in multiple browser windows.

```ts file="app.ts" highlight="12-16"
import { createClient } from "@liveblocks/client";
...
function run() {
  ...
  const whoIsHere: HTMLElement = document.getElementById("who_is_here");
...
_If you want to make your app look a bit more polished while following along,
...
function run() {
  ...
  const whoIsHere: HTMLElement = document.getElementById("who_is_here");
  const todoInput: HTMLElement = document.getElementById("todo_input");
...
```ts highlight="14,19-23" file="app.ts"
import { createClient } from "@liveblocks/client";
...
function run() {
  ...
  const whoIsHere: HTMLElement = document.getElementById("who_is_here");
  const todoInput: HTMLElement = document.getElementById("todo_input");
  const someoneIsTyping: HTMLElement = document.getElementById("someone_is_typing");
...
[`LiveList.push`][] when the user presses "Enter".

```ts highlight="1,8-11,17-19,28" file="src/app.ts"
...
```ts highlight="16,34-56" file="src/app.ts"
import { createClient, LiveList } from "@liveblocks/client";
...
async function run() {
  ...
  const todoInput: HTMLElement = document.getElementById("todo_input");
  const someoneIsTyping: HTMLElement = document.getElementById("someone_is_typing");
  const todosContainer: HTMLElement = document.getElementById("todos_container");
...
```ts highlight="48-54" file="src/app.ts"
import { createClient, LiveList } from "@liveblocks/client";
