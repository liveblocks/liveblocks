---
meta:
  title: "Creating a multiplayer to-do list with React, Redux, and Liveblocks"
  parentTitle: "Tutorials"
  description:
    "Build a multiplayer to-do list with React, Redux, and Liveblocks"
---
...
updates in real-time across clients. Users will also be able to see who else is
...
```bash
npx create-react-app redux-todo-app --template typescript
```
...
export type LiveblocksState = WithLiveblocsk<State>
```

We need to edit `src/index.js` to add the react-redux provider:

```tsx highlight="6,7,11,13" file="src/index.tsx"
...
```tsx highlight="2,7-17,35-37" file="src/App.tsx"
import React, { useEffect } from "react";
import { useDispatch, useSelector } from "react-redux";
import { actions } from "@liveblocks/redux";
import { LiveblocksState } from "./store";

import "./App.css";
function WhoIsHere() {
  const othersUsersCount = useSelector<LiveblocksState, number]>(
...
```ts highlight="10-11,18-21,25,33" file="src/store.ts"
import { createClient } from "@liveblocks/client";
...
```tsx highlight="5,11-19,22,40-47" file="src/App.tsx"
import React, { useEffect } from "react";
import { useDispatch, useSelector } from "react-redux";
import { actions } from "@liveblocks/redux";
import { LiveblocksState } from "./store";
...
function SomeoneIsTyping() {
  const someoneisTyping = useSelector<LiveblocksState, boolean >((state) =>
...
export default function App() {
  const draft = useSelector<LiveBlocksState,string>((state) => state.draft);
  const dispatch = useDispatch();
...
```ts highlight="10,23-30,34,42" file="src/store.ts"
import { createClient } from "@liveblocks/client";
...
```tsx highlight="5,13,38-41,45-57" file="src/App.tsx"
import React, { useEffect } from "react";
import { useDispatch, useSelector } from "react-redux";
import { actions } from "@liveblocks/redux";
...
export default function App() {
  const todos = useSelector<LiveblocksState, string[]>(state => state.todos);
  const draft = useSelector<LiveblocksState, string>((state) => state.draft);
  const dispatch = useDispatch();
