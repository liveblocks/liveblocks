...
If the shared package declares Liveblocks as a regular `"dependency"`, your
package manager will install the Liveblocks version specified by the shared
package, which may conflict with the version your application needs:

```mermaid
graph TD
    subgraph "Problem: dependencies conflict"
        A1[App 1] --> SL1[Shared Lib]
        A1 -->|"3.2.0"| LB1["@liveblocks"]
        SL1 -->|"3.2.0"| LB1

        A2[App 2] --> SL1b[Shared Lib]
        A2 -->|"3.13.3"| LB2["❌ @liveblocks<br/><small><i>conflicting versions</i></small>"]
        SL1b -->|"3.2.0"| LB2
    end

    style LB1 fill:#6f6,stroke:#090
    style LB2 fill:#fcc,stroke:#b33
```

The recommended approach is to declare Liveblocks as a `"peerDependency"` in
your shared package's `package.json`. This tells the package manager: "I'm
compatible with Liveblocks ^3, but the host application decides which version to
use." Then, only declare Liveblocks in the `"dependencies"` of your actual
applications.
...
```mermaid
graph TD
    subgraph "Solution: use peerDependencies"
        A1[App 1] --> SL1[Shared Lib]
        A1 -->|"3.2.0"| LB1["@liveblocks"]
        SL1 -.->|"peer: ^3"| LB1

        A2[App 2] --> SL1b[Shared Lib]
        A2 -->|"3.13.3"| LB2["@liveblocks"]
        SL1b -.->|"peer: ^3"| LB2
    end

    style LB1 fill:#6f6,stroke:#090
    style LB2 fill:#6f6,stroke:#090
```
