---
meta:
  title: "End-to-end testing a Liveblocks app with Playwright"
  parentTitle: "Tools"
  description:
    "Learn how to write end-to-end tests for your Liveblocks application using
    Playwright and the local dev server."
---

End-to-end tests give you confidence that your collaborative features work
...
application using the local dev server, so your tests run locally or in CI,
without hitting the Liveblocks cloud API.

We'll use the
[Next.js Todo List example](https://github.com/liveblocks/liveblocks/tree/main/examples/nextjs-todo-list)
as a reference, but a similar approach works with any Liveblocks project.

## Prerequisites

- A Liveblocks app that uses `@liveblocks/react` (or any other Liveblocks
  client package)
- Node.js 18+
- [Bun](https://bun.sh) (used by the Liveblocks dev server)
...
1. **Point your app at the local dev server** instead of the Liveblocks cloud,
   using the `baseUrl` prop and the magic `pk_localdev` public key.
...
- **`reuseExistingServer`** — Locally, this lets you leave the servers running
  ...
  time.
- **`CI` check** — In CI, the dev server is provided by a Docker service
  container (see [Running in CI](#running-in-ci)), so we skip starting it via
  `webServer`.
...
```ts file="test/todo.spec.ts"
test("should sync a new todo to a second client", async ({ page }, testInfo) => {
...
as a service container instead of starting it via `bunx`:
  ...
  e2e:
    ...
    services:
      liveblocks:
        ...
        env:
          LIVEBLOCKS_DEVSERVER_HOST: "0.0.0.0"
        ports:
          - 1153:1153
...
- **`LIVEBLOCKS_DEVSERVER_HOST: "0.0.0.0"`** — The dev server must bind to
  `0.0.0.0` inside the container so it's reachable via Docker port mapping.
...
- **Artifact upload** — On failure, the Playwright HTML report is uploaded so
  you can debug test failures.
...
- [Test file (`test/todo.spec.ts`)](https://github.com/liveblocks/liveblocks/blob/main/examples/nextjs-todo-list/test/todo.spec.ts)
  — 11 tests covering CRUD, toggle, multi-client sync, and typing indicators
- [Playwright config (`playwright.config.ts`)](https://github.com/liveblocks/liveblocks/blob/main/examples/nextjs-todo-list/playwright.config.ts)
  — local dev server + Next.js web server setup
- [CI workflow (`.github/workflows/e2e-todo-list.yml`)](https://github.com/liveblocks/liveblocks/blob/main/.github/workflows/e2e-todo-list.yml)
  — GitHub Actions pipeline using the dev server Docker image
...
- **`reuseExistingServer`** — During local development, keep the dev server
  running separately (`bunx liveblocks dev`) for faster test iteration.
