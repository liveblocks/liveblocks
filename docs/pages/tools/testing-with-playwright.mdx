...
application using the local dev server, so your tests run without hitting the
Liveblocks cloud API.

We'll use the
[Next.js Todo List example](https://github.com/liveblocks/liveblocks/tree/main/examples/nextjs-todo-list)
as a reference, but the same approach works with any Liveblocks project.
