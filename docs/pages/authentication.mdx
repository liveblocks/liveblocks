...
Your application likely supports multiple organizations. In this example, we’ll
set up ID token authentication for a multi-organization application. Each
organization has its own tenant, and each tenant defines its own set of user
roles aligned with its business requirements. We use the permissions provided by
Liveblocks to define and enforce access control for each role.
...
Liveblocks provides the following permissions to control access to the room:
...
We can map the roles to the permissions as follows:
...
For our example we will have two organizations, each with its own tenant and set
of roles.

| Organization Name | Tenant     | Available Roles                |
| ----------------- | ---------- | ------------------------------ |
...
Here is how we can set up rooms and tenants for each organization.

```ts
// Setting up a room for the Acme Inc. organization
// Room ID: acme-inc:document:123
...
// Setting up a room for the Lumon organization
// Room ID: lumon:document:123
const room = await liveblocks.createRoom(
...
Now each time a user accesses a tenant, we will identify them and set the
desired role for them.

```ts
// Identifying Mislav Abha and giving them admin access in the Acme Inc. organization
const { status, body } = await liveblocks.identifyUser({
...
// Identifying Mislav Abha and giving them commenter access in the Lumon organization
const { status, body } = await liveblocks.identifyUser({
  userId: "mislav.abha@example.com",
  tenantId: "lumon",
  groupIds: ["commenter"],
...
```ts
// Identifying Charlie Layne and giving them admin access in the Lumon organization
const { status, body } = await liveblocks.identifyUser({
...
// Identifying Charlie Layne and giving them viewer access in the Acme Inc. organization
const { status, body } = await liveblocks.identifyUser({
  userId: "charlie.layne@example.com",
  tenantId: "acme-inc",
  groupIds: ["viewer"],
...
To give enhanced permissions to a user in a room we can use `usersAccesses` and
set the user's `userId` as the key and the permissions as the value when
...
const acmeRoom = await liveblocks.updateRoom("acme-inc:document:456", {
  usersAccesses: {
    "charlie.layne@example.com": ["room:write"],
  },
});
```
...
<Figure>
  <Image
    src="/assets/authentication/concrete-example.png"
    alt="Concrete example"
    width={1494}
    height={840}
    quality={90}
  />
