...
meta:
  ...
  description:
    "Understand authentication, permissions, and start authenticating your users
    in your application with ID tokens."
---
...
2. Read access to everything, except write access to comments and no access to
   feeds:
...
create or modify a room, you can set permissions on the room itself, making the
room the source of truth. Later, when a user tries to enter a room, Liveblocks
checks if the user has permission and denies access if the permissions aren’t
set.
...
### Workspace permissions [#id-token-permissions]
