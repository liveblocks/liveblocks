...
<PropertiesList title="Arguments">
  ...
  <PropertiesListItem
    name="liveblocksOptions.collaborationMode"
    defaultValue='"yjs"'
    type='"yjs" | "liveblocks"'
  >
    The collaboration backend to use. `liveblocks` stores editor documents in
    `root._tiptap_docs`, keyed by `field`. [Learn
...
By default, BlockNote uses Yjs-backed collaboration. To store text as
...
In this mode, documents are stored in `root._tiptap_docs`, keyed by their
`field` values. This means you can list all known editors directly from storage.
...
`offlineSupport_experimental` and `ai` require the default Yjs mode, as do
