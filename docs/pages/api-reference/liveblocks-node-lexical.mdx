...
This package assumes the room’s document is Yjs-backed, which is the case when
using
[`@liveblocks/react-lexical`](/docs/api-reference/liveblocks-react-lexical). To
edit Storage-backed text from the server, use
