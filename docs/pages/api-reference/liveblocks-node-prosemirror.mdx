...
[Tiptap](/docs/api-reference/liveblocks-react-tiptap) documents as well. For a
client-side ProseMirror editor backed by `LiveText` in Storage, use
[`@liveblocks/prosemirror`](/docs/api-reference/liveblocks-prosemirror). For a
client-side Tiptap editor using the same Storage-backed collaboration, use
