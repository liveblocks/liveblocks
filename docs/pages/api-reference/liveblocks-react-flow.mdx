...
<PropertiesList title="Options">
  ...
  <PropertiesListItem name="storageKey" type="string" optional>
    The key used to store the flow in Storage. Defaults to `"flow"`. See
...
Some React Flow fields are intentionally **not** written to Storage so each
client keeps their own selection and interaction state:
...
Storage. Use the `storageKey` option to choose a different key or to support
multiple diagrams in a single room.
...
collection of nodes and edges stored in Storage. By default, your flow gets
persisted in the Room’s Storage under the top-level key `"flow"`, but you can
change this with the `storageKey` option.

<Banner type="info" title="No need to work with Liveblocks primitives">
  Even though internally `mutateFlow` uses Storage and its primitives like
  LiveObjects, LiveLists, and LiveMaps, you don’t need to work with those
...
<PropertiesList title="Options">
  ...
  <PropertiesListItem name="storageKey" type="string" optional>
    The key used to store the flow in Storage. Defaults to `"flow"`. Must match
    the [`storageKey`](#storageKey) used on the client.
  </PropertiesListItem>
  <PropertiesListItem name="nodes.sync" type="NodeSyncConfig" optional>
...
but rather than giving you access to the Storage root directly, it gives you a
convenient API tailored to React Flow—a `MutableFlow` instance you can use to
read or mutate your nodes and edges directly.
