...
## Updating your Flow from the server [#node-subexport]
...
<Banner type="info" title="No need to work with Liveblocks primitives">
  ...
  directly. You "just" work with React Flow nodes and edges like you know
  already, but internally these get intelligently stored and synchronized via
  Live structures in the most efficient manner.
</Banner>
...
<PropertiesList title="Options">
  ...
  <PropertiesListItem name="storageKey" type="string" optional>
    The key used to store the diagram in Liveblocks Storage. Defaults to
    `"flow"`. Must match the [`storageKey`](#storageKey) used on the client.
  </PropertiesListItem>
  <PropertiesListItem name="nodes.sync" type="NodeSyncConfig" optional>
...
but instead of giving you access to the Liveblocks Storage root directly,
instead it gives you a convenient API tailored to React Flow, the `MutableFlow`
instance you can use to read or mutate your nodes and edges directly.
