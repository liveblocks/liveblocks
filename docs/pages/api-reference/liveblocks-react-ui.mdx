...
Displays an interactive AI chat. AI can [use knowledge](#TODO),
[run actions](#TODO) and [display tools](#TODO).
...
Each chat is stored permanently, and is indentified by its unique `chatId`.
Chats are only visible to the [authenticated user](/docs/authentication) who
created the chat.
...
```tsx
<AiChat chatId="my-chat-id" copilotId="co_a7Gd3x..." />
```
...
In chats without messages, you can display placeholder content to guide the
user...

{/* TODO the API we figure out */}

{/* TODO show initial suggestions if it's supported */}

##### Adjusting the chat’s width [#AiChat-width]
...
##### List the user’s chats and switch between them [#AiChat-chat-listing]
...
##### Display the chat’s title [#AiChat-title]

{/* TODO mention metadata too, if supported */}

Each chat has a `title` property, generated by AI from the user’s first message.
The title of a new chat starts empty, and is automatically updated after AI
receives this message. You can render this alongside your chat with
[`useAiChat`](#TODO).
...
function ChatWithTitle({ chatId: { chatId: string } }) {
  // +++
  const { title, error, isLoading } = useAiChat();
  // +++
...
##### Use custom markdown components in the chat [#AiChat-markdown]

AI can use markdown to display text, lists, and more. You can replace the
default markdown components with your own custom components by...
...
Displays AI tool progress and results. Can be customized for many different UIs.
...
```tsx
// TODO show usage with RegisterAiTool
```
...
render: () => <AiTool title="Event booked" icon="📅" />,
...
Displays an AI tool with a confirmation dialog. This allows you to create
actions that users must confirm or cancel before they’re run.
...
when the users clicks the buttons. You must return information that helps the AI
understand what has taken place, in any format. This will never be shown in the
chat, but it will be stored in the AI’s memory. Returning a JS object often
helps the AI understand the result better.

```tsx
// TODO show usage with RegisterAiTool
```

`AiTool.Confirmation` dialog will display different content depending on the
status of the tool. For example, the confirm and cancel buttons will disappear
when clicked.

###### Props [#AiTool.Confirmation-props]

{/* TODO ai tool confiramtion props table */}

##### AiTool.Inspector

Displays code snippets of the JSON values returned and sent by AI in the current
tool call, helpful for debugging or for showing the internals of your app to
developers.
...
{/* TODO ai tool inspector props table */}

#### TODO RegisterAiKnowledgeSource (check name)

{/* TODO knowledge source */}

##### Props [#TODO-props]

{/* TODO knowledge source props table */}

#### TODO RegisterAiTool (check name)

{/* TODO tool */}

##### Props [#TODO-props]

{/* TODO tool props table */}

### Primitives [#AI-Copilots-Primitives]
