...
When using [`@liveblocks/react`][] it's best to use the `getYjsProviderForRoom`
factory function which will be automatically clean up the provider when the room
...
LiveblocksYjsProvider, or the second argument of the `getYjsProviderForRoom`
factory function:
...
Gets the current or creates a new LiveblocksYjsProvider for a room. This provider will
automatically be cleaned up when the room is destroyed, so you don't need to
destroy the provider manually. The second argument is the LiveblocksYjsProvider
options.
