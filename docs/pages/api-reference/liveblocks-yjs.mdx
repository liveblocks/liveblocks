...
`LiveblocksYjsProvider` is a [Yjs provider](https://github.com/yjs/yjs#providers)
...
When using [`@liveblocks/react`][] you can connect by using `LiveblocksYjsProvider`
...
Cleanup function. Destroys the [`LiveblocksYjsProvider`][] instance and removes all
resources.
