...
Each error has a `message`, a `roomId`, and a `context` object which has
different values for each error type.

```ts
import { useErrorListener } from "@liveblocks/react/suspense";

useErrorListener((error) => {
  // { message: "You don't have access to this room", roomId: "my-room-id", context: { ... }}
  console.error(error);
});
```
...
notifications are not part of rooms, and errors for these will show. To keep the
old behavior, use the [`useRoomErrorListener`][] hook instead.
...
### useRoomErrorListener [@badge=RoomProvider]

<Banner>

Before 2.16, this hook was named `useErrorListener`. However, that name is now
used for a global error listener, notifying you about errors from _any_ room.

</Banner>

Like [`useErrorListener`][], but scoped to only notify about errors from the
current room.

```tsx
import { useRoomErrorListener } from "@liveblocks/react/suspense";

useRoomErrorListener((error) => {
  // Here error.roomId will always match the current room's ID
});
```
...
[`useroomerrorlistener`]:
  /docs/api-reference/liveblocks-react#useRoomErrorListener
