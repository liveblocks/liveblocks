...
all changes to the selected data. A [Suspense version][] of this hook is also available.
...
subscribe to all changes to the selected data. A [Suspense version][] of this hook
is also available.
...
👉 A [Suspense version][] of this hook is also available, which will never return
a loading state and will throw error to the nearest ErrorBoundary if initial load
failed.
...
👉 A [Suspense version][] of this hook is also available, which will never return
a loading state and will throw when there’s an error.
...
{/* TODO move this whole section into the Notifications section after release */}

<Banner type="warning" title="Feature still in beta">
  User notification settings feature is currently in beta and has not been
  publicly released yet.
</Banner>

### useNotificationSettings [@badge=LiveblocksProvider]

Returns the user’s notification settings in the current project. User
notification settings are project-based, which means that `settings` and
`updateSettings` are for the current user’s settings in every room.
...
Use this hook to adjust which
[notification webhook events](/docs/platform/webhooks#NotificationEvent) are
sent for each user—each notification `kind` must first be enabled on your
project’s notification dashboard before settings can be used. You can also pass
custom notifications, if they’ve been added to your dashboard.
...
#### Updating settings [#useNotificationSettings-updating-settings]

The `updateSettings` function can be used to update the current user’s
notification settings, changing their settings for every room in the project. A
partial can be passed.
...
👉🏻 A [Suspense version][] of this hook is also available, which will never return
a loading state and will throw and error to the nearest `ErrorBoundary` if initial
load failed.

### useUpdatesNotificationSettings [@badge=LiveblocksProvider]

Returns a function that updates user's notification settings in the current
project. Use this if you don't need the current user's notification settings in
your component, but you need to update them (e.g a unsubscribe button).
