...
Return a function that marks an inbox notification as read.
...
Return a function that marks all inbox notification as read.
...
#### Notification settings [@badge=Beta]

By default, users receive notifications when:

- they are mentioned on a thread
- there is a reply to a thread they have been participating in

It's possible to override this behavior for a given user and room using
`useUpdateRoomNotificationSettings`.

#### useRoomNotificationSettings [@badge=Beta]

Returns the room notification settings for the current user.

```tsx
import { useRoomNotificationSettings } from "../liveblocks.config.ts";

const { isLoading, error, settings } = useRoomNotificationSettings();
settings.threads; // "all" or "repliesAndMentions" or "mute"
```
...
updateRoomNotificationSettings({
  threads: "all" | "repliesAndMentions" | "mute",
});
```

- `"all"`: notify current user when a new comment posted in a room
