...
**The behavior of this hook changed in 2.16.** Before, this hook required a
RoomProvider, and would only notify about connection errors for that room. Since
2.16, this hook no longer requires a RoomProvider, and will notify about any
Liveblocks errors, and for any room. To keep the old behavior, use the
[`useRoomErrorListener`][] hook instead.

</Banner>

Listen to potential Liveblocks errors. Examples of errors include room
connection errors, errors creating or deleting threads, editing thread metadata,
etc.

```ts file="simple-example.ts" isCollapsable
...
```ts file="full-example.ts" isCollapsable isCollapsed
import { useErrorListener } from "@liveblocks/react/suspense";
...
useErrorListener((error) => {
  switch (error.context.type) {
    ...
    case "ROOM_CONNECTION_ERROR": {
      ...
      // Any other error code is from the future
      break;
    }
