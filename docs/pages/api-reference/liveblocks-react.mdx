...
<PropertiesList>
  <PropertiesListItem name="client" type="Client">
    The Liveblocks client instance from the nearest LiveblocksProvider.
  </PropertiesListItem>
</PropertiesList>
...
[upgrade guide for 2.16](/docs/platform/upgrading/2.16) for more details.
...
<PropertiesList>
  <PropertiesListItem name="options" type="UseAiChatsOptions">
    Optional configuration object for filtering and querying AI chats.
  </PropertiesListItem>
</PropertiesList>
...
<PropertiesList>
  <PropertiesListItem name="query" type="AiChatsQuery">
    Optional query to filter chats by metadata values or absence of metadata keys.
  </PropertiesListItem>
</PropertiesList>
...
<PropertiesList>
  ...
  <PropertiesListItem name="error" type="Error | null">
    Any error that occurred while loading the chats.
  </PropertiesListItem>
  <PropertiesListItem name="hasFetchedAll" type="boolean">
    Whether all available chats have been fetched.
  </PropertiesListItem>
  <PropertiesListItem name="fetchMore" type="() => void">
    A function to fetch more chats.
  </PropertiesListItem>
  <PropertiesListItem name="isFetchingMore" type="boolean">
    Whether more chats are currently being fetched.
  </PropertiesListItem>
  <PropertiesListItem name="fetchMoreError" type="Error | null">
    Any error that occurred while fetching more chats.
  </PropertiesListItem>
</PropertiesList>
...
You can optionally set a `title`, which prevents the AI auto-generating a title
from the first messages. Additionally, you can choose to set custom `metadata`
...
<PropertiesList>
  <PropertiesListItem name="createAiChat" type="(chatIdOrOptions: string | CreateAiChatOptions) => AiChat">
...
Returns a function that sends a message to an AI chat, indentified by its ID.
Useful for
...
You can set options related to the message being sent, such as the copilot ID to
use. Alternatively, you can also override or complete the hook’s options when
calling the function by passing an object to it.

```tsx
import { useSendAiMessage } from "@liveblocks/react";

const sendAiMessage = useSendAiMessage("my-chat-id", { copilotId: "co_xxx" });

sendAiMessage("Hello!");

// Use another copilot ID dynamically
sendAiMessage({ text: "Hello!", copilotId: "co_yyy" });

// Use another chat ID dynamically
sendAiMessage({ text: "Hello!", chatId: "my-other-chat-id" });
```

You can also pass the chat ID in the function rather than the hook. This is
particularly helpful in dynamic scenarios where the chat ID might not be known
...
// Because `useSendAiMessage` didn't receive a chat ID, `sendAiMessage`
// needs to receive one or it will throw an error.
sendAiMessage({ text: "Hello!", chatId: "my-chat-id" });

// The following calls would result in both TypeScript and runtime errors.
sendAiMessage("Hello!");
sendAiMessage({ text: "Hello!" });
sendAiMessage({ text: "Hello!", copilotId: "co_xxx" });
```

If needed (e.g. using the message’s ID), you can also access the newly created
message.
...
const sendAiMessage = useSendAiMessage("my-chat-id");
const message = sendAiMessage("Hello!");
```
...
<PropertiesList>
  ...
  <PropertiesListItem name="options" type="UseSendAiMessageOptions">
    Optional configuration object for the message sending behavior.
  </PropertiesListItem>
</PropertiesList>
...
<PropertiesList>
  <PropertiesListItem name="sendAiMessage" type="(messageOrOptions: string | SendAiMessageOptions) => AiChatMessage">
...
a given location, then renders a compoennt in the chat.
...
<PropertiesList>
  <PropertiesListItem name="options" type="UseRoomOptions">
...
<PropertiesList>
  <PropertiesListItem name="allowOutsideRoom" type="boolean" defaultValue="false">
...
<PropertiesList>
  <PropertiesListItem name="room" type="Room">
    The Room instance from the nearest RoomProvider. Returns `null` if `allowOutsideRoom` is `true` and the hook is used outside of a RoomProvider.
  </PropertiesListItem>
</PropertiesList>
...
<PropertiesList>
  <PropertiesListItem name="status" type="'initial' | 'connecting' | 'connected' | 'reconnecting' | 'disconnected'">
...
<PropertiesList>
  <PropertiesListItem name="options" type="UseSyncStatusOptions">
    Optional configuration object.
  </PropertiesListItem>
</PropertiesList>
...
<PropertiesList>
  <PropertiesListItem name="smooth" type="boolean" defaultValue="false">
...
<PropertiesList>
  <PropertiesListItem name="callback" type="(event: OthersEvent) => void">
    A callback function that is called when an "others" event occurs. The event object contains `type`, `user`, and `others` properties.
  </PropertiesListItem>
</PropertiesList>
...
<PropertiesList>
  <PropertiesListItem name="callback" type="(event: 'lost' | 'restored' | 'failed') => void">
...
<PropertiesList>
  <PropertiesListItem name="myPresence" type="Presence">
    The current user's presence data.
  </PropertiesListItem>
  <PropertiesListItem name="updateMyPresence" type="(patch: Partial<Presence>, options?: { addToHistory?: boolean }) => void">
...
<PropertiesList>
  <PropertiesListItem name="others" type="readonly User[] | T | null">
...
<PropertiesList>
  <PropertiesListItem name="others" type="readonly [connectionId: number, data: T][] | null">
...
<PropertiesList>
  <PropertiesListItem name="callback" type="(event: { event: TBroadcastEvent; user: User | null; connectionId: number }) => void">
    A callback function that is called when a custom event is received. The callback receives an object with the event data, user information, and connection ID.
  </PropertiesListItem>
</PropertiesList>
...
<PropertiesList>
  <PropertiesListItem name="mutationFn" type="(context: MutationContext, ...args: TArgs) => void">
...
characters using `startsWith`. Returned threads match the entire query.
...
<PropertiesList>
  <PropertiesListItem name="options" type="UseThreadsOptions">
    Optional configuration object.
  </PropertiesListItem>
</PropertiesList>
...
<PropertiesList>
  <PropertiesListItem name="query" type="ThreadsQuery">
    Optional query to filter threads by resolved status and metadata values.
  </PropertiesListItem>
  <PropertiesListItem name="scrollOnLoad" type="boolean">
    Whether to scroll to a comment if the URL's hash is set to a comment ID. Defaults to `true`.
  </PropertiesListItem>
</PropertiesList>
...
<PropertiesList>
  ...
  <PropertiesListItem name="hasFetchedAll" type="boolean">
    Whether all available threads have been fetched.
  </PropertiesListItem>
  <PropertiesListItem name="fetchMore" type="() => void">
    A function to fetch more threads.
  </PropertiesListItem>
  <PropertiesListItem name="isFetchingMore" type="boolean">
    Whether more threads are currently being fetched.
  </PropertiesListItem>
  <PropertiesListItem name="fetchMoreError" type="Error | null">
    Any error that occurred while fetching more threads.
  </PropertiesListItem>
</PropertiesList>
...
<PropertiesList>
  <PropertiesListItem name="createThread" type="(options: CreateThreadOptions) => ThreadData">
...
<PropertiesList>
  <PropertiesListItem name="editThreadMetadata" type="(options: { threadId: string; metadata: ThreadMetadata }) => void">
...
<PropertiesList>
  <PropertiesListItem name="markThreadAsResolved" type="(threadId: string) => void">
...
<PropertiesList>
  <PropertiesListItem name="markThreadAsUnresolved" type="(threadId: string) => void">
...
<PropertiesList>
  <PropertiesListItem name="subscribeToThread" type="(threadId: string) => void">
...
<PropertiesList>
  <PropertiesListItem name="unsubscribeFromThread" type="(threadId: string) => void">
...
<PropertiesList>
  <PropertiesListItem name="createComment" type="(options: CreateCommentOptions) => CommentData">
...
<PropertiesList>
  <PropertiesListItem name="editComment" type="(options: EditCommentOptions) => void">
...
<PropertiesList>
  <PropertiesListItem name="deleteComment" type="(options: { threadId: string; commentId: string }) => void">
...
<PropertiesList>
  <PropertiesListItem name="addReaction" type="(options: { threadId: string; commentId: string; emoji: string }) => void">
...
<PropertiesList>
  <PropertiesListItem name="removeReaction" type="(options: { threadId: string; commentId: string; emoji: string }) => void">
...
<PropertiesList>
  <PropertiesListItem name="options" type="UseInboxNotificationsOptions">
    Optional configuration object.
  </PropertiesListItem>
</PropertiesList>
...
<PropertiesList>
  <PropertiesListItem name="query" type="InboxNotificationsQuery">
    Optional query to filter notifications by room ID or kind.
  </PropertiesListItem>
</PropertiesList>
...
<PropertiesList>
  ...
  <PropertiesListItem name="hasFetchedAll" type="boolean">
    Whether all available notifications have been fetched.
  </PropertiesListItem>
  <PropertiesListItem name="fetchMore" type="() => void">
    A function to fetch more notifications.
  </PropertiesListItem>
  <PropertiesListItem name="isFetchingMore" type="boolean">
    Whether more notifications are currently being fetched.
  </PropertiesListItem>
  <PropertiesListItem name="fetchMoreError" type="Error | null">
    Any error that occurred while fetching more notifications.
  </PropertiesListItem>
</PropertiesList>
...
<PropertiesList>
  <PropertiesListItem name="options" type="UseUnreadInboxNotificationsCountOptions">
    Optional configuration object.
  </PropertiesListItem>
</PropertiesList>
...
<PropertiesList>
  ...
  <PropertiesListItem name="error" type="Error | null">
    Any error that occurred while loading the count.
  </PropertiesListItem>
</PropertiesList>
...
<PropertiesList>
  <PropertiesListItem name="markInboxNotificationAsRead" type="(inboxNotificationId: string) => void">
...
<PropertiesList>
  <PropertiesListItem name="deleteInboxNotification" type="(inboxNotificationId: string) => void">
...
<PropertiesList>
  ...
  <PropertiesListItem name="updateSettings" type="(settings: Partial<RoomSubscriptionSettings>) => void">
...
<PropertiesList>
  <PropertiesListItem name="updateRoomSubscriptionSettings" type="(settings: Partial<RoomSubscriptionSettings>) => void">
...
<PropertiesList>
  ...
  <PropertiesListItem name="updateSettings" type="(settings: Partial<NotificationSettings>) => void">
...
<PropertiesList>
  <PropertiesListItem name="updateNotificationSettings" type="(settings: Partial<NotificationSettings>) => void">
...
<PropertiesList>
  ...
  <PropertiesListItem name="isLoading" type="boolean">
    Whether the versions are currently being loaded.
  </PropertiesListItem>
  <PropertiesListItem name="error" type="Error | null">
    Any error that occurred while loading the versions.
  </PropertiesListItem>
</PropertiesList>
...
[`resolveUsers`][] option in [`createClient`][] or [`LiveblocksProvider`][].
...
[`resolveRoomsInfo`][] option in [`createClient`][] or [`LiveblocksProvider`][].
...
To use `useGroupInfo`, you should provide a resolver function to the
[`resolveGroupsInfo`][] option in [`createClient`][] or
[`LiveblocksProvider`][].
