...
locations, `@liveblocks/react` and `@liveblocks/react/suspense`. This is because
...
default, as it often makes it easier to build your application.

### Suspense hooks

Suspense hooks can be wrapped in `ClientSideSuspense`, which acts as a loading
spinner for any components below it. When using this, all components below will
only render once their hook contents have been loaded.
...
`<LiveblocksProvider>` is required in all applications as of Liveblocks 2.0.

</Banner>

Sets up a client for connecting to Liveblocks, and is required for all React
apps. You must define either `authEndpoint` or `publicApiKey`. Resolver
functions should be placed inside here, and a number of other options are
available. Unlike [`RoomProvider`][], `LiveblocksProvider` doesn’t call
...
advanced usage only. We generally recommend using ... TODO ... unless you need
to define multiple room types in your application.

</Banner>

Creates a [`LiveblocksProvider`][] and a set of typed hooks. We recommend using
it in `liveblocks.config.ts` and re-exporting your typed hooks like below.
...
is unmounted. When using [Realtime APIs](/docs/products/realtime-apis), initial
Presence values for each user, and Storage values for the room can be set.
...
Creates a [`RoomProvider`][] and a set of typed hooks to use in your app. We
generally recommend typing your app using the newer method instead. When using
...
To use the React suspense version of our hooks, which we recommend, you can
...
`liveblocks.config.ts` file, so it’s easy to keep track of your types. Each type
(`Presence`, `Storage`, etc.), is optional, but it’s recommended to make use of
them.
...
Before Liveblocks 2.0, it was recommend to create your hooks using
[`createRoomContext`][], and manually pass your types to this function. This is
no longer the recommended method for setting up Liveblocks, but it can still be
helpful, for example you can use `createRoomContext` multiple times to create
different room types, each with their own correctly typed hooks.
...
[`cl`]: /docs/api-reference/liveblocks-react#RoomProvider
