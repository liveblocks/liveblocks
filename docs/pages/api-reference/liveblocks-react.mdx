...
## Channels Notification Settings [#channels-notification-settings]

<Banner title="Private beta">

Channels notification settings is currently in private beta. If you would like
access to the beta, please [contact us](https://liveblocks.io/contact/sales).
We’d love to hear from you.

</Banner>

### useChannelsNotificationSettings [@badge=LiveblocksProvider]

Channels notification settings are project based. It means the settings are the
same for every room.

Returns the user’s notification settings for the channels in the current
project.
...
The `updateSettings` function can be used to update the notification settings.

```tsx
const onUpdate = () => {
  updateSettings({
    email: { thread: true, textMention: false },
  });
};
```

Error handling is an important aspect to consider when using the
`useChannelsNotificationSettings` hook. The `error` fields provides information
about any error that occurred during the fetch operation.
...
if (error) {
  return (
    <div>
      <p>Error loading channels notification settings: {error.message}</p>
    </div>
  );
}
```
...
### useUpdateChannelsNotificationSettings [@badge=LiveblocksProvider]

Returns a function that updates user's channels notification settings in the
current project.

Use this if you don't need the current user's channels notification settings in
your component, but you need to update them (e.g a unsubscribe button).

```tsx
import { useUpdateChannelsNotificationSettings } from "@liveblocks/react";
