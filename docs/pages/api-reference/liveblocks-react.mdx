...
<PropertiesList title="Props">
  ...
  >
    ...
    when necessary, so it's not recommended to cache it yourself. Either
...
Is it not recommended to cache your authentication endpoint. The Liveblocks
client will cache results for you, only making requests to the endpoint if
necessary, such as when the the token has expired.
