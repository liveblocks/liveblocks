...
Returns information about an AI chat, for example it’s title and metadata.

```tsx
import { useAiChat } from "@liveblocks/react/suspense";

const { title, metadata, error, isLoading } = useAiChat("my-chat-id");
```

### useDeleteAiChat

Returns a function that deletes an AI chat by its ID.
...
[Sync Datastore](/docs/platform/sync-datastore), initial Presence
