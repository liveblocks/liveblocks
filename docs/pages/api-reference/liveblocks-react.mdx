...
Returns information about an AI chat, for example it’s title. Titles are
automatically generated from the content of the first user message in a chat,
...
either a string, object or array. These components can be placed anywhere in
your app, so long as they’re under
...
<PropertiesList>
  ...
  <PropertiesListItem name="value" type="string" isRequired>
    The knowledge content as a string. Can be plain text, JSON, or any string
    format that provides useful context to the AI.
  </PropertiesListItem>
    ...
    <>
      <RegisterAiTool
        ...
        tool={defineAiTool()({
          ...
          render: ({ result }) => <AiTool title="Toast sent" icon="🍞" />,
        })}
      />
      <AiChat chatId="my-chat-id" />
      <Toaster />
...
Starting with 2.12, we recommend using [`useSyncStatus`][] instead for tracking
sync status, because it will reflect sync status of all parts of Liveblocks, not
just Storage.
...
This hook was renamed to [`useRoomSubscriptionSettings`][] in 2.24, read more in
...
This hook was renamed to [`useUpdateRoomSubscriptionSettings`][] in 2.24, read
more in our [migration guide](/docs/platform/upgrading/2.24).
