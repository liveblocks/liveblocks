...
`resolveUsers` option in [`createRoomContext`](#createRoomContext).

This resolver function will receive a list of user IDs and should return a list
of user objects of the same size and in the same order.
...
Makes a [`Room`] available in the component hierarchy below. Joins the room when
...
- `initialStorage` (optional) - The initial Storage structure to create when a
  ...
  JSON.
- `autoConnect` (optional) - Whether or not the room should automatically
  connect to Liveblocks servers when the RoomProvider is mounted. By default
  it’s set to `typeof window !== "undefined"`, meaning the RoomProvider attempts
  to connect to Liveblocks servers only on the client side.

The `initialPresence`, `initialStorage` and `autoConnect` props are ignored
...
`useThreads` can also take an optional `query` parameter, to filter threads
...
[`resolveUsers`](#resolveUsers) option in
[`createRoomContext`](#createRoomContext).
...
Comments mutation hooks are used to modify comments and threads. Examples
include creating threads, deleting comments, and adding reactions.
...
```tsx
import { useCreateComment } from "../liveblocks.config.ts";
...
```tsx
import { useEditComment } from "../liveblocks.config.ts";
...
```tsx
import { useDeleteComment } from "../liveblocks.config.ts";
...
```tsx
import { useRemoveReaction } from "../liveblocks.config.ts";

const removeReaction = useRemoveReaction();
removeReaction({ threadId: "th_xxx", commentId: "cm_xxx", emoji: "👍" });
```

#### useMarkThreadsAsRead [@badge=Beta]

Returns a function that marks threads as read.

```tsx
import { useMarkThreadsAsRead } from "../liveblocks.config.ts";

const markThreadsAsRead = useMarkThreadsAsRead();
markThreadsAsRead({ threadIds: ["th_xxx_1", "th_xxx_2"] });
```

### Notification hooks

Comments notification hooks are all related to handling inbox notifications.

#### useInboxNotifications [@badge=Beta]

Returns the 20 more recent inbox notifications for the current user across all
rooms.

```tsx
import { useInboxNotifications } from "../liveblocks.config";

const { inboxNotifications, error, isLoading, loadMore } =
  useInboxNotifications();
```

`loadMore` will fetch the next 20 inbox notifications and append them to the
array of `inboxNotifications`

👉 A [Suspense version][] of this hook is also available, which will never
return `null`.

#### useUnreadInboxNotificationsCount [@badge=Beta]

Returns the count of unread inbox notifications. Useful when building a 🔔
component so users can see how many unread inbox notifications they have.

```tsx
import { useUnreadInboxNotificationsCount } from "../liveblocks.config.ts";

const count = useUnreadInboxNotificationsCount();
```

#### useReadInboxNotification [@badge=Beta]

Returns a function that marks an inbox notification as read.

```tsx
import { useReadInboxNotification } from "../liveblocks.config.ts";

const readInboxNotification = useReadInboxNotification();
readInboxNotification("in_xxx");
```

#### useReadAllInboxNotifications [@badge=Beta]

Returns a function that marks all inbox notification as read.

```tsx
import { useReadAllInboxNotifications } from "../liveblocks.config.ts";

const readAllInboxNotifications = useReadAllInboxNotifications();
readAllInboxNotifications();
```

#### useRoomNotificationSettings [@badge=Beta]

Returns the room notification settings for the current user. It’s possible to
override notification behavior for a given user and room with
[`useUpdateRoomNotificationSettings`](#useUpdateRoomNotificationSettings).

```tsx
import { useRoomNotificationSettings } from "../liveblocks.config.ts";

const { isLoading, error, settings } = useRoomNotificationSettings();

// "all" | "repliesAndMentions" | "mute"
console.log(settings.threads);
```

#### useUpdateRoomNotificationSettings [@badge=Beta]

Returns a function that updates the user notification settings for the current
room. By default, users receive notifications when, they’re mentioned in a
thread, or if there’s a reply to a thread they have been participating in, but
this can be customized.

```tsx
import { useUpdateRoomNotificationSettings } from "../liveblocks.config.ts";

const updateRoomNotificationSettings = useUpdateRoomNotificationSettings();

// Default. Notify if mentioned or participating in a thread
updateRoomNotificationSettings({
  threads: "repliesAndMentions",
});
```

There are three options available.

- `"all"`: Notify the current user when a new comment is posted in a room.
- `"repliesAndMentions"`: Default behavior.
- `"mute"`: Do not notify current user, even if they are mentioned or if there
  is a reply on a thread they participated on.

```tsx
// Default. Notify if mentioned or participating in a thread
updateRoomNotificationSettings({
  threads: "repliesAndMention",
});

// Notify the current for every comment in the room
updateRoomNotificationSettings({
  threads: "all",
});

// Never notify the current user, even if mentioned or participating in a thread
updateRoomNotificationSettings({
  threads: "mute",
});
```
...
### LiveblocksProvider [@badge=Deprecated]
