...
Storage provides four different conflict-free data structures, which you can
...
- [`LiveText`][] - A collaborative rich-text document. Use this to store plain
  text with optional inline formatting attributes (for example `{ bold: true }`).
  Concurrent insertions, deletions, and formatting changes are merged
  automatically using server-ordered operational transformation.
...
To get the plain text content without attributes, join the segment texts. This is
equivalent to calling [`.toString()`][] on a `LiveText` instance—for example
...
text. Learn more about [listening for nested changes](/docs/api-reference/liveblocks-client#listening-for-nested-changes).
