...
<PropertiesList title="Props">
  ...
  >
    ...
    Liveblocks is still synchronizing local changes. Available since 2.12.0.
  </PropertiesListItem>
  <PropertiesListItem
...
Liveblocks usually synchronizes milliseconds after a local change, but in some
circumstances, such as a user with a poor (or entirely offline) connection, it
may take longer before they receive a response from our servers. It can be
helpful to check if local changes have been synchronized, and prevent users from
leaving the page if they haven’t.

It’s possible to opt-in to this protection by setting the
`preventUnsavedChanges` prop on the [`LiveblocksProvider`][].
