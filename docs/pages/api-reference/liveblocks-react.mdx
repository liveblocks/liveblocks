...
### useErrorListener [@badge=Both]

<Banner>

The behavior of this hook changed in 2.16. Before, this hook needed to be nested
under a RoomProvider, and would only notify you about connection errors for that
room. Since 2.16, this hook will notify you about any Liveblocks error (not just
connection errors), and for any room.
...
useErrorListener((error) => {
  ...
  switch (error.context.type) {
    ...
    case "TODO _ SPLIT THIS _ OUT _ FURTHER ": {
      // These can happen if you use Comments or Notifications
      // The following fields will be available as context to the error when applicable
      const { roomId, threadId, commentId, inboxNotificationId } = error;
      toast.error(error);
      break;
    }
