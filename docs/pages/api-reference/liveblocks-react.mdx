...
Makes Liveblocks features outside of rooms (e.g. Notifications) available in the
component hierarchy below.

Unlike [`RoomProvider`][], `LiveblocksProvider` doesn’t call Liveblocks servers
when mounted, so it can (and should) live higher in the component tree of your
app.
...
<Banner title="Different API in 0.18 and earlier versions">

In 0.18 and earlier versions, a different `LiveblocksProvider` API was
available. It was exported from `@liveblocks/react` directly instead of
`createLiveblocksContext`, and accepted the
[`Client`](/docs/api-reference/liveblocks-client#Client) via the `client` prop.
This previous API has been deprecated in 0.17 and removed in 0.18.
