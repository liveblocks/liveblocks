...
Returns a paginated list of feeds within the current room. Results are merged
into a per-room cache; `since` and `metadata` filter that cache **for this hook
call**, and `feeds` is sorted by `createdAt` ascending (oldest first), matching
the chronological order used by [`useThreads`](#useThreads) for room threads.
`limit` only sets the **page size** for each fetch (including `fetchMore`), not
a maximum length of the returned array.
[Suspense](/docs/api-reference/liveblocks-react#Suspense-hooks) and
[regular](/docs/api-reference/liveblocks-react#Regular-hooks) versions of this
hook are available.
...
<PropertiesList title="Arguments">
  ...
  <PropertiesListItem name="options.metadata" type="Record&lt;string, string&gt;">
...
<PropertiesList title="Returns">
  <PropertiesListItem name="feeds" type="Feed[] | undefined">
    An array of feeds within the current room matching `since` and `metadata` for
    this call, sorted by `createdAt` ascending (tie-break on `feedId`), or
...
Returns a paginated list of messages for a specific feed within the current
room. Messages are sorted by `createdAt` ascending (chronological). Initially
fetches messages based on the provided options.
[Suspense](/docs/api-reference/liveblocks-react#Suspense-hooks) and
[regular](/docs/api-reference/liveblocks-react#Regular-hooks) versions of this
hook are available.
...
updateFeedMetadata("my-feed-id", { name: "Updated Name", updated: new Date().toISOString() });
```
...
createFeedMessage("my-feed-id", { role: "user", content: "Hello!" }, {
  id: "my-message-id",
  timestamp: Date.now(),
});
```
...
You can type feed metadata and feed message data using the `Liveblocks` interface
in your `liveblocks.config.ts` file. This provides type safety for
