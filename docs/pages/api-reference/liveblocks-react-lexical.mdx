...
A more render-efficient way to implement `ThreadOverlay` would be with a
`useIsMobile` hook, allowing us to conditionally render the components.

```tsx title="useIsMobile" isCollapsed isCollapsable
...
[TODO one line description of features @Nimesh]
...
[TODO any other info we need @Nimesh]
