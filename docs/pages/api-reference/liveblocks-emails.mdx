...
- A thread has an _unread mention_ if it has unread replies, and one reply
  mentions the user. The comment with latest mention is returned.

The `type` property that lets you know which this notification is for.
...
// Your Liveblocks client and webhook event setup (see "Setup" section)
// ...
