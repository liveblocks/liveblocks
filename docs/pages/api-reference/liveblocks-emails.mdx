...
The text mention email functions decode Yjs-backed Tiptap and Lexical
documents. They are not wired to Storage-backed editors using
...
Takes a
[thread notification webhook event](/docs/api-reference/webhook-events#Thread-notification)
...
mention_ in a [Text Editor](/docs/use-cases/text-editor) document. In this case,
...
[render multiple editors per page](/docs/api-reference/liveblocks-react-tiptap#Multiple-editors),
...
Takes a
[text mention notification webhook event](/docs/api-reference/webhook-events#TextMention-notification)
...
<PropertiesList title="Arguments">
  ...
  >
    ...
    [Comments](/docs/products/comments). Return an array of `UserMeta["info"]`
    objects in the same order they arrived. Works similarly to the [resolver on
    the client](/docs/use-cases/text-editor#Users-and-mentions). [Learn
