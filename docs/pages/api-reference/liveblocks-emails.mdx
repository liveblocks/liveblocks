...
<Banner type="warning" title="Multiple text editors with Tiptap">
  Tiptap optionally allows you to [render multiple editors per
  page](/docs/products/text-editor/tiptap#Multiple-editors), instead of just
  one. For now, these functions only support one editor per room, but we’ll be
  looking to add support for more later.
</Banner>
