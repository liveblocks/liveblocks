...
<PropertiesList title="Arguments">
  ...
  <PropertiesListItem name="options.styles" type="object">
    [Learn more](#prepare-thread-notification-email-as-html-styling-components).
  </PropertiesListItem>
...
`paragraph`, `stront`, `code`, `link`, `mention`.
...
These functions help your create emails to notify users when they are mentioned
in a [Text Editor](/docs/products/text-editor) document and when they did not
have seen it. They fetch a text mention and its surrounding text (to give more
context) only if the notification has not already been read, and help you style
the mention content with either
[React](#prepare-text-mention-notification-as-react) or
[HTML](#prepare-text-mention-notification-as-html).
...
ndicate that there is no email to send in this case.

<Banner type="warning" title="Limited multiple text editors support">
  For now the functions documented below do not support multiple text editors.
  It means they will work only for one text editor in a room. Multiple editors
  in one room will be supported later.
</Banner>

### prepareTextMentionNotificationEmailAsReact [#prepare-text-mention-notification-as-react]
...
can also returns `null` if the text mention do not exists anymore or has been
...
<PropertiesList title="Returns">
  ...
  >
    ...
    use in emails. Returns `null` if the text mention do not exists anymore or
...
<PropertiesList title="Arguments">
  ...
  >
    ...
    [Comments](/docs/products/comments). Return an array of `UserMeta["info"]`
    objects in the same order they arrived. Works similarly to the [resolver on
    the client](/docs/products/text-editor/lexical#Users-and-mentions). [Learn
...
Takes a
[text mention notification webhook event](/docs/platform/webhooks#TextMention-notification)
and returns an unread text mention with its surrounding text as a HTML string.
It can also returns `null` if the text mention do not exists anymore or has been
already been read. You can also
...
<PropertiesList title="Returns">
  ...
  >
    Returns text mention information, and a formatted HTML content ready for
    use in emails. Returns `null` if the text mention do not exists anymore or
    has been already been read.
...
<PropertiesList title="Arguments">
  ...
  <PropertiesListItem name="options.styles" type="object">
    ...
    content. Four elements can be styled: `container`, `strong`, `code`, and,
    `mention`. [Learn
    more](#prepare-text-mention-notification-email-as-react-styling-elements).
  </PropertiesListItem>
...
Each element in the comment body can be styled with custom CSS properties, if
you would like to change the appearance. Five elements are available:
`container`, `strong`, `code`, and, `mention`.
