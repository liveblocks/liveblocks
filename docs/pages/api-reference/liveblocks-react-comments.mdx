...
Displays an inbox notification.

```tsx
<InboxNotification inboxNotification={inboxNotification} />
```

Map through `inboxNotifications` to render a list of the user's inbox
notifications. InboxNotification can be retrieved with
[`useInboxNotifications`](/docs/api-reference/liveblocks-react#useInboxNotifications).

```tsx highlight="10"
import { InboxNotification } from "@liveblocks/react-comments";
...
#### Props [#inbox-notification-props]
