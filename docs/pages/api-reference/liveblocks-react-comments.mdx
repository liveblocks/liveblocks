  ...
  <Composer.Form>
    <Composer.Editor
      components={{
        ...
        Portal: forwardRef(() => <Composer.Portal />),
        Link: () => <Composer.Link />,
      }}
    />
    <Composer.Submit />
...
| `Portal`             | <code>ComponentType&lt;ComposerEditorPortalProps&gt;</code>  | The component used to display portaled elements. It must apply its ref using `forwardRef`.<br/>Defaults to `Composer.Portal`. |
...
###### Portal [#primitives-Composer.Editor-Portal]

The component used to display portaled elements.

<Banner title="Applying its ref" type="warning">

In order to work, the provided component must apply its ref using
...
| Prop       | Type            | Description            |
| ---------- | --------------- | ---------------------- |
| `style`    | `CSSProperties` | The portal’s style.    |
| `children` | `ReactNode`     | The portaled elements. |
...
#### Composer.Portal [#primitives-Composer.Portal]

Displays portaled elements within `Composer.Editor`.

```tsx
<Composer.Portal {...props}>{children}</Composer.Portal>
```
