...
Liveblocks client that was previously connected lost its connection (e.g. due to
a network outage) and was unable to recover quickly.

This event allows you to build high-quality UIs by warning your users that under
the hood the app is still trying to re-establish the connection, but that they
may want to take extra care in the mean time to ensure their changes won't go
unsaved. Or to help them understand why they're not seeing updates made by
others yet.

When this happens, this callback is called with the event `lost`. Then, if/once
the connection restores, the callback will be called with the value `restored`.
If the connection could definitively not be restored, it will be called with
`failed` (uncommon).
...
`authenticating` → `connecting` (also)  
`open` → `connected`  
`failed` → `disconnected`  
`closed` → `initial`  
