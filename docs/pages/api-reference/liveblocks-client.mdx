...
If you are connected to a room and then that connection is somehow lost,
Liveblocks will normally reconnect automatically and quickly. If for some reason
this takes longer than usual (for example, your network is offline), then the
room will emit an event that you can subscribe to to get informed about such
...
Takes a callback that is called whenever the connection status changes. The
possible value are: `initial`, `connecting`, `connected`, `reconnecting`, or
`disconnected`.
...
using quick `connected` → `reconnecting` → `connected` status jumps. If you want
to let users know that there may be connectivity issues, don't use this API, but
instead refer to [`Room.subscribe("lost-connection")`][].

<Banner title="Notice">

Do not use this API to detect when Storage or Presence are initialized or
loaded. "Connected" does not mean that storage or presence is loaded. To detect
when Storage is loaded, rely on awaiting the [`Room.getStorage`][] promise or
using the [`Room.subscribe("storage-status")`][] event.

</Banner>

### Room.subscribe("lost-connection") [#Room.subscribe.lost-connection]

XXX document this. XXX mention that the `failed` case is very uncommon.

```ts
const unsubscribe = room.subscribe("lost-connection", (event) => {
  switch (event) {
    case "lost":
      // Display a "Still trying to reconnect..." toast
      break;

    case "restored":
      // Display a "Successfully reconnected again!" toast
      break;

    case "failed":
      // Display a "Could not restore the connection" toast
      break;
  }
});
```
...
You can roughly map these legacy statuses to the new/simpler statuses as
follows:

- `authenticating` (equivalent to `connecting`)
- `connecting` (equivalent to `connecting`)
- `open` (equivalent to `connected`)
- `failed` (equivalent to `disconnected`)
- `closed` (equivalent to `initial`)
- `unavailable` (equivalent to `reconnecting`)
