...
<PropertiesList title="Options">
  ...
  >
    ...
    callback function that returns a Liveblocks token result. Either
  ...
  >
    ...
    objects. [Learn more](#createClientResolveMentionSuggestions).
  </PropertiesListItem>
...
[Text Editor](/docs/use-cases/text-editor) store user IDs in their system, but
no other user information. To display user information in Comments, Text Editor,
...
user IDs. Similarly to [`resolveUsers`](#createClientResolveUsers), you can
...
to the `resolveMentionSuggestions` option in [`createClient`](#createClient).
...
Return the current user’s Presence. [Presence](/docs/products/sync/presence) is
used to store custom properties on each user that exist until the user
disconnects. An example use would be storing a user’s cursor coordinates.
...
when you [enter a room](#Typing-with-client.enter). The example above is using
the following type:
...
Updates the current user’s [Presence](/docs/products/sync/presence). Only pass
the properties you wish to update—any changes will be merged into the current
...
Each such update is a `{ type, node, updates, source }` object.
...
const unsubscribe = room.subscribe(
  ...
  (storageUpdates) => {
    for (const update of storageUpdates) {
      const {
        type, // "LiveObject", "LiveList", "LiveMap", or "LiveText"
        node,
        updates,
        source, // Where the change came from, see below
      } = update;
      switch (type) {
        case "LiveObject": {
        ...
        case "LiveText": {
          // updates[0]?.type; is "insert", "delete", or "format"
          // update.node is the LiveText that has been updated
          break;
        }
      }
    }
...
#### Checking where a change came from [#update-source]

Every update carries a `source` field, telling you where the change came from.
Use it to tell changes made by this client apart from changes made by others,
for example when syncing Storage into an editor that already applied the local
change itself.
  ...
  (storageUpdates) => {
    for (const update of storageUpdates) {
      ...
      if (source.origin === "remote") {
        // Changed by another client, and received over the network
      } else if (source.via === "edit") {
        // Changed by this client, as a regular edit
      } else {
        // Changed by this client, replayed from history ("undo" or "redo")
      }
    }
  },
  { isDeep: true }
...
<PropertiesList title="Properties">
  <PropertiesListItem name="source.origin" type='"local" | "remote"'>
    `"local"` if the change was made by this client, `"remote"` if it was made
    by another client and reached this client over the network.
  </PropertiesListItem>
  <PropertiesListItem name="source.via" type='"edit" | "undo" | "redo"'>
    Only present when `origin` is `"local"`. How the change was made: a regular
    edit, or a replay from the [undo/redo history](#Room.history).
  </PropertiesListItem>
</PropertiesList>

When a single notification merges local and remote changes to the same node, the
merged update is reported as `{ origin: "remote" }`.
...
<PropertiesList title="Returns">
  <PropertiesListItem name="feeds" type="Feed[]">
    Feeds within the current room.
  </PropertiesListItem>
  <PropertiesListItem name="nextCursor" type="string | undefined">
    Cursor for fetching the next page of feeds.
  </PropertiesListItem>
</PropertiesList>

<PropertiesList title="Options">
  <PropertiesListItem name="cursor" type="string">
    Optional cursor for pagination.
  </PropertiesListItem>
  <PropertiesListItem name="since" type="number">
    Optional timestamp filter (ms). Only messages whose `createdAt` is at or
    after this value are included.
  </PropertiesListItem>
  <PropertiesListItem name="limit" type="number">
    Optional limit for the number of feeds to return.
  </PropertiesListItem>
  <PropertiesListItem name="metadata" type="object">
...
<PropertiesList title="Returns">
  <PropertiesListItem name="messages" type="FeedMessage[]">
...
<PropertiesList title="Arguments">
  ...
  <PropertiesListItem name="options.metadata" type="object">
    Optional custom metadata for the feed.
  </PropertiesListItem>
  <PropertiesListItem name="options.timestamp" type="number">
...
<PropertiesList title="Arguments">
  ...
  <PropertiesListItem name="metadata" type="object" required>
...
<PropertiesList title="Arguments">
  ...
  <PropertiesListItem name="data" type="object" required>
    The message data.
  </PropertiesListItem>
  <PropertiesListItem name="options.id" type="string">
...
<PropertiesList title="Arguments">
  ...
  <PropertiesListItem name="messageId" type="string" required>
...
[notification webhook events](/docs/api-reference/webhook-events#NotificationEvent)
will be sent for the current user. Notification settings are project-based,
which means that this modifies the current user’s settings in every room. Each
...
- [`LiveFile`][] - An immutable reference to a file uploaded to Liveblocks. Use
  this for large files such as images and videos. All file types are supported.
  Learn more about [file size limits](/docs/pricing/limits).
...
Uploads a file to Liveblocks and returns a [`LiveFile`][] which can then be
added to the room's Storage tree.

```ts
const liveFile = await room.uploadFile(file);

const { root } = await room.getStorage();
root.set("file", liveFile);
```

<PropertiesList title="Returns">
  <PropertiesListItem name="liveFile" type="LiveFile">
    An immutable Storage reference to the uploaded file.
  </PropertiesListItem>
</PropertiesList>
...
<PropertiesList title="Arguments">
  ...
  <PropertiesListItem name="options" type="UploadFileOptions">
    A set of options.
  </PropertiesListItem>
</PropertiesList>
...
<PropertiesList title="Options">
  <PropertiesListItem name="signal" type="AbortSignal">
    An optional signal for cancelling the upload.
  </PropertiesListItem>
</PropertiesList>

### Room.getFileUrl

Returns a presigned URL for the file referenced by a [`LiveFile`][]. You can
pass a `LiveFile`, its `LiveFileData`, or its ID.
...
### LiveObject.from [#LiveObject.from]
...
```ts
const liveObject = LiveObject.from({
  name: "Grace",
...
const liveObject = new LiveObject({
  ...
  address: new LiveObject({ city: "New York", zip: "10001" }),
});
```

<PropertiesList title="Returns">
  <PropertiesListItem name="liveObject" type="LiveObject">
    A new `LiveObject` with deeply converted children.
  </PropertiesListItem>
</PropertiesList>

<PropertiesList title="Arguments">
  <PropertiesListItem name="obj" type="JsonObject" required>
...
<PropertiesList title="Returns">
  <PropertiesListItem name="snapshot" type="object">
...
### reconcile [#LiveObject.reconcile]
...
// Updates nested.y, deletes age (absent from input)
liveObject.reconcile({ name: "Grace", nested: { x: 1, y: 99 } });
```

<PropertiesListEmpty title="Returns">_Nothing_</PropertiesListEmpty>

<PropertiesList title="Arguments">
  <PropertiesListItem name="jsonObj" type="JsonObject" required>
...
<Banner title="Removed in 3.18" type="warning">

This method has been replaced by [`.toJSON()`](#LiveObject.toJSON), which
...
// { florent: { role: "engineer" }, marc: { role: "designer" } }
map.toJSON();
```
...
This method has been replaced by [`.toJSON()`](#LiveMap.toJSON), which returns
...
// [{ name: "Olivier" }, { name: "Vincent" }]
list.toJSON();
```

<PropertiesList title="Returns">
  <PropertiesListItem name="snapshot" type="array">
...
This method has been replaced by [`.toJSON()`](#LiveList.toJSON), which returns
...
<Banner title="Deprecated" type="warning">

Use [`.toJSON()`](#LiveList.toJSON) instead. It’s faster, cached, and deeply
...
The `LiveText` class represents a collaborative rich-text document that is
synchronized across clients. Use it to store plain text with optional inline
formatting attributes. To add typing, read more under
[typing Storage](#typing-storage).

Unlike [`LiveList`][], [`LiveMap`][], and [`LiveObject`][], a `LiveText` node
cannot contain child Storage structures. It stores a flat sequence of text
segments, each with optional attributes.

```ts
type Document = LiveText;
```

### Data format

A `LiveText` document is serialized as an array of segments. Each segment is
either a plain string, or a tuple of `[text, attributes]`:
...
Attributes are JSON objects. Set an attribute to `null` to remove it from a
range (for example `{ bold: null }`).

<Banner title="Size limit">

A single `LiveText` node can currently hold up to around 2 MB of text and
inline attributes. Documents that can grow past that should be split across
multiple `LiveText` nodes, for example one per section or block. For full-page
editor experiences, where the page is the document and its text can keep
growing, we still recommend [Yjs](/docs/api-reference/liveblocks-yjs). See
[LiveText vs Yjs](/docs/guides/livetext-vs-yjs) for the full comparison.

</Banner>

### new LiveText [#LiveText.constructor]

Create an empty `LiveText`.

```ts
const text = new LiveText();
```

Create a `LiveText` with initial plain text.

```ts
const text = new LiveText("Hello world");
```

Create a `LiveText` with initial formatted data.

```ts
const text = new LiveText([["Hello ", { bold: true }], ["world"]]);
```

<PropertiesList title="Returns">
  <PropertiesListItem name="LiveText" type="LiveText">
    The newly created `LiveText`.
  </PropertiesListItem>
</PropertiesList>

<PropertiesList title="Arguments">
  <PropertiesListItem name="textOrData" type="string | LiveTextData">
    Initial plain text, or an array of text segments with optional attributes.
    Defaults to an empty document.
  </PropertiesListItem>
</PropertiesList>

### insert [#LiveText.insert]

Inserts text at the given index. Optionally applies inline attributes to the
inserted text.
...
<PropertiesList title="Arguments">
  <PropertiesListItem name="index" type="number" required>
    The character index at which to insert. Values outside the document range
    are clipped.
  </PropertiesListItem>
  <PropertiesListItem name="text" type="string" required>
    The text to insert.
  </PropertiesListItem>
  <PropertiesListItem name="attributes" type="LiveTextAttributes">
    Optional inline attributes to apply to the inserted text.
  </PropertiesListItem>
</PropertiesList>

### delete [#LiveText.delete]
...
<PropertiesList title="Arguments">
  <PropertiesListItem name="index" type="number" required>
    The character index at which to start deleting.
  </PropertiesListItem>
  <PropertiesListItem name="length" type="number" required>
    The number of characters to delete.
  </PropertiesListItem>
</PropertiesList>

### replace [#LiveText.replace]

Replaces a range of text with new text. Equivalent to deleting the range and
inserting new text at the same index.
...
<PropertiesList title="Arguments">
  <PropertiesListItem name="index" type="number" required>
    The character index at which to start replacing.
  </PropertiesListItem>
  <PropertiesListItem name="length" type="number" required>
  ...
  <PropertiesListItem name="attributes" type="LiveTextAttributes">
    Optional inline attributes to apply to the replacement text.
  </PropertiesListItem>
</PropertiesList>

### format [#LiveText.format]

Applies or removes inline attributes on a range of text. Set an attribute to
`null` to remove it.
...
<PropertiesList title="Arguments">
  <PropertiesListItem name="index" type="number" required>
    The character index at which to start formatting.
  </PropertiesListItem>
  <PropertiesListItem name="length" type="number" required>
    The number of characters to format.
  </PropertiesListItem>
  <PropertiesListItem name="attributes" type="LiveTextAttributesPatch" required>
    Attributes to apply. Use `null` as a value to remove an attribute from the
    range.
  </PropertiesListItem>
</PropertiesList>

### length [#LiveText.length]
...
### toString [#LiveText.toString]

Returns the plain text content of the document, without attributes. Joining the
text from each segment in [`toJSON()`](#LiveText.toJSON) produces the same
result.
...
// Equivalent when working with LiveTextData:
text
  .toJSON()
  .map(([segmentText]) => segmentText)
  .join("");
```
...
### toJSON [#LiveText.toJSON]

Returns a JSON-compatible snapshot of the document as a `LiveTextData` array.
The result is cached and only recomputed when the contents change.
...
<PropertiesList title="Returns">
  <PropertiesListItem name="snapshot" type="LiveTextData">
    A plain JSON-compatible array of text segments. Always serializable —
    `JSON.stringify()` works out of the box.
  </PropertiesListItem>
</PropertiesList>

<PropertiesListEmpty title="Arguments">_None_</PropertiesListEmpty>

### clone [#LiveText.clone]

Returns a deep clone of this `LiveText` that does not share mutable state with
the original.
...
<PropertiesList title="Returns">
  <PropertiesListItem name="clone" type="LiveText">
    A new `LiveText` instance with the same content.
  </PropertiesListItem>
</PropertiesList>

<PropertiesListEmpty title="Arguments">_None_</PropertiesListEmpty>

### Example: editing Storage [#LiveText.example]

Here’s a complete example of creating a room with a `LiveText` document and
editing it collaboratively.
...
```ts
import { LiveText, createClient } from "@liveblocks/client";
...
document.insert(document.length, "Hello world");

// Make the first word bold
document.format(0, 5, { bold: true });
...
const unsubscribe = room.subscribe(
  ...
  (updates) => {
    for (const update of updates) {
      if (update.type !== "LiveText") continue;

      for (const change of update.updates) {
        if (change.type === "insert") {
          console.log(`Inserted "${change.text}" at ${change.index}`);
        }
      }
    }
  },
...
Each update also carries a [`source`](#update-source), saying whether the change
was made by this client or by another one.

`LiveText` edits support [undo and redo](#Room.history) when made through a
connected room.

To build a collaborative [CodeMirror](https://codemirror.net) editor, use
[`@liveblocks/codemirror`](/docs/api-reference/liveblocks-codemirror). For
[ProseMirror](https://prosemirror.net/), use
[`@liveblocks/prosemirror`](/docs/api-reference/liveblocks-prosemirror). For
[Tiptap](https://tiptap.dev/), use
...
The `LiveFile` class is an immutable Storage leaf that references file bytes
stored outside the realtime Storage tree. Create one by uploading a file with
[`Room.uploadFile`](#Room.uploadFile), then add it to a `LiveObject`, `LiveMap`,
or `LiveList`.

```ts
const liveFile = await room.uploadFile(file);

// { id: "fl_xxx", name: "photo.png", size: 12345, mimeType: "image/png" }
console.log(liveFile.data);
```

<PropertiesList title="Properties">
  <PropertiesListItem name="data" type="Readonly<LiveFileData>">
    The immutable file metadata.
  </PropertiesListItem>
  <PropertiesListItem name="id" type="string">
    The file ID, which starts with `"fl_"`.
  </PropertiesListItem>
  ...
  <PropertiesListItem name="mimeType" type="string">
    The MIME type of the uploaded file.
  </PropertiesListItem>
</PropertiesList>

### clone [#LiveFile.clone]

Returns a copy of the `LiveFile` that references the same uploaded file and can
be inserted elsewhere in the Storage tree.

```ts
root.set("original", liveFile);
root.set("copy", liveFile.clone());
```

<PropertiesList title="Returns">
  <PropertiesListItem name="clonedFile" type="LiveFile">
    The cloned `LiveFile`.
  </PropertiesListItem>
</PropertiesList>
