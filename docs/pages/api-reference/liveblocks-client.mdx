...
copilot to look up information on-demand or perform actions in your application
on behalf of the current user, such as creating content, updating the
application state, or interacting with external services.
  ...
  render: ({ result }) => (
    <AiTool title="User Lookup" icon="👤">
      {!result ? (
        ...
        <div>Found user: {result.user.name}</div>
      )}
    </AiTool>
  ),
});
...
(double-parens). This allows TypeScript's inference to work correctly.
...
[`RegisterAiTool`](/docs/api-reference/liveblocks-react-ui#RegisterAiTool) or
...
<PropertiesList>
  ...
  <PropertiesListItem name="value" type="string">
    ...
    format that provides context to the AI.
  </PropertiesListItem>
</PropertiesList>

Knowledge sources can be registered using
[`RegisterAiKnowledge`](/docs/api-reference/liveblocks-react-ui#RegisterAiKnowledge)
...
const userKnowledge: AiKnowledgeSource = {
  ...
  value: JSON.stringify({ name: "John Doe", role: "admin" }),
};
...
### Room.getNotificationSettings [@badge=Deprecated]

<Banner title="Deprecated" type="error">
  This method was renamed to [`room.getSubscriptionSettings`][] in 2.24 and
  removed in 3.0, read more in our [migration
  guide](/docs/platform/upgrading/2.24).
</Banner>
...
### Room.updateNotificationSettings [@badge=Deprecated]

<Banner title="Deprecated" type="error">
  This method was renamed to [`room.updateSubscriptionSettings`][] in 2.24 and
  removed in 3.0, read more in our [migration
  guide](/docs/platform/upgrading/2.24).
</Banner>
