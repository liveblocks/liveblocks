...
user’s name or avatar, you need to resolve these IDs into user objects. This
...
Return the current user’s Presence. [Presence](/docs/products/presence) is used
to store custom properties on each user that exist until the user disconnects.
An example use would be storing a user’s cursor coordinates.
...
Updates the current user’s [Presence](/docs/products/presence). Only pass the
properties you wish to update—any changes will be merged into the current
...
<PropertiesList title="Options">
  <PropertiesListItem name="query" type="{ resolved, metadata }" optional>
    Options for filtering threads.
  </PropertiesListItem>
  <PropertiesListItem name="query.resolved" type="boolean" optional>
    Only return `resolved` or `unresolved` threads.
  </PropertiesListItem>
  <PropertiesListItem name="query.metadata" type="{ key: value }" optional>
    Only return threads containing the custom metadata. Metadata is set yourself when creating a thread, for example `{ priority: "HIGH" }`.
  </PropertiesListItem>
</PropertiesList>
...
modified or deleted since the requested date.

```ts
const { threads, inboxNotifications, requestedAt } = await room.getThreadsSince(
  { since: result.requestedAt }
);
```

Helpful when used in combination with [`Room.getThreads`](#Room.getThreads) to
initially fetch all threads, then update later.

```ts
const initialThreads = await room.getThreads();

const latestThreads = await room.getThreadsSince({
  since: initialThreads.requestedAt,
});
```
...
<PropertiesList title="Returns">
  <PropertiesListItem
    name="threads"
    type="{ modified: ThreadData[], deleted: ThreadDeleteInfo[] }"
  >
    Threads that have been modified or deleted since the requested date.
  </PropertiesListItem>
  <PropertiesListItem
    name="inboxNotifications"
    type="{ modified: InboxNotificationData[], deleted: InboxNotificationDeleteInfo[] }"
  >
    Inbox notifications that have been modified or deleted since the requested
    date.
  </PropertiesListItem>
  <PropertiesListItem name="requestedAt" type="Date">
    The request date to use for subsequent polling.
...
<PropertiesList title="Options">
  <PropertiesListItem name="since" type="Date">
    Only return threads that have been modified or deleted after this date.
  </PropertiesListItem>
  <PropertiesListItem name="query" type="{ resolved, metadata }" optional>
    Options for filtering threads.
  </PropertiesListItem>
  ...
  <PropertiesListItem name="query.metadata" type="Patchable<ThreadMetadata>" optional>
    Only return threads containing the custom metadata. Metadata is set yourself when creating a thread, for example `{ priority: "HIGH" }`.
  </PropertiesListItem>
</PropertiesList>
...
`room.getThreadsSince` is helpful for update an existing list of threads with
any updates. Calling `update` in the snippet below fetches all new threads.

```ts
let requestedAt = new Date(0);
const threads = [];
const inboxNotifications = [];

// Your custom thread query
const query = {
  resolved: true,
  metadata: { priority: "HIGH" },
};

// Fetches the latest threads
async function update() {
  const newThreads = await room.getThreadsSince({
    since: requestedAt,
    query,
  });

  requestedAt = newThreads.requestedAt;
  threads.push(newThreads.threads);
  inboxNotifications.push(newThreads.inboxNotifications);
}
...
<PropertiesList title="Options">
  ...
  <PropertiesListItem name="metadata" type="Patchable<ThreadMetadata>">
...
Edits a comment, replacing its existing comment body. Learn more about
[creating comment content](#creating-comment-content).
...
<PropertiesList title="Options">
  ...
  <PropertiesListItem name="emoji" type="string">
...
Returns the modified and deleted inbox notifications and their associated
threads since the requested date.
...
Helpful when used in combination with
[`Client.getInboxNotifications`](#Client.getInboxNotifications) to initially
fetch all inbox notifications, then update later.

```ts
const initial = await room.getInboxNotifications();

const latest = await room.getInboxNotificationsSince({
...
<PropertiesList title="Returns">
  <PropertiesListItem
    name="inboxNotifications"
    detailedType="{ modified: InboxNotificationData[], deleted: InboxNotificationDeleteInfo[] }"
  >
    Inbox notifications that have been modified or deleted since the requested
    date.
  </PropertiesListItem>
  <PropertiesListItem
    name="threads"
    detailedType="{ modified: ThreadData[], deleted: ThreadDeleteInfo[] }"
  >
    Threads that have been modified or deleted since the requested date.
  </PropertiesListItem>
  <PropertiesListItem name="requestedAt" type="Date">
...
<PropertiesList title="Arguments">
  <PropertiesListItem name="options" type="{ since: Date }">
    `since`: Only the inbox notifications modified or deleted after this date
    will be returned.
  </PropertiesListItem>
</PropertiesList>

#### Updating inbox notifications

`client.getInboxNotificationsSince` is helpful for update an existing list of
notifications with any updates. Calling `update` in the snippet below fetches
all new inbox notifications.
...
// Fetches the latest threads
async function update() {
  const newNotifications = await client.getInboxNotificationsSince({
    since: requestedAt,
  });

  requestedAt = newNotifications.requestedAt;
  inboxNotifications.push(newNotifications.inboxNotifications);
  threads.push(newNotifications.threads);
}
...
Gets the number of unread inbox notificatiosn for the current user.
