...
By default, Liveblocks applications will always try to maintain an active
connection to the Liveblocks servers, even when running in a background tab.

If it’s okay for your application to disconnect from the Liveblocks servers
after a period of inactivity, then you can specify a
`backgroundKeepAliveTimeout` value. Setting this value may result in a lower
bill.

When specified, the client may disconnect inactive background applications from
the room automatically if they have been in the background for _at least_ this
amount of time. (In practice it may take a few seconds longer.)

When the background tab gets focused again, the client will immediately try to
reconnect to the room and synchronize the document.
...
We recommend setting this value to at least multiple minutes.
