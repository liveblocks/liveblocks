...
### Client.getChannelsNotificationSettings

Returns the current user's channels notification settings.

```ts
const settings = await client.getChannelsNotificationSettings();
...
<PropertiesList title="Returns">
  <PropertiesListItem name="settings" type="ChannelsNotificationSettings">
    Current user’s channels notification settings.
  </PropertiesListItem>
</PropertiesList>
...
Updates the current user’s channels notification settings.

```ts
const settings = await client.updateChannelsNotificationSettings({
  email: { thread: false },
  slack: { thread: true },
});
...
<PropertiesList title="Arguments">
  <PropertiesListItem name="settings" type="object" required>
    A deep partial object containing the channels notification settings to
    update.

    ```js title="deep partial 1" isCollapsable isCollapsed
    {
      email: { thread: false },
      slack: { thread: true },
    }
    ...
    ```js title="deep partial 2" isCollapsable isCollapsed
    {
      email: { thread: false, textMention: true },
      webPush: { textMention: false },
    }
    ```
