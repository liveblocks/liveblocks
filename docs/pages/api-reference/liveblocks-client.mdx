...
function (unsubscribe function). The authentication endpoint is called as soon
...
We no longer recommend this API, because issues will occur if you 
simultaneously connect to the same room with multiple instances.
We recommend using [`client.enterRoom`][] instead.
...
We no longer recommend using this API, because it will cause issues if you have
multiple component trees in your application that connect to the same room.
Prefer the new and improved [`client.enterRoom`][] API.
...
You’ll likely not need this API if you’re using the new [`client.enterRoom`][]
API.
...
storage mutations), but since there is no connectivity any longer, those changed
will not be persisted or synchronized until the room instance is reconnected to
the server again.
