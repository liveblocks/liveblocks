...
Note that when using Notifications, the param `room` can be undefined. This
means that the client requests a token that grants access to multiple rooms
instead of a specific room.
...
const client = createClient({
  authEndpoint: async (room) => {
    const response = await fetch("/api/liveblocks-auth", {
      ...
      // Don't forget to pass `room` down,
      //  and note that in some cases, `room` can be undefined.
...
`resolveUsers` option in [`createClient`](#createClient).
...
<Banner title="User object">
  ...
  default. The [default
  components](/docs/api-reference/liveblocks-react-comments#Components) look
  specifically for the `name` and `avatar` properties to display user names and
  avatars.
</Banner>
...
When using [Notifications](/docs/products/comments/notification-inbox) with
