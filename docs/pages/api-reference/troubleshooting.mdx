...
Liveblocks data structures like `LiveObject`, `LiveMap`, and `LiveList` require
that their payloads are always JSON-serializable to be able to send them over
WebSocket connections reliably and without surprises. `LiveFile` is the
exception: it stores immutable file metadata in the Storage tree while the file
bytes are uploaded separately. Starting with 0.16, we’re enforcing valid values
with the `Lson` type constraint. (LSON is a Liveblocks-specific extension of
JSON that also allows nesting more Live data structures.)
