...
Displays accept/reject controls near the current suggested edit. Suggestions are
enabled with the `suggestions` option in [`useLiveblocksExtension`][].
...
Use the `setSuggestionMode` command to start recording local edits as
suggestions. Suggested insertions and deletions remain visible in the editable
document so selections and collaborative positions stay stable.

```tsx
<button onClick={() => editor?.commands.setSuggestionMode(true)}>
  Suggest edits
</button>
```

Accepting and rejecting suggestions can also be handled with commands.
...
If you need to render a document without pending suggestions, use
`getCleanSuggestionContent` with Tiptap JSON output and pass the result to your
renderer/export pipeline.

```tsx
import { getCleanSuggestionContent } from "@liveblocks/react-tiptap";

const cleanContent = getCleanSuggestionContent(editor.getJSON());
```

#### Props [#FloatingSuggestions-props]

<PropertiesList>
  <PropertiesListItem name="editor" type="Editor | null" required>
    The Tiptap editor.
  </PropertiesListItem>
  <PropertiesListItem name="children" type="function">
    Customize the review controls. Receives the suggestion, plus `accept` and
    `reject` callbacks.
  </PropertiesListItem>
</PropertiesList>
...
<PropertiesList title="Arguments">
  ...
  <PropertiesListItem
    name="options.suggestions"
    defaultValue="false"
    type="boolean | SuggestionsConfiguration"
  >
    Enable suggestion mode commands and marks. When suggestion mode is turned on
    with `editor.commands.setSuggestionMode(true)`, local insertions and
    deletions are recorded as suggested changes.
  </PropertiesListItem>
  <PropertiesListItem
    name="options.ai"
    defaultValue="false"
