...
<PropertiesList title="Arguments">
  ...
  <PropertiesListItem name="options.field" type="string">
    ...
    one page, if each has a separate field value. In `collaborationMode:
    "liveblocks"`, documents are stored in `root._tiptap_docs` and keyed by this
    field. [Learn more](#Multiple-editors).
  </PropertiesListItem>
  <PropertiesListItem
...
When using `collaborationMode: "liveblocks"`, these editors are stored in
`root._tiptap_docs`, keyed by their `field` values. That means you can list all
known editors directly from storage.
