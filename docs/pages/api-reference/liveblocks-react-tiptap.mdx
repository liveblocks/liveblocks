...
mentions. Read our [get started guides](/docs/get-started/multiplayer/text-documents) to
learn more. Use
...
<Banner title="LiveText vs Yjs">
  Liveblocks Tiptap fully supports both [LiveText and Yjs-backed
  collaboration](/docs/guides/livetext-vs-yjs). Yjs is used by default, but opt
  into using `LiveText` instead by passing [`collaborationMode:
  "liveblocks"`](#Liveblocks-collaboration-mode).
</Banner>
...
Learn more in our [get started guides](/docs/get-started/multiplayer/text-documents).
...
#### Extending the defaults [#floating-toolbar-extending-the-defaults]
...
<PropertiesList title="Arguments">
  ...
  <PropertiesListItem
    name="options.collaborationMode"
    defaultValue='"yjs"'
    type='"yjs" | "liveblocks"'
  >
    The collaboration backend to use. `liveblocks` stores editor documents in
    `root._tiptap_docs`, keyed by `field`, with text nodes backed by
    [`LiveText`](/docs/api-reference/liveblocks-client#LiveText). [Learn
    more](#Liveblocks-collaboration-mode).
  </PropertiesListItem>
  <PropertiesListItem
...
#### Liveblocks collaboration mode

By default, Tiptap uses Yjs-backed collaboration. To store text as
[`LiveText`](/docs/api-reference/liveblocks-client#LiveText) backed by
Liveblocks Storage instead, pass `collaborationMode: "liveblocks"`.

Documents are stored under `root._tiptap_docs`, keyed by each editor’s
[`field`](#Multiple-editors). Comments, mentions, cursors, and toolbars work the
same as in Yjs mode. Read our [React](/docs/get-started/react-tiptap) or
[Next.js](/docs/get-started/nextjs-tiptap) Storage get started guides for a full
setup.
...
Use [`initialContent`](#Setting-initial-content) for the first-load document
when the Storage field is empty.

You can list editors from Storage by reading `_tiptap_docs` keys:
...
[`offlineSupport_experimental`](#Offline-support) and [`ai`](#AiToolbar) require
the default Yjs collaboration mode (`collaborationMode: "yjs"`), as do
server-side editing with
[`@liveblocks/node-prosemirror`](/docs/api-reference/liveblocks-node-prosemirror)
and text mention emails with
[`@liveblocks/emails`](/docs/api-reference/liveblocks-emails). To edit
Storage-backed documents from the server, use
[`Liveblocks.mutateStorage`](/docs/api-reference/liveblocks-node#mutate-storage).

</Banner>

In Storage mode, [`useIsEditorReady`](#useIsEditorReady) is usually not
needed—the editor syncs as soon as Storage has loaded.
...
Used to check if the editor content has been loaded or not in the default Yjs
mode, helpful for displaying a loading skeleton. In
`collaborationMode: "liveblocks"`, this hook is usually not needed.
