...
### getMentionIdsFromCommentBody [#get-mention-ids-from-comment-body]

Returns an array of each user’s ID that has been mentioned in a `CommentBody`
(found under `comment.body`).

```ts
import { getMentionIdsFromCommentBody } from "@liveblocks/node";

const mentionedIds = getMentionIdsFromCommentBody(comment.body);
```
...
```ts
import { Liveblocks, getMentionIdsFromCommentBody } from "@liveblocks/node";
...
```ts
import { CommentBody, getMentionIdsFromCommentBody } from "@liveblocks/node";
...
// Get the mentions inside the comment's body
const mentionedIds = getMentionIdsFromCommentBody(commentBody);
...
const stringComment = stringifyCommentBody(comment.body);
```
...
// "<b>Hello</b> Marc from <a href="https://liveblocks.io">https://liveblocks.io</a>"
stringifyCommentBody(comment.body, {
  format: "html",
...
// "<b>Hello</b> <a href="https://example.com" data-id="marc@example.com">Marc</a> fr..."
stringifyCommentBody(comment.body, {
  format: "html",
