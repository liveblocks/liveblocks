...
[`useErrorListener`](/docs/api-reference/liveblocks-react#useErrorListener) in
your application to get the new room’s ID, and redirect users to the renamed
room.
...
function App() {
  useErrorListener((error) => {
    if (error.code === 4006) {
      // Room ID has been changed, get the new ID
      const newRoomId = error.message;
      __redirect__(`/document/${newRoomId}}`);
    }
  });
}
```
