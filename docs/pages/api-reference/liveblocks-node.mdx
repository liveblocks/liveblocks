...
```ts
const editedThread = await liveblocks.editThreadMetadata({
  roomId: "my-room-id",
  threadId: "th_d75sF3...",
...
#### Liveblocks.deleteCommentReaction [#post-rooms-roomId-threads-threadId-comments-commentId-remove-reaction] [@badge=Beta]
...
[Delete Comment Reaction API](/docs/api-reference/rest-api-endpoints#post-rooms-roomId-threads-threadId-comments-commentId-remove-reaction)
and returns no response.
...
await liveblocks.deleteCommentReaction({
  ...
  data: {
    ...
    removedAt: new Date(), // Optional, the time the reaction is to be deleted
  },
});
```
