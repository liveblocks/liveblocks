...
custom error class extends the standard JavaScript Error and includes a `status`
property, which provides the HTTP status code for the error, such as 404 for not
found or 500 for server errors.
