...
<Banner title="What are access tokens?">
  Access tokens are like issuing a _hotel key card_ from a hotel’s front desk
  (your back end). Any client with a key card can enter any room that the card
...
<Banner title="What are ID tokens?">
  Issuing identity tokens is like issuing _membership cards_. Anyone with that
  ...
  permissions need to be set upfront using the Liveblocks REST API.
</Banner>
