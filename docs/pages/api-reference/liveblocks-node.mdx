...
const { body, status } = await liveblocks.identifyUser(
  {
    ...
    tenantId: "acme-corp", // Optional, tenant ID
  },
...
const room = await liveblocks.createRoom("my-room-id", {
  ...
  // Optional, the tenant ID to create the room for
  tenantId: "acme-corp",
});
```
...
await liveblocks.triggerInboxNotification({
  ...
  // Optional, the tenant ID to trigger the notification for
  tenantId: "acme-corp",
});
```
...
await liveblocks.deleteAllInboxNotifications({
  userId: "steven@example.com",
  // Optional, the tenant ID to delete notifications for
  tenantId: "acme-corp",
});
```
