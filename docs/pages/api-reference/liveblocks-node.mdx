...
const { body, status } = await liveblocks.identifyUser(
  {
    ...
    tenantId: "acme-inc", // Optional, tenant ID for multi-tenant apps
  },
...
`tenantId` (optional) can be used to specify which tenant this user belongs to
in multi-tenant applications. A tenant ensures separation of data between
different tenants, they are a sub-division of the project. If not provided,
Liveblocks will use the `default` tenant.
