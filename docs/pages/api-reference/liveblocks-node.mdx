...
`startsWith` to match the start of a string. Permissions are taken into account
so querying private threads will return no threads if the current user does not
have access to them.
...
visibility, and a creation date for the comment. Permissions are taken into
account when threads are created and retrieved. A user without write access to
private threads can’t create a private thread, and users without read access to
private threads won’t receive private threads from
[`liveblocks.getThreads`](#get-rooms-roomId-threads) or
