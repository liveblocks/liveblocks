...
const {
  ...
  // Optional query parameter for more advanced filtering
  // Recommend as an object for better type safety
  query: {
    // Optional, filter for rooms with custom metadata in `metadata`
    metadata: {
      myRoomType: "whiteboard",
    },
    // Optional, filter for rooms with a specific starting ID
    roomId: {
      startsWith: "liveblocks:",
    },
  }
  // The query parameter can also be a string, e.g. `query=metadata["myRoomType"]:"whiteboard" AND roomId^"liveblocks:"`
...
- Only the `AND` operator is supported, it can also be omitted and will default
  to `AND`
  - Omitted: `metadata["myRoomType"]:"whiteboard" roomId^"liveblocks:"`
  - Explicit: `metadata["myRoomType"]:"whiteboard" AND roomId^"liveblocks:"`
- `string` values for `metadata` can be exact matched with `:` and `roomId` can
  be prefix matched with `^`
  - Exact: `metadata["myRoomType"]:"whiteboard"`
  - Prefix: `roomId^"liveblocks:"`
...
#### Liveblocks.editThreadMetadata [#post-rooms-roomId-threads-threadId-metadata] [@badge=Beta]
...
// { color: "yellow", page: 3, resolved: true }
...
// { title: "My thread title", page: 3, resolved: true }
console.log(editedMetadata);
```

#### Liveblocks.createComment [#post-rooms-roomId-threads-threadId-comments] [@badge=Beta]
...
#### Liveblocks.editComment [#post-rooms-roomId-threads-threadId-comments-commentId] [@badge=Beta]
...
#### Liveblocks.deleteComment [#delete-rooms-roomId-threads-threadId-comments-commentId] [@badge=Beta]
...
#### Liveblocks.addCommentReaction [#post-rooms-roomId-threads-threadId-comments-commentId-add-reaction] [@badge=Beta]
...
#### Liveblocks.removeCommentReaction [#post-rooms-roomId-threads-threadId-comments-commentId-remove-reaction] [@badge=Beta]
...
#### Liveblocks.updateRoomNotificationSettings [#post-rooms-roomId-users-userId-notification-settings] [@badge=Beta]
...
#### Liveblocks.deleteRoomNotificationSettings [#delete-rooms-roomId-users-userId-notification-settings] [@badge=Beta]
