...
#### Liveblocks.getChannelNotificationSettings [#get-users-userId-channels-channelId-notification-settings]

Returns the user's channels notification settings. This a wrapper around the
[ADD REST API LINK]().

```ts
const settings = await liveblocks.getChannelNotificationSettings({
...
#### Liveblocks.updateChannelNotificationSettings [#post-users-userId-channels-channelId-notification-settings]

Updates the user's channels notification settings. This a wrapper around the
[ADD REST API LINK]().

```ts
const updatedSettings = liveblocks.updateChannelsNotificationSettings({
  userId: "steven@liveblocks.io",
  data: {
    email: { thread: false },
    slack: { thread: true },
  },
...
// { email: { thread: false, ... }, slack: { thread: true, ... }, ... }
console.log(updatedSettings);
```

### Liveblocks.deleteChannelsNotificationSettings [#delete-users-userId-channels-channelId-notification-settings]

Deletes the user's channels notification settings. This a wrapper around the
[ADD REST API LINK]().

```ts
await liveblocks.deleteChannelsNotificationSettings({
  userId: "adri@liveblocks.io",
});
```
