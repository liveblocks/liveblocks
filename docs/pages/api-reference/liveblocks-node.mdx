...
#### Liveblocks.deleteInboxNotifications [#delete-users-userId-inboxNotifications-inboxNotificationId]

Deletes all inbox notifications for a user. This is a wrapper around the
[Delete Inbox Notifications API](/docs/api-reference/rest-api-endpoints#delete-users-userId-inboxNotifications).

```ts
await liveblocks.deleteInboxNotifications({
  userId: "steven@example.com",
});
```
