...
- [`Liveblocks.prepareSession`](#access-tokens) Access token authentication is
  ...
  [limitations](/docs/api-reference/authentication/access-token#limitations).
...
<Banner>
  Learn how to [get started with ID
  tokens](/docs/api-reference/authentication#id-token).
</Banner>
...
complex
[workspace permissions](/docs/api-reference/authentication#id-token-workspace-permissions)
and
[room permissions](/docs/api-reference/authentication#id-token-room-permissions).
For example, this user can only see resources in the `acme-corp` workspace, and
they’re part of a `marketing` rooms group within it.
...
When using
[text editor integrations](/docs/products/sync/integrations#official-packages),
...
[ID tokens](/docs/api-reference/authentication#id-token-authenticating) from
your back end. An ID token does not grant any permissions in the token directly.
Instead, it only securely identifies your user, and then uses any permissions
set via the [Permissions REST API][] to decide whether to allow the user on a
room-by-room basis.
...
[authentication section](/docs/api-reference/authentication#select-your-framework).
...
<Banner>
  ...
  tokens](/docs/api-reference/authentication/access-token).
</Banner>
...
[naming patterns](/docs/api-reference/authentication/access-token#Naming-pattern).
...
<Banner>
  ...
  permissions](/docs/api-reference/authentication/access-token#permissions).
</Banner>

Additionally, you can pass additional options to `prepareSession`, enabling you
to create complex permissions using
[organizations](/docs/api-reference/authentication/organizations) and
[accesses](/docs/api-reference/authentication/permissions). For example, this
...
[access tokens](/docs/api-reference/authentication/access-token) from your back
end.
...
[authentication section](/docs/api-reference/authentication/access-token).
...
const { data: rooms, nextCursor } = await liveblocks.getRooms({
  ...
  query: {
    ...
    // Optional, filter by whether rooms contain at least one thread
    hasThreads: true,
  },
...
The `hasThreads` filter includes threads regardless of their resolved status or
visibility.
...
[ID token permission to enter](/docs/api-reference/authentication#id-token-room-permissions).
This is a wrapper around the
...
more about
[managing permission with ID tokens](/docs/api-reference/authentication#id-token-room-permissions).
...
#### Liveblocks.uploadFile [#upload-file]

Uploads a file to a room and returns a
[`LiveFile`](/docs/api-reference/liveblocks-client#LiveFile) which can then be
...
#### Liveblocks.getFileUrl [#get-file-url]

Returns a presigned URL and its expiration time for a
[`LiveFile`](/docs/api-reference/liveblocks-client#LiveFile). You can pass a
`LiveFile`, its `LiveFileData`, or its ID. Throws an error if the room or file
isn't found.

```ts
const { url, expiresAt } = await liveblocks.getFileUrl({
  roomId: "my-room-id",
  file: liveFile,
});
```
...
<PropertiesList>
  ...
  <PropertiesListItem name="expiresAt" type="string">
...
Returns a room’s version history snapshots, sorted by creation date from newest
to oldest. Throws an error if the room isn’t found. This is a wrapper around the
...
Yjs documents. Throws an error if the room isn’t found. This is a wrapper around
the
...
Returns a specific version of a room’s Yjs document encoded as a binary Yjs
update. Throws an error if the room or version isn’t found. This is a wrapper
around the
...
Permanently deletes a version from a room’s history. Throws an error if the room
or version isn’t found. This is a wrapper around the
...
<PropertiesList>
  <PropertiesListItem name="type" type={'"attachment"'}>
...
<Banner title="Creating a comment from Markdown">

You can also convert a Markdown string to a `CommentBody` with
[`markdownToCommentBody`](#markdown-to-comment-body).
...
[enable batching in the dashboard](/docs/products/notifications/concepts#Notification-batching).
...
example [`getComment`](/docs/api-reference/liveblocks-node#get-rooms-roomId-threads-threadId-comments-commentId).
...
comments with [`createThread`](#post-rooms-roomId-threads),
[`createComment`](#post-rooms-roomId-threads-threadId-comments), or
[`editComment`](#post-rooms-roomId-threads-threadId-comments-commentId).

```ts
import { markdownToCommentBody } from "@liveblocks/node";

const body = markdownToCommentBody(`
...
[`ThreadNotificationEvent`](/docs/api-reference/webhook-events#Thread-notification)
send from Comments. Particularly helpful when creating
...
Type guard to check if a received webhook event is a
[`TextMentionNotificationEvent`](/docs/api-reference/webhook-events#TextMention-notification)
...
Type guard to check if a received webhook event is a
[`CustomNotificationEvent`](/docs/api-reference/webhook-events#Custom-notification)
sent from
...
[Permissions REST API]:
  /docs/api-reference/authentication#id-token-room-permissions
