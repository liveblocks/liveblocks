...
const { data: rooms, nextCursor } = await liveblocks.getRooms({
  ...
  // Optional, filter for rooms that are created for a specific tenant
...
This endpoint is particularly useful when you want to optimize the initial
connection time for users. For example, you might call this method when a user
navigates to a page where they're likely to join a room, but before they
actually connect.
...
const group = await liveblocks.createGroup({
  ...
  // Optional, set tenant ID for multi-tenant applications
...
const updatedCopilot = await liveblocks.updateAiCopilot("co_abc123...", {
  ...
  // Optional, update provider-specific options
