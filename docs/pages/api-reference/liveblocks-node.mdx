...
Subscribes a user to a thread. Throws an error if the room or thread isn’t
found. This is a wrapper around the
...
Unsubscribes a user from a thread. Throws an error if the room or thread isn’t
found. This is a wrapper around the
...
Gets a thread's subscriptions. Throws an error if the room or thread isn’t
found. This is a wrapper around the
...
Returns a user’s subscription settings for a specific room. This is a wrapper
around the
...
Updates a user’s subscription settings for a specific room. This is a wrapper
around the
...
[`Liveblocks.getRoomSubscriptionSettings`](#get-rooms-roomId-users-userId-subscription-settings)
in 2.24, read more in our [migration guide](/docs/platform/upgrading/2.24).
