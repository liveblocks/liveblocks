...
and returns the same response. This method is not rate limited, but each
document runs in a single-threaded isolated microVM, which means it can start to
slow down or crash if it’s overwhelmed with too many requests. If you experience
performance issues, consider adding a self-imposed rate limit.
...
changes. This method is not rate limited, but each document runs in a
single-threaded isolated microVM, which means it can start to slow down or crash
if it’s overwhelmed with too many requests. If you experience performance
issues, consider adding a self-imposed rate limit.
