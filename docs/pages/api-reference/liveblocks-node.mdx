...
If you want to iterate over all your rooms, it’s most convenient to use the
`.iterRooms()` method instead. This method will automatically paginate your API
requests.

#### Liveblocks.iterRooms [#iter-rooms]

List `.getRooms()`, but returns an asynchronous iterator, which will help you
...
for await (const room of liveblocks.iterRooms(query)) {
  console.log(room.id, room.metadata);
}
```
...
Mutates the storage contents of a room arbitrarily.
...
Mutates storage contents for multiple rooms simultaneously:
...
await liveblocks.massMutateStorage(
  ...
  // Optional
  {
    ...
    signal, // Optional: provide an abort signal if you want to cancel the mutations mid-way
  }
);
```

The provided query will determine which rooms are selected for mutation:
