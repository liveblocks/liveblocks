...
[2.0 upgrade guide](/docs/api-reference/upgrading/2.0).

</Banner>

We are making `resolved` a first-class citizen property on
[threads](/docs/products/comments/concepts#Threads). You don’t have to use a
thread’s metadata to set a thread as resolved anymore.
...
[`threadMarkedAsResolved`](/docs/api-reference/webhook-events#ThreadMarkedAsResolvedEvent)
and
[`threadMarkedAsUnresolved`](/docs/api-reference/webhook-events#ThreadMarkedAsUnresolvedEvent)
instead of
[`threadMetadataUpdated`](/docs/api-reference/webhook-events#ThreadMarkedAsUnresolvedEvent).
