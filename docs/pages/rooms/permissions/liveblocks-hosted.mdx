...
meta:
  ...
  description: "Learn more about Liveblocks-hosted permissions"
---

Liveblocks-hosted permissions follows the analogy of a "membership card" through
an ID token. Anyone with that membership card can try to enter a room, but your
permissions are checked at the door. It’s like a clerk sits at the room’s door
that will look your membership card up in the system to see if you have access
every time you try to enter a room. This approach to permissions is powerful but
can be hard to manage, because you will have to make sure all the access
information is entered into our system and kept up to date.
...
To set up your authentication endpoint with Liveblocks-hosted permissions, make
sure to
[follow these quick steps](/docs/rooms/authentication/liveblocks-hosted-permissions).
