...
grants the current user permission to enter certain rooms when connecting to
Liveblocks. This token is created by calling
...
organization, and you need to permit users entry to each organization room at
once.

### Naming pattern

Let’s say that your application has a hierarchy of organizations (aka
workspaces), groups, and rooms. Each room is in a group, each group is in an
organization. An example of a naming pattern would be to combine the three of
these into a unique room name separating them with symbols, such as
`<ORGANIZATION_ID>::<GROUP_ID>/<DOCUMENT_ID>`. A room ID following this pattern
may look like `Vu78Rt::design/9Hdu73`.

<Figure>
  <Image
    src="/assets/authentication/room-name-pattern.png"
    srcDark="/assets/authentication/room-name-pattern-dark.png"
    alt="Splitting a room name into the pattern detailed above"
    width={1536}
    height={864}
    quality={100}
  />
...
This is not a strict naming pattern you must follow, and you can use any pattern
you like.
...
Should you wish to retroactively change room names, you can use [name of
function].

[This API has not been created yet]

## Select your framework [#select-your-framework]

Select your framework for specific instructions on setting up acess token
authentication.
