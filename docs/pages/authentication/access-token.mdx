...
Let’s picture this organization. It has a number of teams, and each team can
create documents that other members of the team can view.

In Liveblocks, a team is a group, and a document is a room. We can take this
hierarchy and turn it into a naming system for room IDs.

Let’s say that your application has a hierarchy of organizations (aka
workspaces), groups, and documents. Each document is in a group, each group is
in an organization. An example of a naming pattern would be to combine the three
of these into a unique room ID separating them with symbols, such as
