...
<Steps>
  ...
  <Step>
    ...
    <StepContent>
      ...
      access to a range of rooms at once.

      In the code snippet below, we’re using a naming pattern and wildcard `*` to give the user access to every room in their organization, and every room in their group.
      ...
      If patterns are not an option and you need to assign permissions to individual
      rooms, you can utilize the `room` property in the request's body to grant access
      to the specific room requested by the client. If the `room` property is undefined,
      it indicates that the client is requesting a token for Notifications, and no
      additional permissions need to be added to the token.
      ...
      export async function POST(request: Request) {
        ...
        if (room && **shouldUserHaveAccess**(user, room)) {
