...
<Steps>
  ...
  <Step>
    ...
    <StepContent>
      ...
      If you cannot use patterns and you want to give permissions to one room at a time.
      You can use the request's body property `room` to give access to the room requested by the client.
      If `room` is undefined, it means the client requested a token for Notifications, you don't have to add any permissions to the token.
