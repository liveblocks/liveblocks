...
<Banner title="How this guide affects usage">

This guide sets up an auth endpoint, so each client is identified and usage
counts as
[monthly active users (MAU)](/docs/platform/limits#What-is-a-monthly-active-user), not
[monthly anonymous connections](/docs/platform/limits#What-is-a-monthly-anonymous-connection)
(these apply when the browser only has a
[public key](/docs/api-reference/liveblocks-client#createClientPublicKey)).
