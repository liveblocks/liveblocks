...
- `room:comment:write` - comment on the document
...
our example we will have two tenants: `acme-inc` and `lumon`.

`acme-inc` will have the following rooms:

- `acme-inc:document:123`
- `acme-inc:document:456`
- `acme-inc:public:123`

`lumon` will have the following rooms:
...
Each room will be setup as such:

```ts
const room = await liveblocks.createRoom(
  "acme-inc:document:123", // or "lumon:document:123" for example, this is the room id
  {
    defaultAccesses: [], // no public access by default
    tenantId: "acme-inc", // or "lumon" for example, this is the tenant id
    groupsAccesses: {
      admin: ["room:write"],
      commenter: ["room:read", "room:presence:write", "room:comment:write"],
      viewer: ["room:read", "room:presence:write"],
...
| Name          | User ID                     | Role in acme-inc | Role in lumon |
| ------------- | --------------------------- | ---------------- | ------------- |
| Mislav Abha   | `mislav.abha@example.com`   | admin            | commenter     |
| Charlie Layne | `charlie.layne@example.com` | reader           | admin         |
| Tatum Paolo   | `tatum.paolo@example.com`   | commenter        | reader        |
| Anjali Wanda  | `anjali.wanda@example.com`  | reader           | commenter     |
