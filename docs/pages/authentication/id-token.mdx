...
<dl>
  ...
  <dd>
    Read access with presence and comments. Enables people to edit their presence and create comments, but only view the room&rsquo;s storage. `isReadOnly` is `true`. Perfect for read-only editors with commenting capabilities.
  </dd>
  <dt>`[]`</dt>
...
// Read-only with comments - everyone can view the room and add comments, but cannot edit content
