...
If you don’t need fine-grained permissions, we recommend using simpler
...
At its most basic, creating an ID token involves calling
[`liveblocks.identifyUser`](/docs/api-reference/liveblocks-node#id-tokens)
inside an API endpoint and returning the token, within `body`. For example, lets
assume this is a POST endpoint at `/api/liveblocks-auth`.
...
To retrieve the token on the front end, pass `/api/liveblocks-auth` to
[`authEndpoint`](/docs/api-reference/liveblocks-client#createClientAuthEndpoint)
in your client. This should be found in your config file.

```ts file="liveblocks.config.ts"
const client = createClient({ authEndpoint: "/api/liveblocks-auth" });
```

We recommend you continue reading to learn about permissions, but if you’d like
to dive right in,
[select your framework to get started](#select-your-framework).
...
To remove a group’s permissions, we can use
[`liveblocks.updateRoom`](/docs/api-reference/liveblocks-node#post-rooms-roomId)
...
```ts
const room = await liveblocks.getRoom("Vu78Rt::design/9Hdu73");
...
The next step is to select your framework for specific instructions on setting
up.
