...
- **Personal thread access** with `comments:personal:read`,
  `comments:personal:write`, or `comments:personal:none`.
- **Feeds** with `feeds:read`, `feeds:write`, or `feeds:none`.

The `comments:*` permissions apply to public threads, private threads, and
threads created by the current user. Use `comments:public:*` and
`comments:private:*` to override one visibility.

Personal thread permissions are based on who created the thread, not the
thread’s `visibility` value. For example, `comments:personal:read` lets a user
read threads they created, and `comments:personal:write` lets a user create
threads because those threads are their own. This is useful when reviewers or
external users should only create and see their own threads.
...
| `comments:read`           | Comments         | Read access to public, private, and personal threads.  |
| `comments:write`          | Comments         | Write access to public, private, and personal threads. |
| `comments:none`           | Comments         | No access to public, private, and personal threads.    |
...
| `comments:personal:read`  | Personal threads | Read access to threads created by the current user.    |
| `comments:personal:write` | Personal threads | Write access for threads created by the current user.  |
| `comments:personal:none`  | Personal threads | No access to threads created by the current user.      |
