...
while `LiveFile` stores references to large files such as images and videos.
Liveblocks permanently stores Storage data in each
[room](/docs/concepts#Rooms), handling scaling and
maintenance for you.
...
Storage is optimized for collaborative data that multiple users modify at the
same time. Binary data such as images and videos should not be stored directly
in `LiveObject`, `LiveList`, or `LiveMap`. Use
[`LiveFile`](/docs/api-reference/liveblocks-client#LiveFile) instead to upload
the file separately and keep its reference in your Storage tree.
...
You can also preview a version’s data with
