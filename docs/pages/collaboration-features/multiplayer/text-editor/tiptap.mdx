...
When using `collaborationMode: "liveblocks"`, editor documents are grouped
inside `root._tiptap_docs`, keyed by each editor’s `field`. This makes it easy
to render editors dynamically from storage.
