...
### Use a different return format

Note that when using group mentions, you must always return a list of mention
objects with a `kind` property, not just user ID strings.
