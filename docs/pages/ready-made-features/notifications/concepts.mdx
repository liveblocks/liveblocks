...
represents the events that happened.

If you want to update an existing inbox notification when a new activity
happens, you can configure the notification kind to have batching enabled. Learn
more about
[batching](/docs/ready-made-features/notifications/concepts#batching-configuration).
...
You can configure a notification kind to have batching enabled. With batching,
when you trigger an inbox notification activity for a specific subject id, it
will update the existing inbox notification instead of creating a new one. An
inbox notification can have up to 50 activities, if you exceed this number, a
new inbox notification will be created.

<Figure>
  <Image
    src="/assets/notifications/inbox-notification-batching.png"
    alt="Inbox notification batching"
    width={1456}
    height={896}
  />
</Figure>
