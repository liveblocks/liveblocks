...
- **[Dashboard](#dashboard)**: Create and manage copilots settings.
...
<Figure>
  <Image
    src="/assets/ai-copilots/dashboard-add-copilot.png"
    alt="Add copilot from the dashboard"
    width={768}
    height={512}
  />
</Figure>

The [Liveblocks dashboard](/dashboard) lets you create, configure, and manage AI
copilots. You can:

- Choose the AI provider (OpenAI, Anthropic, etc.)
- Define the copilot’s role, tone, and behavior
- Specify how it should respond (text, custom components, or both)
- Connect it to actions to interact with your app
- Try out predefined personas like “_make it rhyme_” “_wrong answers only_” or a
  helpful assistant for specific workflows

You can also fine-tune how each copilot interacts with users by adjusting the
model settings. These options are passed through directly to the AI provider,
and can influence the creativity, consistency, and safety of generated content.
...
This allows you to build copilots tailored to different pages or user roles
within your product—like one for onboarding, one for advanced editing, or even a
fun AI for brainstorming.

Each copilot can be configured independently, and tested live in the dashboard
before adding it to your application.
