...
<details style={{ marginTop: -24 }}>
  <summary>Optionally, you can also display a custom component in the chat using `render`.</summary>
...
<StepCompact>
  <StepTitle>Define a custom component to display in the chat</StepTitle>
  <StepContent>

    ```tsx
    render: ({ result }) => {
      const weather = result.data;
      const emoji = weather.condition === "sunny" ? "☀️" : "🌧️";
      return <div>{emoji} in <em>{weather.location}</em></div>;
    }
    ```
...
complex tools that allow you to create confirm/deny dialogs, render custom
components, and more. When writing your
