...
[ID token authentication](/docs/authentication#id-token-room-permissions) the
room itself stores who can access it. A share dialog is a UI over these room
...
Each room holds [permissions](/docs/authentication/permissions) at three levels:
`defaultAccesses` for everyone, `groupsAccesses` for teams, and `usersAccesses`
for individuals. Create documents as private by default, with only the creator
having access, using
...
[authenticating](/docs/authentication#id-token-room-permissions) them, so no
Liveblocks-specific accounts are needed.

### Inviting users [#inviting-users]

When someone submits an email in the share dialog, your server grants access
with
[`Liveblocks.updateRoom`](/docs/api-reference/liveblocks-node#post-rooms-roomId).
Only the accesses you pass are changed—existing members keep theirs. Set a
...
await liveblocks.updateRoom("document-a", {
  usersAccesses: {
    // Invite Quinn as an editor
    "quinn@example.com": ["*:write"],
...
Connected users are affected immediately—someone whose access is removed is
...
await liveblocks.updateRoom("document-a", {
  usersAccesses: {
    "quinn@example.com": ROLES.commenter,
  },
});
```

More granular scopes exist too, such as `storage:read` and `feeds:write`. Learn
more under [Permissions](/docs/authentication/permissions).
...
[authentication](/docs/authentication#id-token-room-permissions)—for example
each user’s departments or workspaces.
...
[organizations](/docs/authentication/organizations) to keep each workspace’s
rooms and users separate.
...
// { "olivier@example.com": ["*:write"], "quinn@example.com": ["*:read"] }
...
[`useSelf`](/docs/api-reference/liveblocks-react#useSelf)—for example hiding the
toolbar from viewers, or only showing the share button to editors.
...
Permissions are enforced on Liveblocks servers, so hiding UI is purely
cosmetic—read-only users can’t modify the document even with a modified client.
