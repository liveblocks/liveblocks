...
meta:
  ...
  description:
    ...
    sharing, public link access, removing users, and invite notifications."
---
...
access, remove people from the document, and notify people when a document is
shared with them.
...
- [**Inviting users**](#inviting-users): Grant a person access from your share
  endpoint.
- [**Disconnecting users**](#disconnecting-users): Reload a removed user’s page
...
Removing access prevents them from joining again, but ID tokens stay valid until
they expire, so a connected user can keep editing until then. However, you can
use other Liveblocks APIs to
[disconnect users immediately](#disconnecting-users).

### Disconnecting users [#disconnecting-users]

When a user’s access is removed, they are not automatically disconnected from
the room. After updating the room accesses, one way to remove users is to
broadcast an event with their user ID using
...
// Tell the removed user to reload
// +++
await liveblocks.broadcastEvent("document-a", {
  type: "USER_REMOVED",
...
On the client, you can listen for this event with
[`useEventListener`](/docs/api-reference/liveblocks-react#useEventListener) and
compare the event’s user ID to the current user with
[`useSelf`](/docs/api-reference/liveblocks-react#useSelf). If they match, reload
the page.
...
function Document() {
  ...
  useEventListener(({ event }) => {
    // +++
    if (event.type === "USER_REMOVED" && event.userId === userId) {
...
After the reload, authentication runs again, the user no longer has access, and
they’re kept out of the room. Learn more about broadcasting custom data under
[Events](/docs/products/sync/events).
