...
[organizations](/docs/authentication/organizations) to give each user a separate
inbox per workspace. Notifications from rooms created with an `organizationId`
belong to that organization automatically, and custom notifications can be
assigned to one when triggered.
...
Learn more under [Organizations](/docs/authentication/organizations).
