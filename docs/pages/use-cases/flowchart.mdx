...
Choose whether to start with a collaborative flowchart or add AI editing to an
existing React Flow app.
...
[Sync](/docs/products/sync/overview).
...
const { output } = await generateText({
  ...
  prompt: `Write a concise label for a review step in a flowchart`,
});
// +++
...
Read [Multiplayer undo/redo](/docs/products/sync/multiplayer-undo-redo) for the
underlying history behavior.
