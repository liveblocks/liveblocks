...
- [**Presence**](#presence): Show remote carets, selections, avatar stacks, and
  agent activity.
- [**Server-side editing**](#server-side-editing): Modify code from a trusted
  back end.
...
Code is stored in [Sync](/docs/products/sync/overview) with carets and
selections broadcast using [presence](/docs/products/sync/presence).
...
[Sync](/docs/products/sync/overview). Local edits are written to Sync, remote
...
const { output } = await generateText({
  ...
  prompt: "Write a unit test for the function in the file",
});
// +++
