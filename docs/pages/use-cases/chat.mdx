...
{/* prettier-ignore */}
{/* DECIDE: Plain muliplayer AI chat
...
[Sync](/docs/products/sync/overview)—each conversation is a feed, and each
...
Chat becomes more powerful when AI can take action actions in your app, not just
...
using [conflict resolution](/docs/products/sync/conflict-resolution).
...
const { output: task } = await generateText({
  ...
  prompt: `Create a task from this conversation: ${JSON.stringify(messages)}`,
});
// +++
...
Users shouldn’t miss messages sent while they’re away—add an inbox tray to your
app and trigger a custom notification when the user has a new messages to read.
