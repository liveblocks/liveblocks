...
- Anonymous/inlined object types
...
To use an object type definition as a “live” object type, wrap it in the
built-in [`LiveObject`](/docs/api-reference/liveblocks-client#LiveObject)
construct, like so:
...
To turn an array into a “live” array, replace it in the built-in
[`LiveList`](/docs/api-reference/liveblocks-client#LiveList) construct when you
...
The first argument to a `LiveMap` construct must always be `String`. Other types
are not supported to use as keys.
