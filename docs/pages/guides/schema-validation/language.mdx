...
`{"x":10, "y":20}`. In this case, the field `color` has been deleted. You cannot
explicitly set fields to undefined or null.

## Built-In complex types

We currently offer one globally complex type, with more coming as we complete
the Beta version of schema validation. These types take expressions as
arguments.

- `LiveObject<typeExpr>` (where typeExpr must be an object type) Object Types

Later on, we will support the following:

- `Array<typeExpr>`
- `LiveList<typeExpr>`
- `LiveMap<String, typeExpr> `
...
The schema below represents the use of the built-in complex type `LiveObject`:
