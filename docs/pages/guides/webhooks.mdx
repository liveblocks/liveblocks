...
1. From the [dashboard](/dashboard) overview, navigate to the project you would
   like to add webhooks to
2. Click on the webhooks tab from the lefthand menu

<Figure>
  <Image
    layout="responsive"
    src="/images/docs/webhooks/webhooks-from-dashboard.png"
    alt="How to find webhooks from the dashboard"
    width={600}
    height={335}
    quality={90}
  />
...
<Figure>
  <Image
    layout="responsive"
    src="/images/docs/webhooks/add-endpoint-from-dash.png"
    alt="How to find webhooks from the dashboard"
    width={600}
    height={335}
    quality={90}
...
is enabled by default, and use the steps outlined in the security verification
section to ensure the validity of the message.
...
Message Filtering section after endpoint creation. The subscribed events can be
edited later in the Endpoints Overview.

1. Select the endpoint you would like to edit from the list of webhooks in the
   dashboard
2. Click the "Edit" button on the "Subscribed events"

<Figure>
  <Image
    layout="responsive"
    src="/images/docs/webhooks/edit-events.png"
    alt="Edit webhooks events"
    width={600}
    height={335}
    quality={90}
  />
...
If your service is unreachable, message retries will automatically be attempted.
If your service incurs considerable downtime (over 10 hours), you can replay
...
preceding attempt. If an endpoint is removed or disabled, delivery attempts to
the endpoint will also be disabled. The schedule for retries is as follows:
