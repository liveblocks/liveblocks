...
If the need arises, you can rotate the signing secret from within the Endpoint portion of the dashboard by clicking on the dropdown menu.  Rotating the secret will sign the message with the new secret and old secret. This method of roation gives you time to update the secret on the client side, without losing any event driven data.
