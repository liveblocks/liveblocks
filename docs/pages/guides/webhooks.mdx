...
## Why Webhooks
Webhooks connect two applications and enable sending data asynchronously through event subscription and HTTP requests.  Using webhooks with Liveblocks decreases the complexity of managing and creating cron jobs which reach out to our API endpoints and poll. Using webhooks make a platform observable because updates between applications are immediately available.
