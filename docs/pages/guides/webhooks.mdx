...
## Why Webhooks
Webhooks connect two applications by letting one subscribe to events created by the other. The listening application (yours) subscribes to events, and the event emitting application (Liveblocks) submits events via HTTP requests to the listening application's URL. Thus, using webhooks with Liveblocks reduces the need to poll Liveblocks data as you are now alerted when the data changes. Leveraging webhooks makes Liveblocks observable and reduces the need for cron jobs to poll Liveblocks data. As a result, your application (the listening application) will have less complex logic related to consuming data.

There are many scenarios where webhooks could benefit your application.  For example, you can use webhooks to monitor when users enter and leave a room. After some time has passed, you could create logic that syncs Liveblocks storage to your application database if no users are remaining.

You could also use webhooks to display how many users are currently active in a room at any given time.  The possibilities are endless, and as time progresses, we will include more events for your application to subscribe to.
## What is a webhook?

Webhooks pass information from one application to another instantaneously. A webhook is simply an HTTP callback triggered by an event. When an application subscribes to an event, and that event occurs, the webhook invokes the HTTP callback with a URL you specify (called the webhook endpoint). Webhooks streamline the collection
of data between two connected services in real-time.

Like an API, webhooks are a mechanism for a client to receive data. The main
difference between webhooks and APIs is webhooks function as an inverted
API. A traditional API receives requests and sends responses, whereas a webhook calls an endpoint that you provide where we send events.
...
3. You verify the message request sent to the endpoint and send a 200 response once the request has been verified
...
package as it will verify the events and provide you with fully typed
events.
