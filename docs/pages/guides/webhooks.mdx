...
Storage is updated when a user writes to storage. This event is throttled and,
as such, may not be triggered for every write. The `updatedAt` field is the time
of the last write.
