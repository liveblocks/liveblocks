...
Webhooks connect applications by allowing one to subscribe to events from the
other. With Liveblocks, the listening application subscribes to events, while
Liveblocks submits events via HTTP requests to the listening application's URL.
This eliminates the need to poll Liveblocks data and makes it observable,
reducing the complexity of data consumption.

Webhooks have numerous benefits, such as monitoring user activity in a room and
syncing Liveblocks storage to your application database. You can also use
webhooks to display the number of active users in a room. As time goes on, more
events will be added for your application to subscribe to.

## What is a webhook?

A webhook is an HTTP callback that is activated by an event. When an application
subscribes to an event, the webhook triggers an HTTP callback at the specified
URL (webhook endpoint), enabling real-time data transfer between your service
and Liveblocks.
...
const storageUpdatedEvent = {
  ...
  data: {
    appId: "my-app-id",
    roomId: "my-room-id",
    updatedAt: "2021-10-06T01:45:56.558Z", // 👈 s time of the last write.
  },
};
```
...
An endpoint is a URL designed to receive event messages and initiate a task
based on the message payload. The sole requirement for creating an endpoint is
that it must be under your control.

### Use Cases

With webhooks, you can now subscribe to the events you are interested in, and be
...
While webhooks can create security vulnerabilities, verifying them can help
mitigate these risks. Verification safeguards against man-in-the-middle, CSRF,
and replay attacks. Because of this, it is essential to prioritize verification
in your integration.
