...
internal packages to increase code sharing.
...
Similar to the [` shouldInitiallyConnect`](#react-shouldInitiallyConnect) prop
...
You no longer need to supply a second argument to`state.liveblocks.enterRoom()`
to pass an explicit initial state. It will use the state in your Redux store,
...
- Change the pattern to the Zustand v4 recommended pattern:
