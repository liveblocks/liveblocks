...
and how to include a `userId` into the `authorize` method if you use it.
...
- An anonymous user who connects to a Liveblocks room, at least once a month who
  can be identified by a cookie (if not using authorization)
...
Then, update your `authorize` function to include the `userId` field.
