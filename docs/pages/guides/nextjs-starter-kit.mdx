...
<details>
  ...
  <dl>
    ...
    <dt>`/lib/server/`</dt>
    <dd>Server functions used for accessing documents, database, liveblocks.</dd>
    <dt>`/pages/`</dt>
    <dd>Next.js pages.</dd>
    <dt>`/primitives/`</dt>
...
authentication, meaning many authentication providers can be configured
with minimal code changes. A demo authentication system is used by default, but
 it's easy to add real providers, such as GitHub, Auth0, and more.
...
To use GitHub auth, make sure you selected "GitHub authentication" when running the installer (or you've set up the [provider manually](#add-multiple-authentication-providers)). This is how to set up your secret keys.
...
GitHub authentication is now complete! Next, [add yourself a user](#how-to-sign-up) to test out your authentication.

### Auth0 authentication [#auth0-authentication]

To use Auth0 auth, make sure you selected "Auth0 authentication" when running the installer (or you've set up the [provider manually](#add-multiple-authentication-providers)). This is how to set up your secret keys.

1. Go to your [Auth0 Dashboard](https://manage.auth0.com/dashboard) and click "Create Application".
...
Auth0 authentication is now set up! Next, [add yourself a user](#how-to-sign-up) to test out your authentication.
...
It's possible to add multiple authentication providers to the starter kit using [NextAuth Providers](https://next-auth.js.org/providers/). Open `/pages/api/auth/[...nextAuth].ts` and place your providers in the object:
...
It's not only possible with GitHub and Auth0, any [NextAuth provider](https://next-auth.js.org/providers/) will work.
...
|____components - Components used in the app
| |____Whiteboard
| |____ShareDialog
| |____Document
| |____Dashboard
...
| |____Cursors
| |____Badge
|____data - A demo database
|____layouts - Page layout components
| |____Authenticated
| |____Document
| |____Dashboard
| |____Marketing
...
|____icons
|____lib
| |____server - Server functions used for accessing documents, database, liveblocks
| | |____database
| | | |____getGroup.ts
| | | |____getUser.ts
| | | |____getGroups.ts
...
| | | |____authorize.ts
| | | |____index.ts
| |____client - Client functions used for accessing documents, database, SWR hooks
| | |____database
| | | |____getGroup.ts
| | | |____getUser.ts
| | | |____getGroups.ts
...
| |____globals.css
|____types
|____utils - Handy utils
|____constants.ts
|____liveblocks.config.ts
|____liveblocks.server.config.ts
|____next.config.js
|____next-auth.d.ts
|____next-env.d.ts
|
