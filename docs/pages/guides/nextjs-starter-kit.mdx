...
At this point, if you chose to install, the starter kit can be run with
...
Then modify [`/pages/api/liveblocks/auth.ts`](https://github.com/liveblocks/liveblocks/blob/main/starter-kits/nextjs-starter-kit/pages/api/liveblocks/auth.ts). First, we'll give an anonymous user
a property:
...
To change the theme, you will alter the media query in `styles/globals.css`.  The two possible values you can pass are `@media (prefers-color-scheme: dark)` and `@media (prefers-color-scheme: light)`
