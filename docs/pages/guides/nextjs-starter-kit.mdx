...
6. Enter the `.env.local` file, which is located alongside `package.json` at the
   root level in your project. This file will contain an randomly generated `NEXTAUTH_SECRET` that will be used for authentication, along with a number of empty variables for you to fill in.
...
11. Add `AUTH_BASE_URL` to your `.env.local`.  The value for local development will be `http://localhost:3000` or whatever port you assign to the starter kit on your machine. 
...
AUTHO_BASE_URL=http://localhost:XXXX
```
