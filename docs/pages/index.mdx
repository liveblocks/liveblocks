---
meta:
  title: "Documentation"
  description:
    "Explore the documentation to discover how to make most of our APIs and
    tools"
showTitle: false
---

<DocsHomepageHero
  title="Documentation"
  description="Liveblocks provides integrations and a scalable infrastructure for building
real-time collaborative experiences."
  actions={[
    {
      title: "Learn with a tutorial",
      href: "/docs/tutorial",
      appearance: "primary",
    },
    {
      title: "Browse examples",
      href: "/examples",
    },
  ]}
/>

<DocsHomepageGrid />

## API Reference

<ListGrid columns={3}>
  <DocsCard
    type="technology"
    title="JavaScript"
    href="/docs/api-reference/liveblocks-client"
    description="@liveblocks/client"
    visual={
      <img
        alt=""
        src="/images/docs/javascript.svg"
        width={24}
        height={24}
        loading="lazy"
      />
    }
  />
  <DocsCard
    type="technology"
    title="React"
    href="/docs/api-reference/liveblocks-react"
    description="@liveblocks/react"
    visual={
      <img
        alt=""
        src="/images/docs/react.svg"
        width={24}
        height={24}
        loading="lazy"
      />
    }
  />
  <DocsCard
    type="technology"
    title="Redux"
    href="/docs/api-reference/liveblocks-redux"
    description="@liveblocks/redux"
    visual={
      <img
        alt=""
        src="/images/docs/redux.svg"
        width={24}
        height={24}
        loading="lazy"
      />
    }
  />
  <DocsCard
    type="technology"
    title="Zustand"
    href="/docs/api-reference/liveblocks-zustand"
    description="@liveblocks/zustand"
    visual={
      <img
        alt=""
        src="/images/docs/zustand.png"
        width={24}
        height={24}
        loading="lazy"
      />
    }
  />
  <DocsCard
    type="technology"
    title="Node.js"
    href="/docs/api-reference/liveblocks-node"
    description="@liveblocks/node"
    visual={
      <img
        alt=""
        src="/images/docs/nodejs.svg"
        width={24}
        height={24}
        loading="lazy"
      />
    }
  />
  <DocsCard
    type="technology"
    title="REST API"
    href="/docs/api-reference/rest-api-endpoints"
    description="HTTP endpoints"
    visual={
      <img
        alt=""
        src="/images/docs/api.svg"
        width={24}
        height={24}
        loading="lazy"
      />
    }
  />
</ListGrid>

## Examples

<ListGrid columns={2}>
  <ExampleCard
    example={{
      title: "Live Cursors",
      slug: "live-cursors",
      image: "/images/examples/thumbnails/live-cursors.jpg",
    }}
    openInNewWindow
  />
  <ExampleCard
    example={{
      title: "Collaborative Whiteboard",
      slug: "collaborative-whiteboard-advanced",
      image:
        "/images/examples/thumbnails/collaborative-whiteboard-advanced.jpg",
      advanced: true,
    }}
    openInNewWindow
  />
</ListGrid>

## Guides

<ListGrid columns={3}>
  <DocsCard
    type="guide"
    title="How to manage rooms and users with Liveblocks"
    href="/docs/guides/"
    tags={["API", "Auth"]}
  />
  <DocsCard
    type="guide"
    title="How to get authentification working with Next.js and Liveblocks"
    href="/docs/guides/"
    tags={["API", "Auth"]}
  />
  <DocsCard
    type="guide"
    title="How to manage rooms and users with Liveblocks"
    href="/docs/guides/"
    tags={["React", "Redux"]}
  />
</ListGrid>

## Community

<ListGrid columns={3}>
  <DocsCard
    title="Twitter"
    href="https://twitter.com/liveblocks"
    description="Follow us on Twitter for the latest news and updates."
    visual={
      <img
        alt=""
        src="/images/docs/twitter.svg"
        width={24}
        height={24}
        loading="lazy"
      />
    }
    openInNewWindow
  />
  <DocsCard
    title="Discord"
    href="https://liveblocks.io/discord"
    description="Join hundreds of developers building with Liveblocks."
    visual={
      <img
        alt=""
        src="/images/docs/discord.svg"
        width={24}
        height={24}
        loading="lazy"
      />
    }
    openInNewWindow
  />
  <DocsCard
    title="GitHub"
    href="https://github.com/liveblocks/liveblocks"
    description="Follow development progress and contribute to the code base."
    visual={
      <img
        alt=""
        src="/images/docs/github.svg"
        width={24}
        height={24}
        loading="lazy"
      />
    }
    openInNewWindow
  />
</ListGrid>
