---
meta:
  title: "Modify storage with JSON Patch"
  parentTitle: "Platform"
  description:
    "Use the JSON Patch HTTP API to modify Liveblocks Storage from any language."
---

The **Storage JSON Patch** endpoint lets you apply partial updates to a room's
Storage document over HTTP. You send an array of operations (add, remove,
replace, move, copy, test) and the server applies them in order. This is
especially useful for languages that don't yet have a native Liveblocks client,
for example: **Python**, so you can still read and update Storage using the REST
API.
...
Use the secret key from your
[Liveblocks dashboard](https://dashboard.liveblocks.io) (Project Settings → API
keys). Never expose the secret key in client-side code.

## Liveblocks data types and paths

Storage is a tree of Liveblocks types: **LiveObject**, **LiveList**, and
**LiveMap**. When building your patch:
...
[API reference](api-reference/rest-api-endpoints) for the Storage JSON Patch
endpoint.
