...
We’ll dive in soon, but here is a sneak peek:

```diff showLineNumbers={false} file="liveblocks.config.ts"
+declare global {
+  // These custom types are all optional, use whichever ones you want/need
...
```diff showLineNumbers={false} file="Component.tsx"
