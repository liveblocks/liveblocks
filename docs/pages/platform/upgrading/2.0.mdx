...
+const { ... } = createRoomContext(client);
```
...
## Step 3: Optional cleanup of type annotations

If you also exported your `Presence`, `Storage`, etc types from
`liveblocks.config.ts` before, you no longer need to do that. The main reason to
export these before was to use them in helper functions that used some of the
