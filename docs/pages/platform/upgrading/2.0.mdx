...
There are also some **breaking changes in this update**. Most APIs are backward
compatible, however there’s a chance that some of these changes will affect your
code base. We’ve built codemods to help you transition.

### Breaking change 1: renamed package [#bc1]

Affects `@liveblocks/react-comments`. We’ve renamed our package
`@liveblocks/react-comments` to `@liveblocks/react-ui`, because our library of
pre-built UI components now contains more than just Comments-related components,
we’ve renamed our package accordingly, so please adjust your imports.
...
Affects `@liveblocks/node`. To avoid confusion with the newly introduced custom
`RoomInfo` type, we’ve renamed the `RoomInfo` type in our `@liveblocks/node`
package.
...
Affects `@liveblocks/node`. In `@liveblocks/node`, none of the client methods,
like `liveblocks.getThread()`, take type params any longer.

```diff showLineNumbers={false}
-const thread = await liveblocks.getThread<MyThreadMetadata>()
...
Affects `@liveblocks/yjs`. To make discoverability and refactorings easier, and
to avoid confusion with the newly introduced `LiveblocksProvider` in our React
package, we’re no longer using default exports, but named exports only:
...
Affects apps using Storage. The `LiveList()` constructor’s argument is no longer
optional, because it causes unneeded but confusing type inference issues:
...
Affects apps using webhooks. The webhook event `NotificationEvent`’s type can
represent multiple kinds of notifications (`"thread"`, `"textMention"`, and
custom ones (e.g. `"$myNotification"`)).
...
Affects all packages. All of the following APIs have been removed in 2.0, as
they were deprecated multiple versions ago:
