...
to introduce small breaking changes to ensure a consistent API for Comments and
Notifications.
...
### `ResolveUsersArgs` and `ResolveMentionSuggestionsArgs`

Because the resolvers have moved to `@liveblocks/client`, the `ResolveUsersArgs`
and `ResolveMentionSuggestionsArgs` types are now exported from
`@liveblocks/client` as well, instead of `@liveblocks/react`.
