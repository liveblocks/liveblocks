...
{/* TODO update snippet to say "@latest" */}
...
Because of this change, `CommentBodyResolveUsersArgs` no longer exists, and you
should use `ResolveUsersArgs` instead.

```tsx highlight="5,11"
// ❌ Before - Liveblocks 1.9
import { CommentBodyResolveUsersArgs } from "@liveblocks/client";

// ✅ After - Liveblocks 1.10
import { ResolveUsersArgs } from "@liveblocks/client";
```

## Comments CSS variables

All `elevation` and `tooltip`
[CSS variables](/docs/api-reference/liveblocks-react-comments#CSS-variables)
have been removed:
...
:root {
  ...
  --lb-tooltip-shadow: 5px 5px 5px black;
}
```
...
Also note that you’re now required to use `box-shadow` to replace the two
`shadow` variables.
...
[overrides API reference](/docs/api-reference/liveblocks-react-comments#Overrides).
