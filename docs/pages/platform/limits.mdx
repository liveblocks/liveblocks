...
| [`yDocUpdated`](/docs/platform/webhooks#YDocUpdatedEvent) webhook event throttle       | <Limits.StarterYdocUpdatedWebhookEventThrottle />    | <Limits.ProYdocUpdatedWebhookEventThrottle />    | Custom     |
| [`storageUpdated`](/docs/platform/webhooks#StorageUpdatedEvent) webhook event throttle | <Limits.StarterStorageUpdatedWebhookEventThrottle /> | <Limits.ProStorageUpdatedWebhookEventThrottle /> | Custom     |
...
In each individual project, we allow up to 50,000 requests every 60 seconds when using our [REST API](/docs/api-reference/rest-api-endpoints) and [`@liveblocks/node`](/docs/api-reference/liveblocks-node).
