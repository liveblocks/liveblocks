...
- **Data stored**: Collaborative state written in text editor and sync engine
  integrations, and files uploaded through Storage. This includes Tiptap,
  BlockNote, Lexical, Storage (e.g. `LiveObject` and `LiveFile`), and Yjs (e.g.
  `Y.Doc`).
...
Cloudflare’s global edge network. This data is handled on the edge, as close to
