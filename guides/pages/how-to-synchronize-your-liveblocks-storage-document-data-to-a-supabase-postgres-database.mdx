...
This guide assumes you already have a Liveblocks Storage project set up. If
...
<Steps>
  ...
  <Step>
    ...
    <StepContent>
      Click the **“Create endpoint…”** button on the webhooks dashboard to start setting
      up your webhook.
      <Figure>
        <Image
  ...
  <Step>
    ...
    <StepContent>
      Click **“Create endpoint”** at the bottom, then find your **“Webhook secret key”** on the next
      page, and copy it.
      <Figure>
        <Image
  ...
  <Step lastStep>
    ...
    <StepContent>
      Note that you filter specifically for `storageUpdated` events, but we’re
...
`storageUpdated` events, but we’re ignoring this for now so we can test more
easily. Let’s go back to the endpoint.
...
export async function POST(request: Request) {
  ...
  // When Storage document data has been udpated
  if (event.type === "storageUpdated") {
    const { roomId } = event.data;
...
<Steps>
  ...
  <Step>
    ...
    <StepContent>
      ...
      CREATE TABLE documents ( roomId VARCHAR(255) PRIMARY KEY, storageData TEXT NOT
      NULL ); ```
      <Figure>
        <Image
