...
[basic Comments example](/examples/comments/nextjs-comments) which has this set
up already.
...
to check for [thread notification](/docs/platform/webhooks#Thread-notification),
as we’re specifically listening for new comments. We can do this using
...
We can also edit
[`prepareThreadNotificationEmailAsReact`](/docs/api-reference/liveblocks-emails#prepare-thread-notification-email-as-react)
to allow for custom components in comment bodies, for example we can add margin
around a paragraph, color mentions, and underline links.
