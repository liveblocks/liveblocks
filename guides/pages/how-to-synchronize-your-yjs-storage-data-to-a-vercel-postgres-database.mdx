...
command to generate a temporary URL that lasts while your localhost server is
running:
...
If you correctly input your IP address, the URL `localtunnel` provides can be
...
<Steps>
  ...
  <StepCompact>
    ...
    endpoint, but for now enter your `localtunnel` URL.
  </StepCompact>
...
export async function POST(request: Request) {
  ...
  if (event.type === "ydocUpdated") {
    ...
    // Your Yjs document data
    const yDocData = await response.json();
...
The first step is to create your database. Navigate to the
[Vercel Storage](https://vercel.com/dashboard/stores) dashboard and click
"Create Database", select "Postgres", give it a name and create it.

Follow the getting started guide on the next page to connect the database to
your Vercel project, link your environment variables, and install the packages.

<Steps>

<Step>
<StepTitle>Create your</StepTitle>
<StepContent>
