...
rooms using custom metadata and room ID prefixes using our custom query
...
for your rooms. Filters can be combined using `AND` logic.
...
You can also filter rooms by whether they contain threads:

```text
hasThreads:true
```

Use `hasThreads:false` to return rooms with no threads. Resolved and private
threads count but deleted threads do not.
...
[read our guide](/docs/api-reference/liveblocks-node#post-rooms).
