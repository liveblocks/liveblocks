...
- **LiveText values:** Patch LiveText using its Plain LSON shape:
  `{ "liveblocksType": "LiveText", "data": LiveTextData }`. You can patch fields
  under `data`, for example `/text/data/0/0`. LiveText versioning is an internal
  detail and is not part of this API.
...
LiveText is patched with standard JSON Patch operations against its serialized
segments. Each segment is `[text]` or `[text, attributes]`.

```json
[
  { "op": "replace", "path": "/text/data/0/0", "value": "Hello" },
  { "op": "add", "path": "/text/data/0/1", "value": { "bold": true } }
]
```

You can replace a whole LiveText node by passing a Plain LSON LiveText value.
...
[
  {
    "op": "replace",
    "path": "/text",
    "value": { "liveblocksType": "LiveText", "data": [["Hello world"]] }
  }
]
```

Use `test` for optimistic checks against the current text data.

```json
[
  { "op": "test", "path": "/text/data", "value": [["Original"]] },
  { "op": "replace", "path": "/text/data/0/0", "value": "Updated" }
]
```
