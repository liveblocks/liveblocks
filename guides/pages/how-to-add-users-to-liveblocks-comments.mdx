...
[authentication guide for your framework](/docs/rooms/authentication) and
authenticate your app, as this is necessary for Comments.

Make sure to follow the metadata step and attach the name of your user, along
with the URL of their avatar, as these properties will both be used in the
[default components](/docs/products/comments#Components). Here’s an example
using access token authentication, with an email address as a user’s ID.
...
Don’t forget to modify your `UserMetadata` type in `liveblocks.config.ts` to
match the metadata format, adding type hints to your editor.
...
Any metadata you add is made saved and accessible in your Comments app with the
...
[`resolveUsers`](https://liveblocks.io/docs/api-reference/liveblocks-react#resolveUsers).
...
<Steps>
  <Step>
    ...
    <StepContent>
      The `resolveUsers` function is passed as an option to
      [`createRoomContext`](/docs/api-reference/liveblocks-react#createRoomContext)
      at the bottom of your `liveblocks.config.ts` file—let’s add it. This function
      provides you with `userIds`, an array of user IDs that have created a thread or comment.
      These `userIds` match the IDs set when authenticating users in your app.
      
      ```ts highlight="7-13" file="liveblocks.config.ts"
      // ...
  ...
  <Step>
    ...
    <StepContent>
      ...
      array with each user, in the same order you received the IDs.
...
<Steps>
  <Step>
    ...
    <StepContent>
      ...
      ```ts highlight="10-17" file="liveblocks.config.ts"
      // ...
  ...
  <Step>
    ...
    <StepContent>
      ...
      ```ts highlight="11-21" file="liveblocks.config.ts"
      // ...
