...
meta:
  title:
    "How to synchronize your Storage document data to a PlanetScale MySQL
    database"
  description:
...
[Storage](/realtime-infrastructure?primitive=storage), a persisted conflict-free data store. Using our webhooks
and the [`@liveblocks/node`](/docs/api-reference/liveblocks-node) package, you
can then retrieve the Storage document data as it changes, and store it in your
database.

## What we’re building

In this guide we’ll be linking a Sync application up to a
...
- [StorageUpdated](/docs/api-reference/webhook-events#StorageUpdatedEvent) webhook event
...
This guide assumes you already have a Liveblocks project with Storage set up. If
you haven’t already got one, you can select _Custom_ on our
[getting started page](https://liveblocks.io/docs), choose a
...
Note that the [StorageUpdated](/docs/api-reference/webhook-events#StorageUpdatedEvent)
...
Make sure to add your “Webhook secret” from the Liveblocks dashboard—in a real
project we’d recommend using an environment variable for this.

```ts highlight="1,3-5,11-21"
import { WebhookHandler } from "@liveblocks/node";

// Add your webhook secret from a project's webhooks dashboard
...
this with
[`getStorageDocument`](/docs/api-reference/liveblocks-node#get-rooms-roomId-storage)
from [`@liveblocks/node`](/docs/api-reference/liveblocks-node). To use it, you
need to add your secret key from your project page.
...
export async function POST(request: Request) {
  ...
  if (event.type === "storageUpdated") {
    ...
    // +++
    // Get Storage document data as JSON
...
export async function POST(request: Request) {
  ...
  if (event.type === "storageUpdated") {
    ...
    try {
      await connection.query(sql, [roomId, JSON.stringify(storageData)]);
    } catch (err) {
...
[webhooks guide](/docs/platform/webhooks). We also have an API reference for
[`getStorageDocument`](/docs/api-reference/liveblocks-node#get-rooms-roomId-storage).
