---
meta:
  title: "How to send email notifications when text mentions are created"
  description:
...
[Text Editor](/docs/products/text-editor). Using our webhooks and REST API, it’s
possible get unread text mention to send an email to users when they’re
mentioned in a document. Notifications can also be displayed in your app using
...
<Banner title="Supported text editors">

In this guide we'll use only
[Lexical](/docs/api-reference/liveblocks-react-lexical) text editor. The one of
the two we support officialy at Liveblocks along with
...
you don’t have one yet, you can
[get started with Lexical](docs/get-started/nextjs-lexical), and come back after
you’re set up. You could also use our
[basic LExical example](/examples/collaborative-text-editor/nextjs-lexical)
which has this set up already.
...
[overview page](/docs/products/text-editor/lexical#Notifications).
...
We can also edit
[`prepareTextMentionNotificationEmailAsReact`](/docs/api-reference/liveblocks-emails#prepare-text-mention-notification-email-as-react)
to allow for custom components for the text mention and its surrounding text,
for example we can customize the container, color mentions and so.
