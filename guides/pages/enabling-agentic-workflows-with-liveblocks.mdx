...
- `ttl` — how long (in seconds) the presence should live (minimum: 2,
  maximum: 3599)
...
`useOthers` alongside real users, so existing avatar stack components work
without any changes.
...
`@AI Agent, please review this form`). The `commentCreated` webhook fires for
every new comment, giving you a place to detect the mention and dispatch the
agent.

#### Step 1 — Register the agent as a mentionable user

Add the agent's ID to `resolveUsers` and `resolveMentionSuggestions` in your
`LiveblocksProvider` so it appears in the `@` mention picker alongside real
users.
...
#### Step 2 — Handle the `commentCreated` webhook
...
`getMentionsFromCommentBody` to extract all mentions and check whether the
agent's ID is among them.
...
- **External events** — Slack messages, GitHub PR events, email, Zapier
  triggers, or any inbound webhook that carries a `roomId` can start an agent
  run.
...
- **CrewAI** — Wrap the REST calls in a custom `Tool` and pass it to your
  agents. Agents can then set presence or patch storage as part of a multi-step
  task.
...
- [REST API reference](/docs/api-reference/rest-api-endpoints) — complete
