...
[ID tokens](/docs/api-reference/authentication#id-token) instead. Access tokens
have [limitations when granting nested permissions](/docs/api-reference/authentication/access-token#limitations).

</Banner>

With [access tokens](/docs/api-reference/authentication/access-token) we always
recommend using a
[naming pattern](/docs/api-reference/authentication/access-token#Naming-pattern)
to grant access to multiple rooms at once, for example every room in a user’s
organization.

```ts
// ✅ Grants access to every `acme` organization room
session.allow(`acme:*`, ["*:write"]);
```
...
[Notifications](/docs/products/comments/email-notifications) is authenticating,
which is why we’re checking if it exists. Notifications works across rooms, and
