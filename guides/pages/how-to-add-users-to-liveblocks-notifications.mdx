...
notifications with [Comments](/docs/products/comments) or
[Text editor](/docs/use-cases/text-editor), you’ll notice that each user is
currently “Anonymous”, and that there’s no way to mention or tag other users. To
enable these features, we need to tell Notifications where to find your users’
information.
...
and authenticate your app, as this is necessary for Notifications.
...
in the [default components](/docs/products/notifications/default-components).
Here’s an example using ID token authentication, our recommended method, with an
email address as a user’s ID.
