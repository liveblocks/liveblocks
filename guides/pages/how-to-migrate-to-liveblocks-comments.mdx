...
This equivalent to a document in your project. Make sure to read the sections on
permissions under [access tokens](/docs/authentication/access-token#permissions)
or [ID tokens](/docs/authentication/id-token#permissions) to fully understand
...
Next up is creating threads in the room. Before starting, make sure you under
the [concepts behind Comments](/docs/products/comments/concepts). When a thread
is created, the first comment is also created, and you can do this with
...
Liveblocks stores
[is each user’s ID](/docs/products/comments/users-and-mentions). Other user info
is retrieved in-app by Comments with
...
async function migrateDocument({ oldDocumentId, oldDocumentThreads }) {
  ...
  for (const oldThread of oldDocumentThreads) {
    ...
    for (const oldComment of otherComments) {
      ...
      for (const oldReaction of oldComment) {
        // Add a reaction to a comment
