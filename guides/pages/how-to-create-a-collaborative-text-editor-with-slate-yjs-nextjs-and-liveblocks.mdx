...
Note: if you see an error resembling:
```
...
```
The `transpilePackages` setting is not working correctly and you may need to upgrade to a newer version of the `next`
package.
...
<details>
<summary>Example for the older Next pages API routes.</summary>
...
communicating with the back end. You can do this by modifying `createClient` in
...
features, such as [Presence](/docs/products/presence). The
