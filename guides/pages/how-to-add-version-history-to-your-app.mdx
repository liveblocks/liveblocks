...
You can also create versions manually, using the
[Create version history snapshot](/docs/api-reference/rest-api-endpoints#create-version-history-snapshot)
REST API. A manual snapshot is useful for creating a version at a meaningful
moment, for example when a user clicks a _Save version_ button.

{/* TODO add Node.js code when supported */}

```ts file="app/api/create-version/route.ts"
export async function POST() {
  // Create a version snapshot of the room
  // +++
  const response = await fetch(
    "https://api.liveblocks.io/v2/rooms/my-room-id/versions",
    {
      method: "POST",
...
version can be used for both. However, previewing
...
You can also list versions from the server with the
...
Versions can also be deleted from the server with the
[Delete a version](/docs/api-reference/rest-api-endpoints#delete-version) REST
API.
