...
<Banner title="Threads vs comments">
  ...
  comments. When you create a thread, you also create the first comment in the
  thread.
</Banner>

The next step is to use the
[Get Comment API](/docs/api-reference/rest-api-endpoints#get-comment) to
retrieve the entire comment’s data. To do this, we’ll create a utilities file
containing a helper function that takes the IDs returned from the webhook.
...
thread—we’ll be sending a notification to each of these users.
...
Let’s import these functions into our endpoint to fetch the necessary data.
...
```ts highlight="3,5-6,45-55" file="route.ts" isCollapsable isCollapsed
import { WebhookHandler } from "@liveblocks/node";
import { getComment, getParticipants } from "./utils";
