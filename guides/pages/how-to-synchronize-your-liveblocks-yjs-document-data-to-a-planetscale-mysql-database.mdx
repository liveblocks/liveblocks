...
Using our webhooks and the
[`@liveblocks/node`](/docs/api-reference/liveblocks-node) package, you can then
retrieve the Yjs document data as it changes, and store it in your database.
...
- [YDocUpdated](/docs/api-reference/webhook-events#YDocUpdatedEvent) webhook event
- [`getYjsDocument`](/docs/api-reference/liveblocks-node#get-rooms-roomId-ydoc)
...
[text editor](/docs/get-started/multiplayer/text-documents) or
[code editor](/docs/get-started/multiplayer/text-documents), both of which use Yjs, and come
...
Note that the [YDocUpdated](/docs/api-reference/webhook-events#YDocUpdatedEvent) webhook
...
export async function POST(request: Request) {
  ...
  if (event.type === "ydocUpdated") {
    ...
    // +++
    // Get the Yjs document as JSON
    const yDocData = await liveblocks.getYjsDocument(roomId);
    // +++
...
export async function POST(request: Request) {
  ...
  if (event.type === "ydocUpdated") {
    ...
    try {
      await connection.query(sql, [roomId, JSON.stringify(yDocData)]);
    } catch (err) {
