...
`@liveblocks/node` to retrieve the inbox notification, the corresponding
thread’s data, and a list of each thread participant. In Liveblocks Comments, a
participant refers to a user that has commented or been mentioned in a
...
[`getThread`](/docs/api-reference/liveblocks-node#get-thread), and
...
step before sending the notifications—formatting each comment’s text, found
inside `comment.body`.
...
Now that the HTML’s been generated, we can send the notifications. Earlier we
retrieved `participants`, a list of `userIds` that have been mentioned in the
thread. You most likely have user information in your database, which you can
retrieve from these `userIds`.
...
- How to use [webhooks](/docs/platform/webhooks) and the
  [`ThreadEmailNotiticationEvent`](/docs/platform/webhooks#ThreadEmailNotiticationEvent).
