...
With our webhooks and REST API, it’s possible to aggregate a list of unread
comments from the last 30 minutes into a single email, and send this to your
user.
...
First, we need to understand how inbox notifications in
[Comments](/docs/products/comments). When a user mentions you in a thread, or if
you’ve previously participated in it, then you will receive an inbox
notification for it. For example, in the image below, _Quinn Elton_ and _Emil
Joyce_ are both participants.
...
The
[InboxNotification](/docs/api-reference/liveblocks-react-comments#InboxNotification)
component allows you to create a UI component that displays a live list of these
notifications across your rooms, which is very helpful. But how do you send
emails to your users letting them know they have unread notifications in their
inbox?
...
You now have a URL that can be used in the webhook dashboard.
...
Now that the HTML’s been generated, we can send the notification emails. Earlier
we retrieved `userId`, the ID of the user that’s receiving the notification. You
can use this to get the user’s email address, before sending the email itself.
