...
export function formatAST(
  ...
  if (Array.isArray(node)) {
    const lines = node.map((n) => formatAST(n)).join(",\n");
  ...
  if (
    ...
    if (ast.isLiteral(node)) {
      return `${colors.yellow(node._kind)} ${formatAST(node.value)}`;
    } else if (ast.isComment(node) && node.range) {
      return `${colors.yellow(node._kind)} ${formatAST(
        node.text
      )} ${colors.gray(`/* ${node.range.join("-")} */`)}`;
    }
