...
types—we now want a [`LiveList`][] called `people` containing a person.
...
Next, let’s modify the mutations—switch to [App.tsx][] and look at `updateName`.
We need to update this function to take a list `index`, which we can then use to
get the current `person` with [`LiveList.get`][].
