...
Storage is part of our [Sync Datastore](/docs/collaboration-features/multiplayer/sync-engine) product.
...
- [`LiveFile`][] - An immutable reference to an uploaded file

These data types can be combined and nested. `LiveObject`, `LiveList`, and
`LiveMap` create a realtime data structure that automatically updates when
multiple users modify data at the same time, whilst handling any conflicts along
the way.

Use `LiveFile` for large files such as images and videos instead of storing
binary data directly in the other Storage types.
