...
Conflict-free data, or _storage_, allows us to create persistent data that
multiple users can edit simultaneously. This is ideal for storing real-time
document state, such as shapes on a canvas, paragraphs in a text editor, or
cells in a spreadsheet.

### Data structures

To build applications using conflict-free data, you can make use of our
real-time data types. Each type is similar to a JavaScript counterpart:

- [`LiveObject`][] - JavaScript Object
- [`LiveList`][] - JavaScript Array
- [`LiveMap`][] - JavaScript Map

These data types can be combined and nested, to create a real-time data
structure that automatically updates when multiple users modify data at the same
time, whilst handling any conflicts along the way.
...
To use this real-time data in your app, we can import a hook named
[`useStorage`][]. This hook allows us to select part of our storage, before
returning its value, and automatically updating on real-time changes.
...
Each real-time data structure is converted to an immutable copy of its
...
On the next page we’ll be updating storage.
