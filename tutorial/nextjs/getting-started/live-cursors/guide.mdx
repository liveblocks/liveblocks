...
Switch to [Room.tsx][], import the hook from [liveblocks.config.ts][], and add it to your component.
...
`connectionId` is a unique number for each online user that can be used as a key.
...
We can actually make these cursors even smoother, by asking Liveblocks to send more frequent updates. You can do this in [liveblocks.config.ts][] by adding the `throttle` property.

```tsx highlight="3"
const client = createClient({
  publicApiKey: "pk_dev_PHigBerT_qlDHfGgAVlgreL1PZBL05meCHlRQkli3oZ3tERpzrqhoZV6ioNjrAOu",
  throttle: 16,
});
```
