...
To display presence, we’ll add [`useMyPresence`][] to [Room.tsx][], and quickly display its value, similarly to `useState`.
...
Great! You should now be seeing `cursor: null` in the preview window. On the next page we’ll be creating live cursors with presence.
