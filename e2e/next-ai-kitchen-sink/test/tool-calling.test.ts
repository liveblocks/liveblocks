...
test.describe("Tool Calling", () => {
  test("should perform todo operations via AI tool calls", async ({ page }) => {
    ...
    if (await todoChatLink.isVisible()) {
      const deleteButton = todoChatLink.locator('..').locator('button:has-text("Delete")');
    ...
    await expect(page.locator('li:has-text("Buy test item for AI")')).toBeVisible();
    ...
    await expect(page.locator('text=Buy test item for AI')).toBeVisible({ timeout: 30000 });
    ...
    await sendAiMessage(page, "Toggle the completion status of 'Buy test item for AI'");
    ...
    const completedTodo = page.locator('ul.flex li.line-through:has-text("Buy test item for AI")');
    ...
    await expect(page.locator('text=Okay to delete?')).toBeVisible({ timeout: 10000 });

    // Click the Confirm button (look for various possible button texts)
    const confirmButton = page.locator('button').filter({ hasText: /Confirm|Yes|OK|Delete/ }).first();
    ...
    const deletedText = page.locator('text=Deleted').or(page.locator('text=deleted'));
    if (await deletedText.isVisible()) {
      // If deletion was successful, the item should be gone from the main list
      await expect(page.locator('ul.flex li:has-text("Buy test item for AI")')).not.toBeVisible();
    } else {
      // If deletion failed or was cancelled, just verify the AI handled it gracefully
      console.log("Deletion may have failed or been cancelled - checking AI response");
    }

    // Step 7: Verify the remaining todos are still there in the main list
    await expect(page.locator('ul.flex li:has-text("Get groceries")')).toBeVisible();
    await expect(page.locator('ul.flex li:has-text("Go to the gym")')).toBeVisible();  
    await expect(page.locator('ul.flex li:has-text("Cook dinner")')).toBeVisible();
  });
  ...
  test("should handle cancellation of delete confirmation", async ({ page }) => {
    // Clean up first
    await page.goto("/chats");
    const todoChatLink = page.locator('a[href="/chats/todo"]');
    if (await todoChatLink.isVisible()) {
    ...
    await todoInput.press('Enter');
    await expect(page.locator('ul li:has-text("Test cancellation item")')).toBeVisible();
    ...
    await expect(page.locator('text=denied').or(page.locator('text=cancelled'))).toBeVisible({ timeout: 10000 });
  });
});