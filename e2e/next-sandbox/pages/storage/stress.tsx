...
function stableStringify(obj: unknown): string {
  return JSON.stringify(obj, (key, value) => {
    if (value instanceof Map) {
      ...
      const sorted = [...value.entries()].sort(([a], [b]) => a.localeCompare(b));
...
function collectAttachmentPoints(root: LiveObject<LsonObject>): AttachmentPoint[] {
...
function Sandbox() {
  ...
  const shrink = useMutation(
    ({ storage }, count: number, times: number) => {
      for (let t = 0; t < times; t++) {
        for (let i = 0; i < count; i++) {
          const points = collectAttachmentPoints(storage);
          // Filter to points that have something to remove
          const nonEmpty = points.filter((p) => {
            if (p.type === "object") return Object.keys(p.node.toObject()).length > 0;
  ...
  return (
    <div>
      <h3>
        <a href="/">Home</a> › Storage › Stress Test
      </h3>

      <div style={{ marginBottom: "16px" }}>
        <div style={{ display: "flex", margin: "8px 0", gap: "4px", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", margin: "8px 0", gap: "4px", alignItems: "center" }}>
          ...
          {(Object.keys(SHRINK_COUNTS) as (keyof typeof SHRINK_COUNTS)[]).map((size) => (
            <Button
              key={size}
              id={`shrink-${size}`}
        ...
        <div style={{ display: "flex", margin: "8px 0", gap: "4px", alignItems: "center" }}>
          ...
          {(Object.keys(CHANGE_COUNTS) as (keyof typeof CHANGE_COUNTS)[]).map((size) => (
            <Button
              key={size}
              id={`change-${size}`}
        ...
        <div style={{ display: "flex", margin: "8px 0", gap: "4px", alignItems: "center" }}>
          ...
          <Button
            id="disconnect"
            enabled={status !== "initial" && status !== "disconnected"}
      ...
      <h4>
        Storage{" "}
        <code style={{ fontSize: "12px", background: "#eee", padding: "2px 6px" }}>
