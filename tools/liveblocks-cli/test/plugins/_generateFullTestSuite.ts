 ...
 * - In shared/liveblocks-server, to test the InMemoryStorageDriver
...
import {
  ...
  LogTarget,
  quote,
  test_only__Storage as Storage,
  test_only__YjsStorage as YjsStorage,
} from "@liveblocks/server";
...
// Bun-specific way to turn b64 string into Uint8Array
function b64ToBytes(base64: string): Uint8Array {
  return Buffer.from(base64, "base64");
}

// Bun-specific way to turn Uint8Arrays to b64 string
...
async function withDefaultDocument(driver: IStorageDriver) {
  ...
  const db = driver.load_nodes_api(blackHole);
