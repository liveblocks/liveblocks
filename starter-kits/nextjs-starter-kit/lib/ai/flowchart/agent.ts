...
import {
  ...
  type FlowchartNode,
  type Point,
} from "@/components/Flowchart/shared";

const AI_MODEL = "openai/gpt-5.4-nano";
...
const nodeDataSchema = z.object({
  ...
  color: z.enum(BLOCK_COLOR_NAMES),
});
const edgeDataSchema = z.object({
  label: z.string().optional(),
});

type FlowMutateStorageCallback = Parameters<
  MutateFlowOptions<FlowchartNode, FlowchartEdge>["client"]["mutateStorage"]
>[1];

const flowLiveblocksClient: MutateFlowOptions<
  FlowchartNode,
  FlowchartEdge
>["client"] = {
  async mutateStorage(roomId, callback) {
    await liveblocks.mutateStorage(roomId, async (context) => {
      // XXX: mutateFlow's node helper expects an unmerged LiveObject<LsonObject>
      // root, while the starter-kit server client is globally typed to the
      // full Liveblocks["Storage"] shape. Runtime data is the same root object.
      await callback(
        context as unknown as Parameters<FlowMutateStorageCallback>[0]
      );
    });
  },
};
...
export async function runFlowchartAgent(
      ...
      try {
        ...
        const result = await generateText({
          ...
          system: dedent`
            ...
            - Use moveNode to change position, resizeNode to change width/height, and updateNodeData for label/shape/color -- never mix them.
          `,
          prompt: dedent`
            <diagram>
  ...
  await setPresence({ thinking: false, ttl: PRESENCE_DONE_TTL_SECONDS });
