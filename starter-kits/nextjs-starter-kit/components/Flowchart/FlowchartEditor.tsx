...
import { Cursor, Icon } from "@liveblocks/react-ui";
import {
...
import { submitFlowchartAgentAction } from "@/lib/ai/flowchart/input-agent";
...
type FlowchartBlockStyle = CSSProperties & {
  "--flowchart-block-color": string;
};
...
function getBlockStyle(color: BlockColor | undefined): FlowchartBlockStyle {
  return {
    "--flowchart-block-color": getBlockColor(color),
  };
}

function getMetadataPoint(metadata: Liveblocks["ThreadMetadata"]): Point {
  return {
...
const BlockNode = memo(function BlockNode({
  ...
  const { updateNode } = useReactFlow<FlowchartNode, FlowchartEdge>();
...
const FlowchartLabelEdge = memo(function FlowchartLabelEdge({
  id,
  data,
  selected,
  markerEnd,
  ...
  const { updateEdge } = useReactFlow<FlowchartNode, FlowchartEdge>();
  const [labelDraft, setLabelDraft] = useState(data?.label ?? "");
      ...
      <EdgeLabelRenderer>
        ...
        >
          {showLabel ? (
            ...
            <span className="flowchart-edge-label-readonly" title={trimmedLabel}>
              {trimmedLabel}
            </span>
          ) : null}
        </div>
...
const MiniMapNode = memo(function MiniMapNode({
  ...
  if (shape === "diamond") {
    return (
      <polygon
        points={`${boundsX + width / 2},${boundsY} ${boundsX + width},${
          boundsY + height / 2
        } ${boundsX + width / 2},${boundsY + height} ${boundsX},${
          boundsY + height / 2
        }`}
        fill={color}
      />
  ...
  return (
    <div
      ...
      style={{
        ...
        top: 0,
        left: 0,
        transform: `translate(${position.x - DEFAULT_BLOCK_SIZE / 2}px, ${
          position.y - DEFAULT_BLOCK_SIZE / 2
        }px)`,
        zIndex: 999999,
        pointerEvents: "none",
        cursor: "none",
...
function FlowCursor({ userId, connectionId }: CursorsCursorProps) {
  ...
  const isAgentCursor = userId === AI_USER_ID;
...
function Flow() {
  ...
  const [chatAiEnabled, setChatAiEnabled] = useState<boolean | null>(null);
  ...
  useEffect(() => {
    ...
    getAiConfig()
      .then(({ chatAiEnabled }) => {
        if (!cancelled) {
          setChatAiEnabled(chatAiEnabled);
        }
      })
      .catch(() => {
        if (!cancelled) {
          setChatAiEnabled(false);
        }
      });
    ...
    (changes: NodeChange<FlowchartNode>[]) => {
      ...
      if (removedIds.length > 0) {
        ...
        for (const thread of threads) {
          ...
          const point = normalizedToFlowPoint(
            node,
            getMetadataPoint(thread.metadata)
          );
  ...
  const aiDisabled = chatAiEnabled === false || isAgentPending;
          ...
          <Panel position="top-left">
            <form className="flowchart-agent-panel" action={formAction}>
              ...
              <textarea
                ...
                aria-label={`Ask AI to edit ${initialDocument.name}`}
                value={agentPrompt}
                disabled={aiDisabled}
                onChange={(event) => {
              ...
              />
              {chatAiEnabled === false ? (
                <p className="flowchart-agent-notice">
                  AI prompting is disabled because AI_GATEWAY_API_KEY is not
                  configured.
                </p>
              ) : null}
              {agentState?.error ? (
                <p className="flowchart-agent-error">{agentState.error}</p>
              ) : null}
              <button
                type="submit"
