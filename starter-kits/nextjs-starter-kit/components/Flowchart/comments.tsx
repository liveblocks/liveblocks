...
function getMetadataPoint(metadata: ThreadData["metadata"]): Point {
  return {
...
export function getThreadPinFlowPosition(
  ...
  const metadataPoint = getMetadataPoint(thread.metadata);

  if (attachedToNodeId != null) {
    const node = nodes.find((node) => node.id === attachedToNodeId);

    if (node) {
      return normalizedToFlowPoint(node, metadataPoint);
    }
  }
...
export function FlowchartCanvasComments({
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      const thread = active.data.current?.thread;
