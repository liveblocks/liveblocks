...
function isChatFeedMessage<T extends { data: Liveblocks["FeedMessageData"] }>(
  message: T
): message is T & { data: ChatFeedMessageData } {
  return "role" in message.data;
}
...
function ChatWindow({
  ...
  const ensureFeed = useCallback(
    async (id: string, title: string) => {
      ...
      try {
        ...
        // Feed already exists, likely created by another user.
      }
    },
    [createFeed]
  );
  ...
  useEffect(() => {
    const latestProposal = [...sorted].reverse().find(
      (message) =>
  ...
  const postReply = useCallback(
    async (history: { role: "user" | "assistant"; content: string }[]) => {
      ...
      await fetch("/api/slideshow/ai-reply", {
        method: "POST",
  ...
  const send = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        // Best-effort chat UX: feed write failures should not break the editor.
      } finally {
        ...
        >
          ...
          <PromptInputFooter>
            ...
            <div className="flex items-center gap-1">
              ...
              <PromptInputSubmit
                disabled={aiThinking || streamingInProgress}
                status={aiThinking || streamingInProgress ? "submitted" : undefined}
              />
            </div>
          </PromptInputFooter>
        </PromptInput>
