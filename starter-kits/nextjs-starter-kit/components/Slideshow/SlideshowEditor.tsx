...
export function SlideshowEditor() {
  return (
    <ClientSideSuspense
      fallback={
        <div className="tw flex h-full items-center justify-center text-muted-foreground">
          <Loader size={20} />
        </div>
      }
    >
...
function SlideshowApp() {
  ...
  const resolvePreviewedProposal = useCallback(
    ...
    [handleProposalApplied, previewedProposal, resolvingProposal, room.id]
  );
    ...
    try {
      ...
      for (const id of exportedSlideIds) {
        ...
        try {
          ...
          const element = iframeDocument?.body ?? iframeDocument?.documentElement;
          if (!element) {
      ...
      await pptx.writeFile({
        fileName: `${sanitizeFilename(initialDocument.name)}.pptx`,
      });
    } finally {
      setExporting(false);
    }
  }, [exporting, initialDocument.name, room]);
  ...
  if (!slideId) {
    return (
      <div className="tw flex h-full w-full items-center justify-center bg-neutral-50 text-muted-foreground">
        <Loader size={20} />
      </div>
    );
  }
  ...
  return (
    <div className="tw flex h-full min-h-0 w-full gap-2.5 overflow-hidden bg-neutral-50 p-2.5">
      <SlideSidebar
        slideIds={slideIds}
