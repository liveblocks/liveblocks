...
type SlideshowThread = ThreadData & {
...
function isSlideshowThread(thread: ThreadData): thread is SlideshowThread {
  const { x, y, zIndex, slideId } = thread.metadata;
  return (
    typeof x === "number" &&
    typeof y === "number" &&
...
function useMaxZIndex(threads: readonly SlideshowThread[]) {
  return useMemo(() => {
    let max = 0;
...
export function SlidePreview({
  ...
  const slideThreads = useMemo(
    () =>
      threads
        .filter(isSlideshowThread)
        .filter((thread) => thread.metadata.slideId === slideId),
    [slideId, threads]
  );
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "absolute left-1/2 top-1/2 overflow-visible bg-white shadow-2xl",
          proposal && proposalHtml !== undefined
...
function SlideCursor({
  ...
  const info = useOther(
    connectionId,
    (other): CursorInfo => ({
      cursorSlideId: other.presence.cursorSlideId ?? null,
