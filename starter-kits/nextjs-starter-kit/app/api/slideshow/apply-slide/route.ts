...
import {
  ...
  getSlideText,
  SLIDES_ARRAY_KEY,
} from "@/components/Slideshow/slide-doc";
...
export async function POST(request: NextRequest) {
  ...
  if (!roomId || !feedId || !messageId || !action) {
  ...
  const messages = await liveblocks.getFeedMessages<ChatFeedMessageData>({
    roomId,
    feedId,
  });
  ...
  if (action === "apply") {
    ...
    newSlideIds = await applyProposalsToYjsDocument(roomId, proposals);
  }

  await liveblocks.updateFeedMessage<ChatFeedMessageData>({
    roomId,
    feedId,
    messageId,
    data: {
