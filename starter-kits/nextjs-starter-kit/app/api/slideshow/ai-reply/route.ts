...
import {
  ...
  type HtmlProposal,
} from "@/lib/ai/slideshow/html-proposals";
import { liveblocks } from "@/liveblocks.server.config";
import type { ChatFeedMessageData } from "@/liveblocks.config";
import { INITIAL_SLIDE_ID } from "@/components/Slideshow/slide-doc";
import { STARTER_SLIDE_HTML } from "@/components/Slideshow/slide-html";
...
export async function POST(request: NextRequest) {
  ...
  const created = await liveblocks.createFeedMessage<ChatFeedMessageData>({
    roomId,
    feedId,
  ...
  const update = (data: AssistantUpdate) =>
    liveblocks.updateFeedMessage<ChatFeedMessageData>({
      roomId,
      feedId,
      messageId,
